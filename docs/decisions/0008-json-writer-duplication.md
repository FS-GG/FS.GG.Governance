# ADR 0008 — Per-projection JSON writer duplication is intentional; byte-identity is guarded by tests, not by extraction
...
| `AuditJson.ofShipDecision` vs `ofShipDecisionWithGeneratedViews` | The genuinely shared sub-steps (`verdictByGate`, `outcomeByGate`, `writeExecution`, `writeCause`) **are** already extracted into `JsonWriters`. `ofShipDecisionWithGeneratedViews` adds only the `generatedViews` array, and `ofShipDecision` now **delegates** to it with an empty list — byte-identical, since `writeGeneratedViews []` emits nothing (landed in #82 / spec 110, A5). |
...
- `AuditJson.ofShipDecision` **delegates** to `ofShipDecisionWithGeneratedViews` with an empty view list, exactly as M-JSON-2 recommended (landed on `main` in #82 / spec 110, A5). This is a pure *internal* dedup, not a contract change: `ofShipDecision`'s public signature is unchanged and `writeGeneratedViews []` emits nothing, so the emitted document is byte-identical (asserted by `AuditJson.Tests`). It is the opposite case from the release-side writers below — delegation here removes a hand-copied body **without** touching a wire contract, whereas sharing the release writers *would* couple independent schema versions. Extraction is declined only where it couples contracts, not where it is a plain within-projection dedup.
...
- The release-side writer ratification carries **no code change** — the writers stay per projection. The one code change M-JSON-2 called for, the `ofShipDecision` delegation, already landed on `main` in #82 (spec 110, A5). #52's four other acceptance criteria were satisfied earlier (M-JSON-1/3 in `c1ee7f9`, M-CLI-5/6 in `6b53332`).
