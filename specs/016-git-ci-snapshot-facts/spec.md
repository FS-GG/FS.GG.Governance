...
- **FR-003**: The snapshot MUST capture **working-tree state** beyond committed history: the set of tracked-but-modified ("dirty") paths and the set of untracked paths, each normalized exactly as in FR-002. The committed `Changed` set and the working-tree state are **two distinct planes**: within the working-tree plane a path MUST appear in at most one of dirty / untracked; the committed `Changed` plane is reported separately and MAY also list a path that has since been modified again. This keeps a local preview honest about what is on disk without hiding committed history.
...
- **FR-007**: Every git and CI-context sensing operation MUST be isolated behind an **injected sensing port** behind a dedicated impure effects boundary using the same discipline as the F08 Host interpreter, so a snapshot is produced against a real fixture repository in tests and **no live network or hosting-provider API** is reached during sensing. (The pure snapshot value carries no such ports.)
...
- **SC-003**: A fixture with committed diffs plus uncommitted dirty and untracked changes reports each path in its correct plane — committed-changed in the committed plane, dirty or untracked in the working-tree plane — with dirty and untracked mutually exclusive and no path mis-categorized within a plane.
...
- Sensing is impure and lives in a dedicated, layered library behind an injected port, using the same effects-boundary discipline as the F08 Host interpreter; the snapshot value it yields is pure, deterministic data with no ports embedded. Tests run against a real fixture git repository (real evidence; no live agent or network).
