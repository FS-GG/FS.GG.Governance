...
module Model =
    ...
    /// Uncommitted working-tree state (US2, FR-003). Categorization is EXCLUSIVE: a path appears
    /// in a snapshot's committed `Changed`, in `Dirty`, or in `Untracked`, never in two
    /// (SC-003). Each list is normalized and deterministically ordered.
