    ...
    testList
        ...
        [
            test "worked example renders byte-for-byte equal to the documented block" {
                ...
                let request =
                    PromptIsolation.assemble (QuestionText "Explain the API?") [ DigestOnly(ArtifactHash "sha:a") ]
                ...
                Expect.isFalse
                    ((idOf record).Contains "2026-06-22T10:00:00Z")
                    "the sensed timestamp must not appear in the identity"
            }
