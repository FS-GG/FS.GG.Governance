...
let private idOf (r: ReviewRecord) : string =
        ...
        [
            ...
            <| fun (repro: ReproducibleFacts) (s1: int) (s2: int) ->
                ...
                let a =
                    {
                        Reproducible = repro
                        Sensed = [ sensedAt "a" (string s1) ]
                    }

                let b =
                    {
                        Reproducible = repro
                        Sensed = [ sensedAt "b" (string s2); sensedAt "c" "z" ]
                    }
                ...
                let variants =
                    [
                        ...
                        { baseRecord with
                            Reproducible =
                                { baseRecord.Reproducible with
                                    Request = requestOf "different question?" []
                                }
                        }
                        "request-payload",
                        { baseRecord with
                            Reproducible =
                                { baseRecord.Reproducible with
                                    Request =
                                        requestOf "Does this doc explain the public API?" [ digestPayload "sha:a" ]
                                }
                        }
                        "model",
                        { baseRecord with
                        ...
                        { baseRecord with
                            Reproducible =
                                { baseRecord.Reproducible with
                                    ModelVersion = modelVersion "2026-07"
                                }
                        }
                        "promptHash",
                        { baseRecord with
                        ...
                        { baseRecord with
                            Reproducible =
                                { baseRecord.Reproducible with
                                    ReviewedArtifacts = [ artifactHash "sha:z" ]
                                }
                        }
                        "responseDigest",
                        { baseRecord with
                            Reproducible =
                                { baseRecord.Reproducible with
                                    ResponseDigest = responseDigest "sha:other"
                                }
                        }
                        "verdict",
                        { baseRecord with
