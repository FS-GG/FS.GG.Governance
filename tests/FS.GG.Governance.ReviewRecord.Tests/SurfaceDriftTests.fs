    ...
    testList
        ...
        [
            ...
            test "the public surface is exactly the two modules (Model + ReviewRecord), nothing else" {
                let typeNames =
                    reviewRecordAsm.GetExportedTypes()
                    |> Array.choose (fun t -> Option.ofObj t.FullName)

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.ReviewRecord.ModelModule"))
                    "Model module is public"

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.ReviewRecord.ReviewRecordModule"))
