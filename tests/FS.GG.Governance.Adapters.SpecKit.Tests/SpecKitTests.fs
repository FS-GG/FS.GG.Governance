...
let tests =
    testList
        ...
        [
            test
                "V1 the adapter is fully specified by the five SPI components + the Bridge, and governs synthetic facts through the kernel only" {
                ...
                Expect.equal
                    rules.Length
                    adapter.Rules.Length
                    "toRules translates exactly the catalog (no extra/missing rules)"
            ...
            test "V1 toRef is injective over all nine artifact kinds (distinct kinds → distinct ArtifactRef)" {
                ...
                Expect.equal
                    (refs |> List.distinct |> List.length)
                    allArtifacts.Length
                    "every artifact maps to a distinct ref"
            }

            test
                "V1 identify keys TaskState/ConstitutionArea by entity (a later fact supersedes) and is injective on value-bearing facts" {
                ...
                Expect.equal
                    ...
                    ids.Length
                    "distinct value-bearing facts get distinct ids"
            }
            ...
            test
                "V2(P1) a whenPhase rule is a definite not-applicable (vacuous Pass) before its phase — including with NO PhaseReached at all" {
            ...
            test "V2(P3/P4) the guard is reified-ness preserving and render/hash distinguish the guarded phase" {
                ...
                Expect.equal
                    (Check.isReified (SpecKit.whenPhase Phase.Plan alwaysFail))
                    true
                    "guarded reified stays reified"

                Expect.isFalse (Check.isReified opaqueCheck) "precondition: opaque is not reified"

                Expect.equal
                    (Check.isReified (SpecKit.whenPhase Phase.Plan opaqueCheck))
                    false
                    "guarded opaque stays opaque"
                ...
                Expect.notEqual
                    (Check.hash gPlan)
                    (Check.hash gTasks)
                    "hash (cache key) distinguishes the guarded phase"
            }
        ]
