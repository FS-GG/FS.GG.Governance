...
let tests =
    testList
        ...
        [
            test
                "V7 Synthetic — render & hash are invariant under the lift for 100% of the catalog (the cache key does not move)" {
                for r in Catalog.catalog do
                    ...
                    Expect.equal
                        (Check.isReified lifted)
                        (Check.isReified r.Check)
                        (sprintf "%s: reified-ness invariant" id)
            }

            test
                "V7 Synthetic — verdict & explanation (provenance proof tree) are identical standalone vs lifted, for 100% of the catalog" {
                for r in Catalog.catalog do
                    let (RuleId id) = r.Id
                    let lifted = Lift.check (|SkP|_|) r.Check
            ...
            test
                "V7 Synthetic — the executable lifted rule preserves Id and Provenance verbatim (law L3) for 100% of the catalog" {
                for r in Catalog.catalog do
                    let (RuleId id) = r.Id
            ...
            test
                "V7 Synthetic — the adapter composes with the second domain and drops out cleanly (independence, law L3)" {
                ...
                Expect.equal
                    withoutSk.Catalog.Length
                    memoAdapter.Rules.Length
                    "dropping SpecKit leaves only the memo domain"

                // The composed project still evaluates through the kernel with no adapter code.
                let supplied: FactSet<ProjectFact> =
                    [
                        {
                            Id = projIdentify (Sk(PhaseReached Phase.Merge))
                            Value = Sk(PhaseReached Phase.Merge)
                            Provenance = []
                        }
                        {
                            Id = projIdentify (Memo(MemoApproved true))
                            Value = Memo(MemoApproved true)
                            Provenance = []
                        }
                    ]
                ...
                Expect.isGreaterThan
                    result.Facts.Length
                    supplied.Length
                    "the composed project derives governance facts through the kernel"
            }
        ]
