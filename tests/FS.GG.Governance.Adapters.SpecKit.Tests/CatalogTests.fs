...
let tests =
    testList
        ...
        [ // ── US3 ──
            test
                "V3 the inner loop is advisory across the whole catalog — Blocking = [] for every inner phase, even when a deterministic check fails" {
                ...
                Expect.isTrue
                    (isFail (Check.eval tainted Catalog.evidenceNotSynthetic.Check))
                    "evidence check fails on synthetic"
            ...
            test "V3 merge is the single fence that flips to a blocking gate; a failing blocking rule refuses the merge" {
                ...
                Expect.isTrue
                    (isFail (Check.eval tainted Catalog.evidenceNotSynthetic.Check))
                    "the blocking rule fails ⇒ merge refused"
            }
            ...
            test "V3 an earlier hard-stop is one opt-in EarlyFences entry — no kernel change" {
                ...
                let dial =
                    { Catalog.defaultDial with
                        EarlyFences = [ ("no-cyclic-tasks", Phase.Tasks) ]
                    }
            ...
            // ── US4 ──
            test
                "V5 constitutionComplete is a definite Unmet on a placeholder area, advisory inner, blocking at merge under the dial" {
                let placeholder =
                    [ fact (PhaseReached Phase.Merge); fact (ConstitutionArea("scope", false)) ]
            ...
            test "V5 the merge blocking set is a function of the dial (asserted by specific RuleId, not by count)" {
                ...
                Expect.equal
                    (mergeBlockingIds
                        { Catalog.defaultDial with
                            BlockingAtMerge = Set.ofList [ RuleId "plan-satisfies-spec" ]
                        })
            ...
            test "V5 the dial assembles the fences — mergeFence always present plus one per EarlyFences entry" {
                ...
                Expect.isTrue
                    (adapter.Fences |> List.exists (fun f -> f.Name = "feature-merge"))
                    "the merge fence is always present"
            }

            // ── US5 ──
            test
                "V4 AutoSynthetic propagates down a TaskDependsOn chain via the kernel's Evidence.effective (not adapter code)" {
                let facts =
                    [
                        fact (TaskState("T1", Synthetic))
                        fact (TaskState("T2", Real))
            ...
            test "V4 evidence-not-synthetic is non-negotiable — Blocking under every dial, and no flag flips it" {
                Expect.equal
                    Catalog.evidenceNotSynthetic.Severity
                    Blocking
                    "evidence-not-synthetic is Blocking by default"
                ...
                for dial in
                    ...
                    let ev =
                        adapter.Rules |> List.find (fun r -> r.Id = RuleId "evidence-not-synthetic")

                    Expect.equal ev.Severity Blocking "the rule stays Blocking under the light dial too"
            }

            test
                "V4 tasksGraphWellFormed: malformed ≠ tainted; definite Unmet; advisory inner, blockable at merge only if promoted" {
                let atTasks extra =
                    fact (PhaseReached Phase.Tasks) :: extra
                ...
                let unresolvedDep =
                    atTasks [ fact (TaskState("A", Real)); fact (TaskDependsOn("A", "Z")) ]
                ...
                let missingDeps =
                    atTasks [ fact (TaskState("A", Real)); fact (TaskDependsOn("Q", "A")) ]

                Expect.isTrue (isFail (Check.eval missingDeps check)) "an undeclared dependent task is a definite Unmet"

                // unresolved skill id
                let unresolvedSkill =
                    atTasks [ fact (TaskState("A", Real)); fact (SkillBound("Q", "skill-x")) ]

                Expect.isTrue
                    (isFail (Check.eval unresolvedSkill check))
                    "a skill bound to an undeclared task is a definite Unmet"
                ...
                Expect.isTrue
                    ((mergeBlockingIds
                        { Catalog.defaultDial with
                            BlockingAtMerge = Set.ofList [ RuleId "tasks-graph" ]
                        })
