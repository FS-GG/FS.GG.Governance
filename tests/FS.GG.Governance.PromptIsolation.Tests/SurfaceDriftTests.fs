...
let private promptIsolationAsm =
    SurfaceDrift.assemblyNamed "FS.GG.Governance.PromptIsolation"
    ...
    testList
        ...
        [
            ...
            test "the public surface is exactly the two modules (Model + PromptIsolation), nothing else" {
                let typeNames =
                    promptIsolationAsm.GetExportedTypes()
                    |> Array.choose (fun t -> Option.ofObj t.FullName)

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.PromptIsolation.ModelModule"))
                    "Model module is public"

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.PromptIsolation.PromptIsolationModule"))
