    ...
    testList
        ...
        [
            test "empty excerpt is Whole, renders exc=w,0:, distinct from a digest and from an absent artifact" {
                ...
                let emptyExcerpt =
                    PromptIsolation.render (requestOf baseInstructions [ Excerpt e ])
            ...
            test "digest-only with empty / unusual supplied token is carried verbatim, never parsed" {
                ...
                let unusualDig =
                    PromptIsolation.render (requestOf baseInstructions [ digestPayload unusual ])
                    |> PromptIsolation.renderedValue

                Expect.stringContains
                    unusualDig
                    (sprintf "dig=%d:%s" (System.Text.Encoding.UTF8.GetByteCount unusual) unusual)
                    "unusual token carried verbatim by length"
            }

            test "repeated/identical-content artifacts are each carried and rendered in order" {
                let request =
                    requestOf baseInstructions [ excerptPayload 3 "abc"; excerptPayload 3 "abc" ]

                let rendered = PromptIsolation.render request |> PromptIsolation.renderedValue

                Expect.stringContains
                    rendered
                    "art=2;exc=w,3:abc;exc=w,3:abc"
                    "two identical excerpts rendered in order"
            }

            testPropertyWithConfig
                fscheckConfig
                "totality: excerpt/assemble/render never throw"
                (fun (i: QuestionText) (b: SizeBound) (content: string) (arts: ArtifactPayload list) ->
