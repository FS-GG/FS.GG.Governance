    ...
    testList
        ...
        [
            ...
            test "instruction-imitating excerpt cannot reach the instruction channel" {
                let request =
                    requestOf baseInstructions [ excerptPayload 100 instructionImitatingText ]
                ...
                Expect.stringContains
                    rendered
                    (sprintf "exc=w,%d:%s" (utf8Len instructionImitatingText) instructionImitatingText)
                    "hostile text lives inside its length-prefixed payload"
            }

            test "fence-hostile content stays wholly inside its length-prefixed payload" {
                // Content carrying \n ; : = , and every marker (instr= art= exc= dig=).
                let request =
                    requestOf baseInstructions [ excerptPayload 1000 fenceHostileText; digestPayload fenceHostileText ]
                ...
                Expect.stringStarts
                    rendered
                    (sprintf
                        "instr=%d:Does this doc explain the public API?\n"
                        (utf8Len "Does this doc explain the public API?"))
                ...
                Expect.stringContains
                    rendered
                    (sprintf "exc=w,%d:%s" (utf8Len fenceHostileText) fenceHostileText)
                    "excerpt payload carries the hostile content by length"

                Expect.stringContains
                    rendered
                    (sprintf "dig=%d:%s" (utf8Len fenceHostileText) fenceHostileText)
                    "digest payload carries the hostile token by length"
            }

            testPropertyWithConfig
                fscheckConfig
                "render injectivity: render a = render b ⇒ a = b"
                (fun (a: ReviewRequest) (b: ReviewRequest) ->
                    if PromptIsolation.render a = PromptIsolation.render b then
                        a = b
                    else
                        true)
            ...
            test "empty/zero forms are three distinct strings" {
                let emptyChannel =
                    PromptIsolation.render (requestOf baseInstructions [])
                    |> PromptIsolation.renderedValue

                let emptyExcerpt =
                    PromptIsolation.render (requestOf baseInstructions [ excerptPayload 100 "" ])
                    |> PromptIsolation.renderedValue

                let emptyDigest =
                    PromptIsolation.render (requestOf baseInstructions [ digestPayload "" ])
                    |> PromptIsolation.renderedValue
