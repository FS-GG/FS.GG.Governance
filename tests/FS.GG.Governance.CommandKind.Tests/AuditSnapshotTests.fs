    ...
    testList
        ...
        [
            test "byte-identical for identical inputs / no-op re-derive is stable" {
                Expect.equal
                    (Audit.snapshotIdentity (snapshotWith runs))
                    (Audit.snapshotIdentity (snapshotWith runs))
                    "stable"
            }
            ...
            test "a duration-only change does NOT change the identity" {
                ...
                let slow =
                    [
                        {
                            Kind = Build
                            Record = makeRecord 10_000_000L
                        }
                    ]

                Expect.equal
                    (Audit.snapshotIdentity (snapshotWith fast))
                    (Audit.snapshotIdentity (snapshotWith slow))
                    "duration excluded"
            }
            ...
            test "a failed/timed-out run is KEPT in Runs with its F051 sentinel exit code (never dropped)" {
                ...
                let (ExitCode startFail) =
                    FS.GG.Governance.GateExecution.Interpreter.startFailureExitCode
