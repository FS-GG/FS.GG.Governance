...
let tests =
    testList
        ...
        [
            ...
            test
                "kindToken is the exhaustive build|test|pack|templateInstantiation|gitDiff|packageInspection|visualCapture table" {
            ...
            test "two runs differing ONLY in SensedDuration share a runIdentity (duration excluded)" {
                let a =
                    {
                        Kind = Build
                        Record = makeRecord 111L
                    }

                let b =
                    {
                        Kind = Build
                        Record = makeRecord 999_999L
                    }
