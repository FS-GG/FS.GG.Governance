    ...
    testList
        ...
        [
            testList
                ...
                [
                    ...
                    test "optional evidenceTag/templateProfile/baseline + check tier parse onto their records" {
                        ...
                        Expect.equal
                            (surfaceById f "public-api").Baseline
                            (Some(Baseline "src/public-api.baseline.txt"))
                            "baseline"

                        Expect.equal
                            (surfaceById f "public-api").EvidenceTag
                            (Some(EvidenceTag "api-surface"))
                            "evidenceTag"

                        Expect.equal
                            (surfaceById f "product-root").TemplateProfile
                            (Some(TemplateProfile "fsharp-lib"))
                            "templateProfile"
                    ...
                    test "a malformed kind scalar ⇒ MalformedValue naming the field" {
                        ...
                        let hit =
                            d
                            |> List.tryFind (fun x -> x.Id = MalformedValue && x.Locator.Field = Some "kind")
                    ...
                    test "a malformed tier scalar ⇒ MalformedValue naming the field" {
                        ...
                        let hit =
                            d
                            |> List.tryFind (fun x -> x.Id = MalformedValue && x.Locator.Field = Some "tier")
            ...
            testList
                ...
                [
                    test
                        "a v1 capabilities.yml ⇒ UnsupportedSchemaVersion naming version 1 + the migration pointer (SC-006)" {
                        let d = diagsOf "migration-v1-capabilities"

                        let hit =
                            d
                            |> List.tryFind (fun x -> x.Id = UnsupportedSchemaVersion && x.File = Capabilities)
                        ...
                        Expect.stringContains
                            hit.Value.Message
                            "migration.md"
                            "message points at the migration guidance"
                    }
                    ...
                    test "duplicate ids among the new kinds ⇒ DuplicateId (both locations)" {
                        ...
                        Expect.isTrue
                            (dups |> List.exists (fun x -> x.Locator.Id = Some "dup-surface"))
                            "duplicate surface id"

                        Expect.isTrue
                            (dups |> List.exists (fun x -> x.Locator.Id = Some "dup-check"))
                            "duplicate check id"
                    }
                    ...
                    test "an unknown field on a new-kind surface ⇒ UnknownField naming the field" {
                        ...
                        let hit =
                            d
                            |> List.tryFind (fun x -> x.Id = UnknownField && x.Locator.Field = Some "templateProfle")
