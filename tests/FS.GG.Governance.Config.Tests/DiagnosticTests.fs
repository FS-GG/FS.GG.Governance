        ...
        [
            ...
            test "a ':' in a check id or domain is rejected as located MalformedValue (CORE-1)" {
                ...
                for field in [ "id"; "domain" ] do
                    match
                        diags
                        |> List.tryFind (fun d -> d.Id = MalformedValue && d.Locator.Field = Some field)
                    with
                    | Some d ->
            ...
            test "a non-positive command timeout is rejected as located MalformedValue (CORE-2)" {
                ...
                match
                    diags
                    |> List.tryFind (fun d -> d.Id = MalformedValue && d.Locator.Field = Some "timeout")
                with
                | Some d ->
                    Expect.stringContains d.Message "positive" "the diagnostic explains the timeout must be positive"
                    Expect.isTrue (d.Locator.Field.IsSome || d.Locator.Line.IsSome) "diagnostic is located (field/line)"
