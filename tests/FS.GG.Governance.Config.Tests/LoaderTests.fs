...
let private copyFixtureToTemp (name: string) : string =
    let parent =
        Path.Combine(Path.GetTempPath(), "fsgg-" + Guid.NewGuid().ToString("N"))
    ...
    testList
        ...
        [
            ...
            test "present-but-invalid optional file → Invalid (FR-015)" {
                ...
                | Invalid d ->
                    Expect.isTrue (d |> List.exists (fun x -> x.File = Policy)) "the invalid optional file fails"
            ...
            test
                "no fallback: project.yml present + governance.yml absent → Invalid, project.yml never consumed (C2, SC-004)" {
                ...
                | Invalid _ -> ()
                | Valid _ ->
                    failtest "an SDD project.yml must NOT satisfy the absent governance.yml primary slot (no fallback)"
            }
            ...
            test "host-path independence: same content under two absolute parents → identical Validation (I3)" {
                ...
                try
                    ...
                    Expect.equal
                        (Loader.loadAndValidate p1)
                        (Loader.loadAndValidate p2)
                        "no absolute host path leaks into the facts"
                finally
                    for p in [ p1; p2 ] do
                        try
