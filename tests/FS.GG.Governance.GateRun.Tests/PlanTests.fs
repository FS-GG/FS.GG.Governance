    ...
    testList
        ...
        [
            ...
            test "a bare backslash escapes the next character" {
                Expect.equal (Plan.lexCommandLine "a\\ b") (Some(Executable "a b", [])) "escaped space joins the token"
            }
            ...
            test "argument order is preserved (identity-significant)" {
                ...
                Expect.notEqual (Plan.lexCommandLine "tool -a -b") (Plan.lexCommandLine "tool -b -a") "order matters"
            }
            ...
            test "a gate with a resolvable RequiresCommand ⇒ Ok with declared inputs verbatim" {
                ...
                | Ok cmd ->
                    ...
                    Expect.equal
                        ...
                        "EMPTY env delta (no ambient leak)"
