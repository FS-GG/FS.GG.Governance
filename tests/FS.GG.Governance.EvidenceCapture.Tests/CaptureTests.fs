    ...
    testList
        ...
        [
            ...
            test "capturing into the empty store yields a one-entry store" {
                ...
                | [ e ] ->
                    ...
                    Expect.equal
                        ...
                        "the single entry carries the derived reference"
            ...
            testPropertyWithConfig
                fscheckConfig
                "capture is deterministic over an arbitrary prior store"
                (fun (world: FreshnessInputs) (r: CommandRecord) (store: ReuseStore) ->
            ...
            test "capturing a brand-new world preserves every prior entry byte-for-byte (newest-first)" {
                ...
                | newest :: rest ->
                    ...
                    Expect.equal
                        newest.Evidence
                        (EvidenceCapture.referenceOf baseRecord)
                        "newest entry is the derived reference"

                    Expect.equal
                        rest
                        (EvidenceReuse.entries prior)
                        "every prior entry preserved byte-for-byte, in order"
            ...
            testPropertyWithConfig
                fscheckConfig
                "recompute-safe (a): classification + Reuse reference preserved for every non-captured candidate"
                (fun (world: FreshnessInputs) (r: CommandRecord) (store: ReuseStore) (candidate: FreshnessInputs) ->
            ...
            testPropertyWithConfig
                fscheckConfig
                "recompute-safe (b): full decision preserved for a candidate not sharing the captured world's gate"
                (fun (world: FreshnessInputs) (r: CommandRecord) (store: ReuseStore) (candidate: FreshnessInputs) ->
                    let grown = EvidenceCapture.capture world r store
