    ...
    testList
        ...
        [
            ...
            testPropertyWithConfig
                fscheckConfig
                "duration-invariance over arbitrary records × two durations"
                (fun (r: CommandRecord) (d1: int64) (d2: int64) ->
            ...
            test "all reproducible-fact perturbations are pairwise-distinct references (injective)" {
                let refs =
                    baseRecord :: (reproducibleVariants |> List.map snd) |> List.map refString

                Expect.equal
                    (List.length (List.distinct refs))
                    (List.length refs)
                    "every distinct reproducible record must map to a distinct reference"
            }
            ...
            test "the three captured-output outcomes yield pairwise-distinct references (F032 FR-011)" {
                ...
                Expect.equal
                    (List.length (List.distinct refs))
                    3
                    "NoCapturedOutput, CapturedAt \"\", CapturedAt \"x\" must be three distinct references"
            }
            ...
            testPropertyWithConfig
                fscheckConfig
                "determinism: same record ⇒ byte-identical reference, no clock/GUID/locale leakage"
                (fun (r: CommandRecord) -> EvidenceCapture.referenceOf r = EvidenceCapture.referenceOf r)
        ]
