    ...
    testList
        ...
        [
            test "T033/T036: an unreadable pack output ⇒ InputUnavailable (exit 3), no writes, no hollow attestation" {
                let unreadable =
                    PackedNoArtifact(SurfaceId "pkg", ArtifactUnreadable "io error", synthaticPackRun 0)
                ...
                Expect.equal
                    ...
                    "unreadable pack output ⇒ exit 3 (input-unavailable, not tool-defect/blocked)"
            ...
            test "T035: no packable projects ⇒ vacuously satisfied + reported, no fabricated pack" {
                ...
                Expect.isTrue
                    (m.Attestation
                     |> Option.map (fun a -> List.isEmpty a.Subjects)
