...
let private realPackTests =
    [
        test "bumped: every project packs at a bumped version ⇒ Success, one Pack run each, both artifacts written" {
            ...
            let specs =
                [ buildable "fsggalpha" "1.3.0" "1.2.0"; buildable "fsggbeta" "2.1.0" "2.0.0" ]
        ...
        test
            "failed-pack (Synthetic broken project) ⇒ Blocked naming the project, the failed run recorded with its sentinel" {
        ...
        test
            "unbumped/downgraded: a project packs at <= its baseline ⇒ Blocked naming the project and the offending version" {
            // SC-001 row 3, FR-001, AS-3.
            let specs =
                [
                    buildable "fsggbumped" "1.3.0" "1.2.0"
                    buildable "fsggstale" "2.0.0" "2.0.0"
                ] // packed == baseline ⇒ Unbumped

            withRealPackRepo specs (fun repo ->
                let m, _ = runRelease repo
                Expect.equal m.Exit Loop.Blocked "an unbumped project blocks the release"

                let stale = verdicts m |> List.tryFind (fun v -> v.Surface = SurfaceId "fsggstale")
        ...
        test "downgraded: a project packs below its baseline ⇒ Blocked, reason names project and versions" {
            ...
            let specs = [ buildable "fsggdown" "1.1.0" "1.2.0" ] // packed < baseline ⇒ Downgraded
        ...
        test
            "zero-exit-no-artifact (Synthetic IsPackable=false) ⇒ Blocked 'no artifact emitted', recorded, distinct from a failed pack" {
        ...
        test
            "determinism: re-running the bumped case over unchanged inputs ⇒ byte-identical release.json + attestation.json" {
            ...
            let specs =
                [ buildable "fsggdet1" "1.3.0" "1.2.0"; buildable "fsggdet2" "2.1.0" "2.0.0" ]
