...
let private driveJoin decl sensed packs format =
    ...
    let m2, _ =
        Loop.update (Loop.ProvenanceSensed(Revision "", EnvironmentClass.Local, BuilderIdentity "fsgg")) m1
    ...
    testList
        ...
        [
            ...
            test "DeclarationLoaded(Ok) emits SenseRelease + PackProjects (declared pack commands)" {
                ...
                let expectedPacks =
                    declWithPackables.PackableProjects
                    |> List.map (fun p -> p.Surface, p.PackCommand)
            ...
            test "all packed+bumped ⇒ unblocked; both artifacts written; release.json is v2" {
                let m4, eff4 =
                    driveJoin declWithPackables sensedMet [ synthaticPacked "pkg" "1.3.0" ] Loop.Json

                Expect.equal m4.Exit Loop.Success "bumped ⇒ clean basis"
                Expect.isEmpty (blockerKinds m4) "no blockers"

                match eff4 with
                | [ Loop.WriteArtifact(Loop.ReleaseArtifact, rp, rc)
                    Loop.WriteArtifact(Loop.AttestationArtifact, ap, _) ] ->
            ...
            test "a failed pack blocks: the three pack families go Unmet ⇒ blockers (FR-001)" {
                let m4, _ =
                    driveJoin declWithPackables sensedMet [ synthaticPackFailed "pkg" 7 ] Loop.Text
            ...
            test "a pack at an unbumped version blocks naming VersionBump (FR-002)" {
                let m4, _ =
                    driveJoin declWithPackables sensedMet [ synthaticPacked "pkg" "1.2.0" ] Loop.Text
            ...
            test "a pack at a downgraded version blocks (FR-002)" {
                let m4, _ =
                    driveJoin declWithPackables sensedMet [ synthaticPacked "pkg" "1.1.0" ] Loop.Text
            ...
            test "C2: a zero-exit pack that produced no artifact (PackedNoArtifact) blocks (edge)" {
                let outcome =
                    PackedNoArtifact(SurfaceId "pkg", NoArtifactEmitted, synthaticPackRun 0)
            ...
            test "C3: a first release (no declared baseline) is NOT blocked as a downgrade" {
                ...
                let m4, _ =
                    driveJoin compliantDeclaration sensedMet [ synthaticPacked "pkg" "0.1.0" ] Loop.Text
            ...
            test "no packable projects ⇒ vacuously satisfied (NoPackableProjects), unblocked" {
                ...
                Expect.isTrue
                    (m4.PackEvidence
                     |> Option.map (fun p -> p.NoPackableProjects)
                     |> Option.defaultValue false)
                    "NoPackableProjects set"
            }

            test "C4: a declared matrix is admitted RunNow at the release boundary, never run" {
                let m4, eff4 =
                    driveJoin declWithPackablesAndMatrix sensedMet [ synthaticPacked "pkg" "1.3.0" ] Loop.Text
            ...
            test "an undeclared matrix is NotDeclared (never invented)" {
                let m4, _ =
                    driveJoin declWithPackables sensedMet [ synthaticPacked "pkg" "1.3.0" ] Loop.Text
