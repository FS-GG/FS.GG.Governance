...
let private withRealFixture action =
    let root =
        Path.Combine(Path.GetTempPath(), "fsgg-evidence-boundary-" + Guid.NewGuid().ToString("N"))
    ...
    try
        ...
        if dotnetExit <> 0 then
            failwithf "dotnet --version failed: %s" dotnetError

        if headExit <> 0 then
            failwithf "git rev-parse HEAD failed: %s" headError
        ...
        let request =
            {
                ...
                Evidence =
                    [
                        real SemanticRegression ObservedOutcome
                        real BoundaryFixture ObservedOutcome
                        real GoldenOrSchema ObservedOutcome
                        real ProductionJourney ObservedOutcome
                    ]
                GeneratedArtifacts = [ artifact ]
                Mitigations =
                    [
                        {
                            Claim = "sound is never requested"
                            ProducerClasses = [ "native"; "fallback" ]
                            ReintroducedByMutation = [ "native"; "fallback" ]
                        }
                    ]
                Render =
                    Some
                        {
                            Fixture = "tests/render-evidence.fsx"
                            Executed = true
                            ByteReproducible = false
                            SemanticReceiptStable = true
                        }
            }
        ...
        [
            test "real process filesystem schema golden and consumer receipts feed evaluate" {
                withRealFixture (fun (_, _, _, _, _, request) ->
                    Expect.isEmpty
                        (evaluate request)
                        "actual command, source/head digest, schema/golden regeneration, and consuming route complete the request")
            }
            test "synthetic records are disclosed support but cannot satisfy real obligations" {
                withRealFixture (fun (_, _, _, _, _, clean) ->
                    let request =
                        { clean with
                            Evidence =
                                ...
                                |> List.map (fun item ->
                                    { item with
                                        Provenance = Synthetic "test boundary"
                                    })
                        }
                    ...
                    Expect.contains
                        ...
                        "every required kind needs real evidence")
            }
                ...
                withRealFixture (fun (_, _, _, _, _, clean) ->
                    for observation in [ DispatchOnly; Degraded ] do
                        let request =
                            { clean with
                                Evidence =
                                    clean.Evidence |> List.map (fun item -> { item with Observation = observation })
                            }
            ...
            test "all degraded optional integrations are accepted as explicit outcomes" {
                withRealFixture (fun (_, _, _, _, real, clean) ->
                    ...
                    let evidence =
                        ...
                        |> List.map (fun (kind, subject) ->
                            { real kind Degraded with
                                Subject = subject
                            })

                    let request =
                        { clean with
                            RequiresObservedOutcome = false
                            OptionalIntegrations = subjects
                            Evidence = evidence
                        }

                    Expect.isEmpty
                        (evaluate request)
                        "explicit degraded outcomes satisfy their named optional integrations")
            }
            test "all dispatch only records cannot masquerade as optional degraded outcomes" {
                withRealFixture (fun (_, _, _, _, real, clean) ->
                    ...
                    Expect.equal
                        ...
                        (subjects |> Set.ofList)
                        "every named optional integration remains visible"

                    Expect.all
                        findings
                        (fun finding -> finding.Code = "evidence.optional-outcome-missing")
                        "dispatch is not an observed optional outcome")
            }
            test "explicit malformed nonzero result is a valid safe-failure receipt" {
                withRealFixture (fun (_, sourcePath, _, _, _, clean) ->
                    let exitCode, _, _ =
                        runCommand "dotnet" [ "--definitely-malformed-evidence-boundary-option" ]

                    let request =
                        { clean with
                            Evidence =
                                {
                                    Subject = "malformed boundary input"
                                    Kind = BoundaryFixture
                                    Provenance = Real
                                    Command = "dotnet --definitely-malformed-evidence-boundary-option"
                                    ExitCode = exitCode
                                    SourceDigest = digest sourcePath
                                    Fresh = true
                                    Observation = MalformedInput
                                }
                                :: clean.Evidence
                        }
                ...
                withRealFixture (fun (_, _, _, _, real, clean) ->
                    let controls =
                        [
                            UnknownInput, 0, true
                            PartialWrite, 0, true
                            ObservedOutcome, 0, false
                            ObservedOutcome, 1, true
                        ]
                    ...
                    for observation, exitCode, fresh in controls do
                        let request =
                            { clean with
                                Evidence =
                                    { real BoundaryFixture observation with
                                        ExitCode = exitCode
                                        Fresh = fresh
                                    }
                                    :: clean.Evidence
            ...
            test "actual generated source regeneration schema golden and consumer mutations red" {
                withRealFixture (fun (root, sourcePath, generatedPath, goldenPath, _, clean) ->
                    let nondeterministic =
                        regenerate sourcePath (Path.Combine(root, "contracts", "generated-broken.json")) "-broken"
                    ...
                    let mutations =
                        [
                            { clean.GeneratedArtifacts.Head with
                                Source = None
                            },
                            "evidence.generated-source-missing"
                            { clean.GeneratedArtifacts.Head with
                                RegenerationDeterministic = not nondeterministic
                            },
            ...
            test "each producer inventory mutation has teeth" {
                withRealFixture (fun (_, _, _, _, _, clean) ->
                    let empty =
                        { clean.Mitigations.Head with
                            ProducerClasses = []
                        }

                    let incomplete =
                        { clean.Mitigations.Head with
                            ReintroducedByMutation = [ "native" ]
                        }

                    Expect.contains
                        (codes { clean with Mitigations = [ empty ] })
                        "evidence.producer-inventory-empty"
                        "empty inventory is not a mitigation"
                    ...
                    Expect.contains
                        ...
                        "each producer mutation has teeth")
            }
            test "findings identify the fact and one bounded correction" {
                withRealFixture (fun (_, _, _, _, _, clean) ->
                    ...
                    Expect.equal
                        (finding.Subject, finding.Correction)
                        ("contracts/generated.json", "name and execute a consuming route")
                        "consumer diagnostic is actionable")
            }
            test "render execution and semantic receipt mutations red" {
                withRealFixture (fun (_, _, _, _, _, clean) ->
                    let notExecuted =
                        { clean.Render.Value with
                            Executed = false
                        }
                    ...
                    let unstable =
                        { clean.Render.Value with
                            ByteReproducible = false
                            SemanticReceiptStable = false
                        }

                    Expect.contains
                        (codes { clean with Render = Some notExecuted })
                        "evidence.render-not-executed"
                        "fixture must run"

                    Expect.contains
                        (codes { clean with Render = Some unstable })
                        "evidence.render-receipt-unstable"
                        "non-byte-stable fixture needs stable receipt")
            }
        ]
