...
let private effect category symbol line column =
    {
        Category = category
        Symbol = symbol
        Line = line
...
let tests =
    testList
        "FSharpEffectBoundary"
        [
            test "pure reducer is clean without name-shaped requirements" {
                Expect.isEmpty
                    (evaluate
                        request
                        [
                            { baseFact with
                                Symbol = "reduceEverything"
                            }
                        ])
                    "pure declared transition passes"
            }
                    ...
                    evaluate
                        ...
                        [
                            { baseFact with
                                DirectEffects =
                                    [
                                        effect Filesystem "File.WriteAllText" 8 12
                                        effect Process "Process.Start" 9 12
                                    ]
                            }
                        ]
                    |> List.filter (fun f -> f.Code = "fsharp.effect-in-transition")
                    |> List.map _.Location.Detail

                Expect.equal
                    codes
                    [ "filesystem File.WriteAllText@8:12"; "process Process.Start@9:12" ]
                    "diagnostics identify the calls and their locations"
            }
            test "effect edge requires explicit result retry and idempotency semantics" {
                let codes =
                    evaluate
                        ...
                        [
                            { baseFact with
                                DirectEffects = [ effect Network "HttpClient" 3 9 ]
                                EdgeInterpreter = Some "interpret"
                                Delivery =
                                    Some
                                        {
                                            SuccessMessage = None
            ...
            test "callback hidden state and exception continuations fail" {
                ...
                Expect.equal
                    codes
                    [ "fsharp.callback-hidden-state"; "fsharp.exception-continuation" ]
                    "both hidden continuations are named"
            }
            test "pure parsers and dated exemptions are narrow non-applicable cases" {
                let parser =
                    { baseFact with
                        IsPureParserOrValidator = true
                        DirectEffects = [ effect Filesystem "File.ReadAllText" 2 4 ]
                    }

                let exempt =
                    { baseFact with
                        DirectEffects = [ effect Filesystem "File.ReadAllText" 2 4 ]
                        Exemption = ActiveExemption("team", "one-shot host bridge", DateOnly(2099, 1, 1))
                    }

                Expect.isEmpty
                    (evaluate request [ parser; exempt ])
                    "legitimate exemptions do not manufacture MVU ceremony"
            }
            test "expired exemption fails closed" {
                let findings =
                    evaluate
                        ...
                        [
                            { baseFact with
                                Exemption = InvalidExemption "review date expired"
                            }
                        ]

                Expect.equal
                    (findings |> List.exactlyOne).Code
                    "fsharp.effect-exemption-invalid"
                    "invalid policy is input state"
            }
            test "compiled declared boundary senses only its own transition body" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-effect-boundary-" + Guid.NewGuid().ToString("N"))
                ...
                try
                    File.WriteAllText(
                        ...
                        "<Project Sdk=\"Microsoft.NET.Sdk\"><PropertyGroup><TargetFramework>net10.0</TargetFramework></PropertyGroup><ItemGroup><Compile Include=\"Domain.fs\" /></ItemGroup></Project>"
                    )

                    File.WriteAllText(
                        Path.Combine(root, "Domain.fs"),
                    ...
                    let finding =
                        ...
                        |> List.find (fun f -> f.Code = "fsharp.effect-in-transition")

                    Expect.equal
                        finding.Location.Detail
                        "filesystem File.WriteAllText@2:24"
                        "effect identity and source location are bound to the transition"
                finally
                    Directory.Delete(root, true)
            }
            test "comments literals and identifier-shaped text are not executable effects" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-effect-lexical-" + Guid.NewGuid().ToString("N"))
...
let advance model =
    ...
    model"""
                    ...
                    let fact =
                        senseProject root "App.fsproj" |> Result.defaultWith failtest |> List.exactlyOne
            ...
            test "executable calls inside interpolation holes retain call identity" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-effect-interpolation-" + Guid.NewGuid().ToString("N"))
                ...
                try
                    ...
                    let calls =
                        (senseProject root "App.fsproj" |> Result.defaultWith failtest |> List.exactlyOne).DirectEffects
                    ...
                    Expect.equal
                        ...
                        "the executable interpolation expression remains visible to call sensing"
            ...
            test "an executable call retains exact identity and one-based location" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-effect-call-" + Guid.NewGuid().ToString("N"))
...
let advance model =
    File.WriteAllText("out.txt", model)"""
                    ...
                    Expect.equal
                        ...
                        "the actual call is diagnostic evidence"
                finally
                    Directory.Delete(root, true)
            }
            test "symbol-local sensing excludes later edge bodies and preserves visible delivery values" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-effect-symbol-" + Guid.NewGuid().ToString("N"))
                ...
                try
                    ...
                    File.WriteAllText(
                        Path.Combine(root, "Domain.fs"),
                        "// fsgg:effect-boundary advance edge=interpret success=Saved failure=Failed retry=never idempotency=document-id\nlet advance model = model\nlet interpret text = File.WriteAllText(\"x\", text)"
                    )
                    ...
                    Expect.equal
                        ...
                        (Some
                            {
                                SuccessMessage = Some "Saved"
                                FailureMessage = Some "Failed"
                                RetryPolicy = Some "never"
                                Idempotency = Some "document-id"
                            })
                        "delivery facts come from exact visible values"
                finally
                    Directory.Delete(root, true)
            }
            test "missing symbols and substring-shaped options fail closed" {
                let sense (source: string) =
                    let root =
                        Path.Combine(Path.GetTempPath(), "fsgg-effect-malformed-" + Guid.NewGuid().ToString("N"))
                ...
                Expect.isError
                    (sense "// fsgg:effect-boundary missing\nlet actual model = model")
                    "a nonexistent declaration symbol is malformed"

                Expect.isError
                    (sense "// fsgg:effect-boundary transition not-edge\nlet transition model = model")
                    "not-edge is not accepted by substring semantics"

                Expect.isError
                    (sense "// fsgg:effect-boundary transition edge=missing\nlet transition model = model")
                    "an unresolved edge symbol is malformed"
            }
        ]
