...
let private withTempRepo (body: string -> 'a) : 'a =
    let dir =
        Path.Combine(Path.GetTempPath(), "fsgg-design-" + Guid.NewGuid().ToString("N"))
...
let private writeCatalogs (repo: string) =
    ...
    File.WriteAllText(
        Path.Combine(d, "contrast.json"),
        """{"fg-on-bg":{"ratio":4.6,"threshold":4.5},"low":{"ratio":2.0,"threshold":4.5}}"""
    )
        ...
        [
            test "senseDesign resolves referenced entries + every contrast pair against the real catalogs" {
                withTempRepo (fun repo ->
                    ...
                    Expect.contains
                        (facts.Tokens |> List.map (fun t -> t.Token, t.Outcome))
                        ("color.primary", Resolves)
                        "primary resolves"

                    Expect.contains
                        (facts.Tokens |> List.map (fun t -> t.Token, t.Outcome))
                        ("color.missing", Absent "color.missing")
                        "missing absent"
