...
let tests =
    testList
        ...
        [
            test
                "the empty registry projects to a valid document with an empty gates array, never throwing (AS1, SC-006, FR-009)" {
            ...
            test
                "a registry mixing present/absent prerequisites and Some/None freshness commands renders each gate's own shape (AS2)" {
                ...
                Expect.equal
                    (tests.GetProperty("freshnessKey").GetProperty "command").ValueKind
                    JsonValueKind.String
                    "build:tests command present"

                Expect.equal
                    (format.GetProperty("freshnessKey").GetProperty "command").ValueKind
                    JsonValueKind.Null
                    "build:format command null"
            }

            testPropertyWithConfig
                config
                "total: ofGateRegistry always returns a parseable document and never throws (AS3, SC-006)"
