    ...
    testList
        ...
        [
            test "every declared gate is present exactly once by declared id with carried metadata verbatim (SC-001)" {
                ...
                Expect.equal
                    (List.length emittedIds)
                    (List.length (List.distinct emittedIds))
            ...
            test
                "build:tests carries its declared cost/maturity tokens and its command prerequisite; a command-less gate is present-and-empty (FR-004, FR-005)" {
                use doc = parse (GatesJson.ofGateRegistry fixtureRegistry)
                ...
                Expect.equal
                    (strField g "maturity")
                    "blockOnShip"
                    "declared maturity carried verbatim (not enforcement)"

                Expect.equal
                    (g.GetProperty("timeout").GetInt32())
                    600
                    "timeout resolved from the referenced command, verbatim"

                Expect.equal
                    (prerequisites g)
                    [ "dotnet-test" ]
                    "the command prerequisite is rendered, in carried order"
            ...
            test "no gate the registry did not contain appears; nothing is invented (FR-003)" {
                ...
                let expected =
                    fixtureRegistry.Gates |> List.map (fun g -> gateIdValue g.Id) |> Set.ofList
            ...
            test
                "a default-timeout gate and a command-derived-timeout gate both render their TimeoutLimit verbatim (FR-006)" {
                // build:tests references dotnet-test (600s); build:format references no command → defaultTimeout.
                use doc = parse (GatesJson.ofGateRegistry fixtureRegistry)

                let derived =
                    fixtureRegistry.Gates |> List.find (fun g -> gateIdValue g.Id = "build:tests")

                let defaulted =
                    fixtureRegistry.Gates |> List.find (fun g -> gateIdValue g.Id = "build:format")

                let (TimeoutLimit dSecs) = derived.Timeout
                let (TimeoutLimit fSecs) = defaulted.Timeout

                Expect.equal
                    ((gateById doc "build:tests").GetProperty("timeout").GetInt32())
                    dSecs
                    "command-derived timeout verbatim"

                Expect.equal
                    ((gateById doc "build:format").GetProperty("timeout").GetInt32())
                    fSecs
                    "default timeout carried verbatim"
            }
            ...
            test "JSON-special characters in free text round-trip via the writer, never manual escaping (FR-002/FR-012)" {
                ...
                Expect.equal
                    (strField g "description")
                    gate.Description
                    "description with special chars round-trips exactly"
            }
        ]
