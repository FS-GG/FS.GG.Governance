    ...
    testList
        ...
        [
            testPropertyWithConfig
                config
                "twice-identical: ofGateRegistry is byte-identical for identical input (AS1, SC-002)"
            <| Prop.forAll (Arb.fromGen genRegistry) (fun r -> GatesJson.ofGateRegistry r = GatesJson.ofGateRegistry r)

            test "fixed-fixture twice-identical equality (SC-002)" {
                Expect.equal
                    (GatesJson.ofGateRegistry populated)
                    (GatesJson.ofGateRegistry populated)
                    "same registry → same bytes"
            }

            test
                "permutation-invariant: registries from differently-ordered declared checks project identically (AS2, SC-003)" {
                ...
                Expect.equal
                    (GatesJson.ofGateRegistry a)
                    (GatesJson.ofGateRegistry b)
                    "value-equal registries project to identical bytes"
            }
            ...
            test "the document carries the declared schemaVersion and the fixed field order (AS3, FR-013)" {
                ...
                Expect.equal
                    ...
                    GatesJson.schemaVersion
                    "schemaVersion field equals the constant"
                ...
                for g in gates doc do
                    ...
                    Expect.equal
                        ...
                        "fixed freshnessKey field order"
            }

            test
                "exclusion sweep: the emitted text contains no enforcement/verdict/selection/raw-YAML/clock token (AS4, SC-007, FR-011/FR-012)" {
            ...
            test
                "positive allowlist: every emitted string value is a declared id / vocabulary / carried metadatum (FR-012)" {
                use doc = parse (GatesJson.ofGateRegistry populated)
                ...
                let maturityTokens =
                    Set.ofList [ "observe"; "warn"; "blockOnPr"; "blockOnShip"; "blockOnRelease" ]
                    ...
                    | System.Text.Json.JsonValueKind.Object ->
                        [
                            for p in el.EnumerateObject() do
                                yield! strings p.Value
                        ]
                    | System.Text.Json.JsonValueKind.Array ->
                        [
                            for e in el.EnumerateArray() do
                                yield! strings e
                        ]
                    | _ -> []
                ...
                for s in strings doc.RootElement do
                    ...
                    | v ->
                        Expect.isTrue
                            (allowed v)
                            (sprintf "emitted string %A is a declared id / vocabulary / carried metadatum" v)
            }
        ]
