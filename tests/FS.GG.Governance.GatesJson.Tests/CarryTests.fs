...
let tests =
    testList
        ...
        [
            ...
            test
                "a Some freshness command renders the command string; a None command renders explicit JSON null (AS2, SC-004, FR-014)" {
                use doc = parse (GatesJson.ofGateRegistry carryRegistry)
                ...
                for gate in carryRegistry.Gates do
                    ...
                    | None ->
                        Expect.equal
                            ...
                            JsonValueKind.Null
                            "command input is explicit JSON null"
            }
            ...
            test "productCheck is carried verbatim from the registry, not re-derived (AS3, FR-002)" {
                ...
                Expect.isTrue
                    ((gateById doc "release:publish").GetProperty("productCheck").GetBoolean())
                    "release gate is a product check"
            }
            ...
            test
                "cost/maturity render verbatim with no enforcement translation, no weighted cost scalar (SC-005, FR-005)" {
                use doc = parse (GatesJson.ofGateRegistry carryRegistry)
                ...
                Expect.equal
                    (g.GetProperty "cost").ValueKind
                    JsonValueKind.String
                    "cost is a token string, not a scalar"
                ...
                Expect.equal
                    (strField (gateById doc "release:publish") "maturity")
                    "blockOnRelease"
                    "release maturity verbatim"
            }
        ]
