    ...
    testList
        ...
        [
            ...
            test "the public surface is exactly the two modules (Model + SensedMetadata), nothing else" {
                let typeNames =
                    sensedAsm.GetExportedTypes() |> Array.choose (fun t -> Option.ofObj t.FullName)

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.SensedMetadata.ModelModule"))
                    "Model module is public"

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.SensedMetadata.SensedMetadataModule"))
                ...
                Expect.isFalse
                    (typeNames
                     |> Array.exists (fun n ->
                         ...
                         || n.ToLowerInvariant().Contains "prefix"
            ...
            test "SensedMetadata references only CommandRecord + Config + BCL + FSharp.Core (plan D1 scope guard)" {
                ...
                Expect.isEmpty
                    offending
                    (sprintf
                        "SensedMetadata must depend on CommandRecord/Config/BCL/FSharp.Core only; found: %A"
                        offending)
                ...
                Expect.isEmpty
                    forbidden
                    (sprintf
                        "SensedMetadata must not reference FreshnessKey/Provenance/Gates/Snapshot/Route/Routing/Findings/EvidenceReuse/RouteExplain/RouteJson/GatesJson/AuditJson/Enforcement/Ship/host/adapters/CLI; found: %A"
                        forbidden)
            }
        ]
