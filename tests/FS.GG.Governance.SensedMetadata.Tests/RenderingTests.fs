...
let private rv (m: SensedMetadatum) =
    SensedMetadata.renderingValue (SensedMetadata.render m)
        ...
        [ // (a) marker present & distinguishable (L-R1).
            test "render starts with !sensed!= — a form no reproducible field tag produces" {
                ...
                for tag in reproducibleTags do
                    Expect.isFalse
                        ((rv workedTimestamp).StartsWith(tag + "="))
                        (sprintf "sensed rendering must not begin with the reproducible tag %s=" tag)
            }
            ...
            test "byte-exact single-metadatum worked examples" {
                ...
                Expect.equal
                    ...
                    "empty-label zero-duration: distinct 0: form"

                Expect.equal
                    (rv (markTs "!sensed!" "2026-06-21T12:00:00Z"))
                    "!sensed!=timestamp;8:!sensed!;20:2026-06-21T12:00:00Z"
                    "label text !sensed! neutralized by the length prefix"
            }
            ...
            test "no marker-containing label/value can make two different metadata render equal or masquerade" {
                ...
                Expect.notEqual
                    (rv spoofLabel)
                    (rv realMarkerShape)
                    "a marker-shaped label cannot become another metadatum"
                ...
                Expect.equal
                    (rv trickyValue)
                    "!sensed!=timestamp;1:k;7:a;b:c=d"
                    "tricky value read by length, not by separators"
            ...
            testPropertyWithConfig fscheckConfig "render is injective: equal rendering iff equal kind+label+value"
            <| fun (m: SensedMetadatum) (m': SensedMetadatum) ->
            ...
            test "renderSection is one order-preserving section; empty list ⇒ !sensed-section!=0;" {
                Expect.equal
                    (SensedMetadata.renderingValue (SensedMetadata.renderSection []))
                    "!sensed-section!=0;"
                    "empty list is an ordinary value"

                // The two-element worked-example block, byte-for-byte (timestamp then duration).
                let block =
                    SensedMetadata.renderingValue (SensedMetadata.renderSection [ workedTimestamp; workedDuration ])
                ...
                let reversed =
                    SensedMetadata.renderingValue (SensedMetadata.renderSection [ workedDuration; workedTimestamp ])
                ...
                let dup =
                    SensedMetadata.renderingValue (SensedMetadata.renderSection [ workedTimestamp; workedTimestamp ])
            ...
            test "renderingValue unwraps a SensedRendering" {
                Expect.equal
                    (SensedMetadata.renderingValue (SensedRendering "abc"))
                    "abc"
