    ...
    testList
        ...
        [ // (a) carriage (L-M1) — label and value read back verbatim.
            test "markDuration carries label + DurationValue verbatim" {
                ...
                Expect.equal
                    m
                    { Label = l; Value = DurationValue d }
                    "markDuration L d = { Label = L; Value = DurationValue d }"
            ...
            test "markTimestamp carries label + TimestampValue verbatim" {
                ...
                Expect.equal
                    m
                    { Label = l; Value = TimestampValue t }
                    "markTimestamp L t = { Label = L; Value = TimestampValue t }"
            ...
            test "kindOf agrees with the constructor used; kindToken is the injective two-case map" {
                ...
                Expect.notEqual
                    (SensedMetadata.kindToken DurationKind)
                    (SensedMetadata.kindToken TimestampKind)
            ...
            test
                "zero-length duration, empty label, same-label/different-kind, and large/negative magnitudes are ordinary values" {
                ...
                Expect.notEqual
                    (SensedMetadata.kindOf sameLabelTs)
                    (SensedMetadata.kindOf sameLabelDur)
                    "…while differing in kind"
                ...
                Expect.equal
                    big.Value
                    (DurationValue(duration System.Int64.MaxValue))
                    "large magnitude is an ordinary value"
            ...
            testPropertyWithConfig fscheckConfig "markDuration round-trips label + duration; kindOf = DurationKind"
            <| fun (l: SensedLabel) (d: SensedDuration) ->
                let m = SensedMetadata.markDuration l d

                m.Label = l
                && m.Value = DurationValue d
                && SensedMetadata.kindOf m = DurationKind

            testPropertyWithConfig fscheckConfig "markTimestamp round-trips label + timestamp; kindOf = TimestampKind"
            <| fun (l: SensedLabel) (t: SensedTimestamp) ->
                let m = SensedMetadata.markTimestamp l t

                m.Label = l
                && m.Value = TimestampValue t
                && SensedMetadata.kindOf m = TimestampKind
        ]
