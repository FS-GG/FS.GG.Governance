    ...
    testList
        ...
        [ // (a) determinism (L-D1) — twice yields structurally / byte-identical results.
            test "marking and rendering the same value twice is byte-equal" {
                ...
                Expect.equal
                    (SensedMetadata.render workedTimestamp)
                    (SensedMetadata.render workedTimestamp)
            ...
            testPropertyWithConfig fscheckConfig "render / renderSection are deterministic over generated inputs"
            <| fun (m: SensedMetadatum) (ms: SensedMetadatum list) ->
            ...
            testPropertyWithConfig fscheckConfig "reproducible bytes are invariant under any sensed section"
            <| fun (ms: SensedMetadatum list) ->
                let reproducibleBytes = "src=16:c0ffee\nhead=15:head2"
