...
let tests =
    testList
        ...
        [
            test
                "same-check entry differing in exactly one non-check input ⇒ InputsChanged [thatInput], no equal input named" {
            ...
            test
                "multi-input change ⇒ InputsChanged carries exactly the changed set in F035's fixed diff order, never CheckHashInput" {
                let store = handStore [ baseInputs, refV1 ]
            ...
            test "judge-only change ⇒ every changed input attributes to JudgeIdentity" {
                ...
                let groups =
                    causeInputs (VerdictReuse.lookup request store) |> List.map Model.inputGroup
            ...
            test "no entry sharing the request's check ⇒ NoCachedVerdict (distinct from InputsChanged)" {
                ...
                Expect.equal
                    ...
                    "different work ⇒ NoCachedVerdict"
            }
            ...
            testPropertyWithConfig
                fscheckConfig
                "every Invalidated carries NoCachedVerdict or a non-empty InputsChanged without CheckHashInput"
