    ...
    testList
        ...
        [
            ...
            test "refresh / de-dup most-recent-wins: record i v2 (record i v1 s) ⇒ Valid v2 and no duplicate for i" {
                let store =
                    VerdictReuse.record baseInputs refV2 (VerdictReuse.record baseInputs refV1 VerdictReuse.empty)
            ...
            test "replay determinism: same start store + same recording sequence ⇒ identical lookup decisions" {
                let seq =
                    [ baseInputs, refV1; variantPromptHash baseInputs, refV2; baseInputs, refV3 ]
                ...
                for r in requests do
                    Expect.equal
                        (VerdictReuse.lookup r s1)
                        (VerdictReuse.lookup r s2)
                        "replayed store decides identically"
            ...
            testPropertyWithConfig
                fscheckConfig
                "record i v2 (record i v1 s) ⇒ lookup i = Valid v2 with exactly one matching entry"
            ...
            testPropertyWithConfig
                fscheckConfig
                "recording preserves every prior Valid reuse for a request that does not match the recorded inputs"
            <| fun (inputs: AgentReviewInputs) (v: VerdictRef) (request: AgentReviewInputs) (store: VerdictStore) ->
                ...
                if AgentReviewKey.matches request inputs then
                    ...
                    match
                        ...
                        VerdictReuse.lookup request (VerdictReuse.record inputs v store)
                    with
                    | Valid a, Valid b -> a = b
