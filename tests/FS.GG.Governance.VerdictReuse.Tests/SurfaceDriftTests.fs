...
let private verdictReuseAsm =
    SurfaceDrift.assemblyNamed "FS.GG.Governance.VerdictReuse"
    ...
    testList
        ...
        [
            ...
            test "the public surface is exactly the two modules (Model + VerdictReuse), nothing else" {
                let typeNames =
                    verdictReuseAsm.GetExportedTypes()
                    |> Array.choose (fun t -> Option.ofObj t.FullName)

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.VerdictReuse.ModelModule"))
                    "Model module is public"

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.VerdictReuse.VerdictReuseModule"))
