    ...
    testList
        ...
        [
            test "lookup against the empty store ⇒ Invalidated NoCachedVerdict" {
                Expect.equal
                    (VerdictReuse.lookup baseInputs VerdictReuse.empty)
                    (Invalidated NoCachedVerdict)
                    "empty ⇒ NoCachedVerdict"
            }
            ...
            test
                "an empty-string VerdictRef recorded then looked up ⇒ Valid carrying it verbatim (never parsed/rejected)" {
            ...
            testPropertyWithConfig
                fscheckConfig
                "no AgentReviewInputs/VerdictRef/VerdictStore value makes lookup or record throw (totality)"
