    ...
    testList
        ...
        [
            test "decide / record are identical across a changed cwd and an unrelated temp file (SC-006)" {
                ...
                let expectedEntries =
                    EvidenceReuse.entries (EvidenceReuse.record candidate E2 store)
                ...
                try
                    ...
                    Expect.equal
                        afterCwdChange
                        expectedDecision
                        "changing cwd / creating a file must not change the decision"

                    Expect.equal
                        afterCwdEntries
                        expectedEntries
                        "changing cwd / creating a file must not change the record result"
