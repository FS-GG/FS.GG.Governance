...
let private evidenceReuseAsm =
    SurfaceDrift.assemblyNamed "FS.GG.Governance.EvidenceReuse"
    ...
    testList
        ...
        [
            ...
            test "the public surface is exactly the two modules (Model + EvidenceReuse), nothing else" {
                let typeNames =
                    evidenceReuseAsm.GetExportedTypes()
                    |> Array.choose (fun t -> Option.ofObj t.FullName)

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.EvidenceReuse.ModelModule"))
                    "Model module is public"

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.EvidenceReuse.EvidenceReuseModule"))
                    "EvidenceReuse operations module is public"

                Expect.isFalse
                    (typeNames
                     |> Array.exists (fun n ->
                         n.ToLowerInvariant().Contains "helper"
                         || n.ToLowerInvariant().Contains "internal"))
