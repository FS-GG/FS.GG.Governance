        ...
        [
            ...
            test "refresh / de-dup most-recent-wins: record i E2 (record i E1 s) ⇒ Reuse E2, no dup (US3 #2, FR-008)" {
                ...
                Expect.equal
                    (EvidenceReuse.entries twice |> List.length)
                    1
                    "exactly one entry for the matching-input class"
            }

            test
                "independence: recording non-matching inputs leaves every entry independently reusable (US3 #3, FR-008)" {
                let candidateB =
                    { baseInputs with
            ...
            test "no mutation: record does not alter the input store value (FR-007)" {
                ...
                Expect.equal
                    (EvidenceReuse.decide baseInputs original)
                    (Reuse E1)
                    "the original still decides to its own ref"
            }

            testPropertyWithConfig
                fscheckConfig
                "replay determinism: same start store + same record sequence ⇒ identical decisions (US3 #4, SC-005)"
