        ...
        [
            test "same-gate single-field change ⇒ InputsChanged [thatCategory] (US2 #1, SC-003)" {
                ...
                for (category, vary) in singleCategoryVariants do
                    ...
                    | other ->
                        failtestf "category %A: expected Recompute (InputsChanged [%A]), got %A" category category other
            }
            ...
            testPropertyWithConfig
                fscheckConfig
                "every Recompute carries NoPriorEvidence or a located non-empty InputsChanged (SC-003, FR-006)"
