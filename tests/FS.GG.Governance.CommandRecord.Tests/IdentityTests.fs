    ...
    testList
        ...
        [ // (a) Sensed split — duration reachable, distinct, and identity-invariant (SC-003/SC-004, US2 #1/#3).
            test "duration is reachable as sensed metadata, distinct from the reproducible facts" {
                ...
                Expect.equal
                    baseRecord.Reproducible.Executable
                    baseExecutable
                    "reproducible facts are reachable separately"
            }
            ...
            test "two records differing only in duration have equal canonicalId" {
                ...
                Expect.equal
                    (CommandRecord.canonicalId r1)
                    (CommandRecord.canonicalId r2)
            ...
            test "changing any one reproducible fact changes canonicalId" {
                ...
                for (label, variant) in allReproducibleVariants do
                    ...
                    Expect.notEqual
                        (CommandRecord.canonicalId perturbed)
                        baseId
            ...
            test "NoCapturedOutput, empty path, and non-empty path yield three distinct identities" {
                let idOf cap =
                    CommandRecord.canonicalId (
                        rebuild
                            { baseRecord.Reproducible with
                                CapturedOutput = cap
                            }
                            baseDuration
                    )
            ...
            test "worked example renders to the contract's exact canonical block" {
                let actual =
                    CommandRecord.identityValue (CommandRecord.canonicalId workedExampleRecord)

                Expect.equal
                    actual
                    workedExampleBlock
                    "worked-example identity equals contracts/command-record-identity-format.md"
            ...
            <| fun (facts: ReproducibleFacts) (d1: SensedDuration) (d2: SensedDuration) ->
