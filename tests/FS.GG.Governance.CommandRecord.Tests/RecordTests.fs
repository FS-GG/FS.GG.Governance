        ...
        [ // (a) Verbatim carriage of all ten facts (SC-001, US1 #1).
            ...
            test "build preserves argument order exactly" {
                ...
                Expect.equal
                    ...
                    [ Argument "z"; Argument "a"; Argument "m" ]
                    "order kept verbatim, not sorted"
            }
            ...
            test "env delta is a three-class partition; a changed var appears once in Changed only" {
                ...
                Expect.equal
                    r.Reproducible.Environment.Added
                    [
                        {
                            Name = EnvVarName "NEW"
                            Value = EnvVarValue "v"
                    ...
                    "Added carries exactly the added var"

                Expect.equal
                    r.Reproducible.Environment.Changed
                    [
                        {
                            Name = EnvVarName "PATH"
                            Old = EnvVarValue "/a"
                    ...
                    "Changed carries the changed var once, with Old+New"

                Expect.equal
                    r.Reproducible.Environment.Removed
                    [
                        {
                            Name = EnvVarName "OLD"
                            Old = EnvVarValue "g"
                    ...
                    "Removed carries exactly the removed var"
                ...
                Expect.isFalse
                    (r.Reproducible.Environment.Added |> List.exists (fun a -> a.Name = changedName))
                    "changed var not in Added"

                Expect.isFalse
                    (r.Reproducible.Environment.Removed
                     |> List.exists (fun a -> a.Name = changedName))
                    "changed var not in Removed"
            }
            ...
            testPropertyWithConfig fscheckConfig "build is total and round-trips every fact"
            <| fun (facts: ReproducibleFacts) (duration: SensedDuration) ->
