        ...
        [ // (a) build then canonicalId twice ⇒ structurally / byte equal.
            test "build and canonicalId are deterministic for a representative record" {
                ...
                Expect.equal
                    ...
                    "identical facts ⇒ byte-identical identity"
            }

            testPropertyWithConfig fscheckConfig "build + canonicalId are deterministic over generated facts"
            ...
            test "reordering and duplicating env-delta entries leaves canonicalId unchanged" {
                ...
                let permuted =
                    { facts with
                        Environment = permuteAndDuplicateEnv env
                    }
            ...
            testPropertyWithConfig fscheckConfig "env-delta order/dup permutation is identity-invariant"
            <| fun (facts: ReproducibleFacts) ->
                let permuted =
                    { facts with
                        Environment = permuteAndDuplicateEnv facts.Environment
                    }
            ...
            test "reordering arguments DOES change canonicalId" {
                ...
                let reversed =
                    { baseRecord.Reproducible with
                        Arguments = [ Argument "-b"; Argument "-a" ]
                    }
