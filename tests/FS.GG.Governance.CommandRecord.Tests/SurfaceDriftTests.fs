...
let private commandRecordAsm =
    CommandRecord.identityValue (CommandIdentity "load") |> ignore

    System.AppDomain.CurrentDomain.GetAssemblies()
    |> Array.find (fun a ->
        match Option.ofObj (a.GetName().Name) with
        | Some n -> n = "FS.GG.Governance.CommandRecord"
        | None -> false)

let private baselinePath =
    Path.Combine(repoRoot, "surface", "FS.GG.Governance.CommandRecord.surface.txt")
...
let tests =
    testList
        "SurfaceDrift"
        [ test "CommandRecord public surface equals the committed baseline" {
              let actual = renderSurface commandRecordAsm
          ...
          test "CommandRecord references only Config + BCL + FSharp.Core (plan D1 scope guard)" {
              // One-way dependency: CommandRecord -> Config (for the F014 TimeoutLimit). No git-sensing
              // Snapshot, no Gates/Route record wrapper, no host/adapter/CLI edge, and no new third-party
              // package.
              ...
              let offending =
                  commandRecordAsm.GetReferencedAssemblies()
                  |> Array.choose (fun a -> Option.ofObj a.Name)
                  |> Array.filter (allowed >> not)

              Expect.isEmpty
                  offending
                  (sprintf "CommandRecord must depend on Config/BCL/FSharp.Core only; found: %A" offending)

              // Specifically: NOT Gates/Snapshot/Route/Routing/Findings/FreshnessKey/EvidenceReuse/
              // RouteExplain/host/adapters/CLI or any other edge assembly.
              let forbidden =
                  commandRecordAsm.GetReferencedAssemblies()
                  |> Array.choose (fun a -> Option.ofObj a.Name)
                  |> Array.filter (fun n ->
              ...
              Expect.isEmpty
                  forbidden
                  (sprintf "CommandRecord must not reference Gates/Snapshot/Route/Routing/Findings/FreshnessKey/EvidenceReuse/RouteExplain/host/adapters/CLI; found: %A" forbidden)
          } ]
