...
let private commandRecordAsm =
    SurfaceDrift.assemblyNamed "FS.GG.Governance.CommandRecord"
    ...
    testList
        ...
        [
            ...
            test "the public surface is exactly the two modules (Model + CommandRecord), nothing else" {
                ...
                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.CommandRecord.ModelModule"))
                    "Model module is public"

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.CommandRecord.CommandRecordModule"))
