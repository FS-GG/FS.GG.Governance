...
let private rootFacts surfaces =
...
let tests =
    testList
        ...
        [
            test
                "an UnmatchedInRoot path within a ProtectedSurface escalates and carries the SurfaceId (US3 AS1, SC-003)" {
                ...
                Expect.equal
                    f.Zone
                    (ProtectedBoundaryUnknown(sid "kernel-core"))
                    "zone carries the escalating SurfaceId"
            }

            test
                "an in-root unknown in no declared surface stays the ordinary flavor, distinct from protected (US3 AS2)" {
                let facts = rootFacts [ surface ProtectedSurface "kernel-core" [ "src/Core" ] ]
            ...
            test
                "Protected > Routine: a path in both a Routine and a ProtectedSurface → a single escalated finding (US3 AS, FR-007)" {
                let surfaces =
                    [
            ...
            test
                "multiple matching protected surfaces → ordinal-first SurfaceId, independent of authoring order (FR-009)" {
                let forward =
                    [
                ...
                let fwd =
                    Findings.findUnknownGovernedPaths factsFwd (routeOf factsFwd [ "src/Z/x.fs" ])

                let rev =
                    Findings.findUnknownGovernedPaths factsRev (routeOf factsRev [ "src/Z/x.fs" ])
