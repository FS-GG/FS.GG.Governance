    ...
    testList
        ...
        [
            test "the same unclassified in-root path gives an identical finding from each F016 plane (US5 AS1, SC-007)" {
                ...
                let committed =
                    Findings.findUnknownGovernedPaths facts0 (routeOf facts0 [ "src/New.fs" ])
            ...
            test "a multi-path multi-plane union drops no path and is deterministically ordered (US5 AS3)" {
                ...
                let report =
                    routingsWith
                        [
                            ...
                            routing
                                "src/Kernel/k.fs"
                                (Routed(DomainId "kernel", GovernedPath "src/Kernel/**", OnlyMatch))
                        ]
                ...
                Expect.equal
                    paths
                    [
                        normalizePath "src/Alpha.fs"
                        normalizePath "src/Beta.fs"
