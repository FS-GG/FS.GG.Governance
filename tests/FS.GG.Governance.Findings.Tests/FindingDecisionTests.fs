...
let tests =
    testList
        ...
        [
            ...
            test
                "mixed Routed/UnmatchedInRoot/OutOfScope → a finding only for each non-routine in-root unknown (US1 AS2)" {
                let facts = baseFacts []
            ...
            test "out-of-scope + routine + a third in-root unknown → only the third flags (US2 AS3, SC-002)" {
                ...
                let report = routeOf facts [ "docs/guide.md"; "src/Legacy/Old.fs"; "src/New.fs" ]
