        ...
        [
            ...
            test "an absent catalog/src ⇒ accessors return None (unsensed), never a fabricated empty hash" {
                withBareDir (fun dir ->
                    ...
                    Expect.equal
                        (s.SenseCommandVersion dotnetCmd)
                        None
                        "no catalog ⇒ command version is None (unsensed)"

                    // A missing src/** surface is SENSED-EMPTY (`Some []`), a legitimate resolved value — NOT None.
                    let g = gateWith "build" "tests" (Some dotnetCmd)

                    Expect.equal
                        (s.SenseCoveredArtifacts g)
                        (Some [])
                        "a missing src/** ⇒ Some [] (sensed-empty), not None")
            }

            test
                "senseFreshness assembles a SensedFacts whose present keys match the sensed inputs, base/head passed through" {
                withTempDir (fun t ->
                    let g = gateWith "build" "tests" (Some dotnetCmd)
                    ...
                    | Ok(facts: SensedFacts) ->
                        ...
                        Expect.isTrue
                            (Map.containsKey g.Id facts.CoveredArtifacts)
                            "the gate's covered key is present (sensed)"

                        Expect.isTrue
                            (Map.containsKey dotnetCmd facts.CommandVersions)
                            "the declared command's version key is present (sensed)")
            }
            ...
            test
                "covered-artifact hashes are rename-sensitive: identical content at a different path ⇒ different hash (#56/B11)" {
            ...
            test "an unsensed command version ⇒ the command key is ABSENT (never fabricated)" {
                ...
                | Ok facts ->
                    Expect.isFalse
                        (Map.containsKey dotnetCmd facts.CommandVersions)
                        "an unsensed command version is an ABSENT key, never fabricated"
