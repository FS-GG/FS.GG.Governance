    ...
    testList
        ...
        [
            ...
            test "views are in declared order with the exhaustive status tokens" {
                let statuses =
                    [ for v in views.EnumerateArray() -> v.GetProperty("status").GetString() ]

                Expect.equal
                    statuses
                    [ "regenerated"; "current"; "stale-unresolved"; "not-evaluated" ]
                    "declared order + tokens"
            }

            test "drifted is present for a regenerated view, empty for a current view" {
                let drifted (v: JsonElement) =
                    [ for d in v.GetProperty("drifted").EnumerateArray() -> d.GetString() ]
            ...
            test "reason is present ONLY for stale-unresolved (FR-010/FR-016)" {
                ...
                Expect.stringContains
                    (nonNull (view(2).GetProperty("reason").GetString()))
                    "source not found"
                    "names why"
            }
