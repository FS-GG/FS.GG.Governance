...
// is the `RefreshModel` + `RefreshJson` modules. The committed baseline is produced with BLESS_SURFACE=1.

let private library = typeof<FS.GG.Governance.RefreshJson.RefreshModel.GenerationManifest>.Assembly

let private baselinePath =
    Path.Combine(repoRoot, "surface", "FS.GG.Governance.RefreshJson.surface.txt")
...
let tests =
    testList
        "SurfaceDrift"
        [ test "RefreshJson public surface equals the committed baseline" {
