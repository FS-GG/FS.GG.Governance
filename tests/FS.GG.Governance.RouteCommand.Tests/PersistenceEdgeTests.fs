        ...
        [
            test "SC-001: a seeded non-empty v1 store round-trips losslessly through the real reader" {
                withTempRepo (fun dir ->
                    ...
                    let req =
                        parseOrFail [ "route"; "--repo"; dir; "--since"; "HEAD~1"; "--persist-store" ]
            ...
            test
                "SC-005: an unwritable store target is non-fatal — exit unchanged, route.json intact, no partial file, a note" {
                withTempRepo (fun dir ->
                    ...
                    Expect.equal
                        (File.ReadAllText req.RouteOut)
                        routeBase
                        "route.json is unchanged by the failed store write"
    ...
    testList
        ...
        [
            test "route.json byte-identical to the frozen pre-wiring golden (5a0cb28)" {
                let tmp =
                    Path.Combine(Path.GetTempPath(), "fsgg-golden-route-" + System.Guid.NewGuid().ToString("N"))
