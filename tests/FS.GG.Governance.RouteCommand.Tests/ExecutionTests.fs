    ...
    testList
        ...
        [
            test "US3: a gate that EXITS non-zero is reported in route.json, but the command still EXITS 0 (FR-008)" {
                ...
                let model =
                    Interpreter.run
                        (fakePortsExec validCatalog git fakeSensor absentStoreReader (fakeExecPortExiting 1) cap req)
                        req
                ...
                Expect.stringContains
                    ...
                    "each selected gate's execution outcome is reported"

                Expect.stringContains
                    route
                    "\"passed\":false"
                    "the failing gate's outcome is reported (advisory, not enforced)"
            }
            ...
            test "US3: a reusable gate is NOT spawned a second time; route.json reports it reused (SC-003)" {
                withTempRepo (fun dir ->
                    ...
                    Expect.stringContains
                        (File.ReadAllText req.RouteOut)
                        "\"disposition\":\"reused\""
                        "run 2 reports the gates as reused")
            }
        ]
