    ...
    testList
        ...
        [
            ...
            test "Loaded Valid selects gates and emits SenseFreshness + LoadStore (no write yet) (US1)" {
                ...
                let selectedGates =
                    (Option.get m2.Result).SelectedGates |> List.map (fun sg -> sg.Gate)
            ...
            test "the join fires only once BOTH Sensed and Store are Some, then emits two writes (US1, SC-001)" {
                ...
                let routeDoc =
                    RouteJson.ofRouteResultWithProductSurfaces result (Some expectedReport) outcomes m2.Classifications

                Expect.equal
                    m5.RouteDoc
                    (Some routeDoc)
                    "RouteDoc = ofRouteResultWithProductSurfaces result (Some report) outcomes classifications"
            ...
            test "both Wrote Ok then Emitted reach Done/Success, emitting the summary (US1)" {
                ...
                | [ Loop.EmitSummary(text, _, _) ] ->
                    Expect.equal
                        text
                        (Loop.render m5 req.Format)
                        "second write ack ⇒ EmitSummary with the rendered text"
