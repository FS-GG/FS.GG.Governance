        ...
        [
            test "--paths a b ⇒ ExplicitPaths of the normalized paths (bypasses git)" {
                ...
                | Ok req ->
                    Expect.equal
                        req.Scope
                        (Loop.ExplicitPaths [ normalizePath "a"; normalizePath "b" ])
                        "explicit path scope"
            ...
            test "--paths a --since X ⇒ PathsAndSinceTogether (mutually exclusive)" {
                Expect.equal
                    (parse [ "route"; "--paths"; "a"; "--since"; "X" ])
                    (Error Loop.PathsAndSinceTogether)
                    "exclusive"
            }
            ...
            test "CLI-5: a stray non-`--` positional ⇒ UnexpectedArgument, not UnknownFlag" {
                Expect.equal
                    (parse [ "route"; "junk" ])
                    (Error(Loop.UnexpectedArgument "junk"))
                    "stray positional rejected as UnexpectedArgument"
            }
            ...
            test "--store sets StorePath; omitted ⇒ default <repo>/readiness/evidence-reuse.json" {
                ...
                match parse [ "route"; "--repo"; "/tmp/x" ] with
                | Ok req ->
                    Expect.equal req.StorePath "/tmp/x/readiness/evidence-reuse.json" "default store under --repo"
                | Error e -> failtestf "expected Ok, got Error %A" e
            }
            ...
            test "--store with no value ⇒ a UsageError VALUE, never a throw" {
                Expect.equal
                    (parse [ "route"; "--store" ])
                    (Error(Loop.MissingValue "--store"))
                    "missing --store value is a value"
            }
        ]
    ...
    testList
        ...
        [
            test "--repo followed by a flag ⇒ MissingValue, the flag is not swallowed" {
                Expect.equal
                    (parse [ "route"; "--repo"; "--json" ])
                    (Error(Loop.MissingValue "--repo"))
                    "flag not swallowed as --repo value"
            }
