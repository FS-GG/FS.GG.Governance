...
let tests =
    testList
        ...
        [
            test
                "no-`--json`: the text summary contains the HumanText.ofRouteResult projection of the resolved RouteResult, ANSI-free, with host `wrote` lines (US1)" {
            ...
            test
                "clean / nothing-to-report: a routine-only change still renders the HumanText projection ANSI-free and writes byte-identical JSON (US1 clean state, SC-002)" {
                ...
                let summary =
                    Expect.wantSome (List.tryHead cap.Emits) "a text summary was emitted even when nothing routes"
                ...
                Expect.equal
                    (writtenOf cap Loop.RouteArtifact |> Option.map snd)
                    (Some expectedRoute)
                    "clean route.json byte-identical to RouteJson projection"

                Expect.equal
                    (writtenOf cap Loop.GatesArtifact |> Option.map snd)
                    (Some expectedGates)
                    "clean gates.json byte-identical to GatesJson projection"
            }

            // ── JSON byte-identity goldens (T008 / SC-002) — the wiring touches the human branch only ──

            test
                "JsonGolden: route.json + gates.json are byte-identical to the F020/F021 projections for identical repo state (SC-002)" {
                let git = gitWithChanges [ 'M', "src/Lib/Thing.fs" ]
                ...
                Expect.equal
                    (writtenOf cap Loop.RouteArtifact |> Option.map snd)
                    (Some expectedRoute)
                    "route.json bytes unchanged by the human-branch wiring"

                Expect.equal
                    (writtenOf cap Loop.GatesArtifact |> Option.map snd)
                    (Some expectedGates)
                    "gates.json bytes unchanged by the human-branch wiring"
