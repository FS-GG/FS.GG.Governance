...
let private malformedJson =
    """{ "contractVersion": "2.0.0", "schemaVersion": 1, "evidence": { "nodes": [ this is not json """
...
let private selectedIds (m: Loop.Model) =
        ...
        [ // ── US1 V1 (SC-001): declared surface drives selection with an EMPTY sensed change set ──
            test "V1 — a declared governed path selects its domain gates with no sensed change (SC-001)" {
                let m =
                    runWith
                        (Loop.ExplicitPaths [])
                        [ handoffRead (declaringJson "\"src/Lib/Thing.fs\", \"work/flow/Step.fs\"") ]
                ...
                Expect.exists
                    ...
                    (fun sp ->
                        sp.Path = GovernedPath "src/Lib/Thing.fs"
                        && sp.MatchedGlob = GovernedPath "src/**")
            ...
            test "V2a — a declared-driven domain gate carries the REAL path-map glob, not the self-glob (SC-006)" {
                let m =
                    runWith (Loop.ExplicitPaths []) [ handoffRead (declaringJson "\"src/Lib/Thing.fs\"") ]
            ...
            test "V2b — a path present in both the sensed set and governedReferences selects once (SC-003)" {
                ...
                let m =
                    runWith Loop.DefaultRange [ handoffRead (declaringJson "\"src/Lib/Thing.fs\"") ]

                let build = selectedGateFor m "package-api:build"

                let forThisPath =
                    build.SelectingPaths
                    |> List.filter (fun sp -> sp.Path = GovernedPath "src/Lib/Thing.fs")

                Expect.equal
                    (List.length forThisPath)
                    1
                    "the doubly-sourced path yields exactly one selecting-path entry (deduped)"
            ...
            test "V5a — no handoff ⇒ the route doc is byte-identical to the pre-F082 projection (SC-002)" {
                ...
                let _, expectedRoute =
                    projectExpected validCatalog candidates (Some(snapshotOf gitSrcChange defaultOpts))

                Expect.equal
                    (Option.get m.RouteDoc)
                    expectedRoute
                    "the absent-handoff route doc matches the pre-seam pipeline (the identity merge)"
            }
            ...
            test "V5b — an empty governedReferences handoff contributes no routing candidate (SC-002)" {
                ...
                Expect.exists
                    ...
                    "the handoff's own evidence gate still pre-selects (F081 unchanged)"
            }
            ...
            test "V6 — a malformed handoff adds zero routing candidates yet keeps its blocking integrity gate (SC-005)" {
                ...
                Expect.exists
                    ...
                    "its blocking integrity gate still appears (consume, unchanged)"
            }
        ]
