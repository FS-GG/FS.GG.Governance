...
let tests =
    testList
        ...
        [
            test
                "PersistStore=true + non-degraded store ⇒ emits exactly one PersistStore(StorePath, pipeline) (T012/T019)" {
                ...
                let grown =
                    expectedGrownStore fakeExecPort fakeSensor validCatalog store m4.SelectedGates baseHead
