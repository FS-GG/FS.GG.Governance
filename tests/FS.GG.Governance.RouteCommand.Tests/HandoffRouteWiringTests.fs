...
let private withTempDir (body: string -> 'a) : 'a =
    let dir =
        Path.Combine(Path.GetTempPath(), "fsgg-handoff-" + Guid.NewGuid().ToString("N"))
        ...
        [
            test "handoff gates appear in gates.json and route.json's selected gates (FR-008)" {
                ...
                Expect.stringContains
                    gatesDoc
                    "sdd-handoff:evidence"
                    "the handoff gate appears in gates.json (the registry)"

                Expect.stringContains
                    routeDoc
                    "sdd-handoff:evidence"
                    "the handoff gate appears in route.json's selected gates"
            }

            test
                "the real Handoffs port returns documents in stable <id> order; an empty repo ⇒ [] (FR-012, Principle IV edge)" {
                withTempDir (fun dir ->
                    ...
                    Expect.equal
                        ...
                        [
                            "readiness/aaa/governance-handoff.json"
                            "readiness/bbb/governance-handoff.json"
                        ]
                        "the port reads in stable ordinal <id> order regardless of filesystem order")

                withTempDir (fun dir ->
                    Expect.isEmpty
                        ((Interpreter.realPorts dir).Handoffs dir)
                        "a repo with no readiness/ handoffs ⇒ [] (the no-op path)")
            }
        ]
