...
let private wiredReRender
    (req: Loop.RunRequest)
    (count: int ref)
    : string -> RenderMode.RenderMode -> Watch.WatchSignal =
    fun root _md ->
        try
            let roPorts =
...
let tests =
    testList
        ...
        [
            test
                "real FileSystemWatcher: a tracked-file change drives exactly ONE settled re-render, and the watch writes no contract artifact (SC-005, FR-009)" {
                withTempRepo (fun dir ->
                    ...
                    Expect.isGreaterThanOrEqual
                        count.Value
                        1
                        "a tracked-file change drove at least one settled re-render through the real FileSystemWatcher (SC-005)"
                    ...
                    Expect.isFalse
                        ...
                        "watch re-render writes no contract artifact")
            }
            ...
            test "read-only: the wired re-render computes a report view but emits no WriteArtifact (FR-009)" {
                withTempRepo (fun dir ->
                    ...
                    Expect.isFalse
                        (File.Exists(Path.Combine(dir, "readiness", "route.json")))
                        "no route.json written by the read-only re-render"

                    Expect.isFalse
                        (File.Exists(Path.Combine(dir, ".fsgg", "gates.json")))
                        "no gates.json written by the read-only re-render")
            }

            test
                "headless (H3 / #47): `route --watch` over a nonexistent repo fails to arm the watcher and exits input-unavailable (3), never crashing" {
                ...
                let missing =
                    Path.Combine(Path.GetTempPath(), "fsgg-watch-missing-" + System.Guid.NewGuid().ToString("N"))
                ...
                Expect.equal
                    code
                    3
                    "a nonexistent watch root ⇒ InputUnavailable exit (3), not a FileSystemWatcher crash"
            }

            test
                "headless (H3 / #47): the shared `safeKeyPoll` swallows an unreadable console (redirected stdin) instead of throwing" {
            ...
            test
                "safe failure: a transiently-unreadable/malformed tree yields InputUnreadable, no crash, no fabricated report (FR-010)" {
                ...
                let dir =
                    Path.Combine(Path.GetTempPath(), "fsgg-watch-bad-" + System.Guid.NewGuid().ToString("N"))
