...
let tests =
    testList
        ...
        [
            test
                "real git + real catalog: --since HEAD~1 writes both artifacts = F021/F020 bytes, re-run identical, exit 0" {
                withTempRepo (fun dir ->
                    ...
                    let expectedGates, expectedRoute =
                        projectExpected validCatalog candidates (Some(snapshotOfRepo dir (sinceOpts "HEAD~1")))
                    ...
                    Expect.equal
                        (File.ReadAllText req.GatesOut)
                        expectedGates
                        "gates.json bytes = GatesJson projection (SC-001)"

                    Expect.equal
                        (File.ReadAllText req.RouteOut)
                        expectedRoute
                        "route.json bytes = RouteJson projection (SC-001)"
