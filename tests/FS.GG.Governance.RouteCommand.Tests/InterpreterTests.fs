    ...
    testList
        ...
        [ // ── US1: route + persist, bytes = F020/F021 ──
            ...
            test "writes both artifacts byte-for-byte equal to the F021/F020 projections, exits 0 (US1, SC-001, SC-007)" {
                ...
                let expectedGates, expectedRoute =
                    projectExpected validCatalog candidates (Some(snapshotOf git defaultOpts))

                Expect.equal
                    (writtenOf cap Loop.GatesArtifact)
                    (Some(req.GatesOut, expectedGates))
                    "gates.json = GatesJson.ofGateRegistry"

                Expect.equal
                    (writtenOf cap Loop.RouteArtifact)
                    (Some(req.RouteOut, expectedRoute))
                    "route.json = RouteJson.ofRouteResult (Some report)"
            ...
            test
                "F23: a change under a product surface ⇒ route.json carries a productSurfaces section + summary line (US1 vertical slice)" {
            ...
            test "the summary lists each selected gate by id with its path and the cost rollup (US1 AS2)" {
                ...
                Expect.stringContains summary req.GatesOut "summary names the gates path (host operational line)"
                |> ignore
            }
            ...
            test "a valid empty catalog yields an empty gate list and exits 0 (SC-006, empty catalog)" {
                ...
                let expectedGates, _ =
                    projectExpected emptyCatalog candidates (Some(snapshotOf git defaultOpts))
                ...
                Expect.equal
                    (writtenOf cap Loop.GatesArtifact)
                    (Some(req.GatesOut, expectedGates))
                    "gates.json = projection of the empty registry"
            ...
            test "ExplicitPaths routes exactly those paths WITHOUT consulting git (US2 AS1)" {
                ...
                let req, cap, model =
                    runWith validCatalog gitNotRepo (Loop.ExplicitPaths [ gp "src/Lib/Thing.fs" ]) Loop.Text
                // ExplicitPaths ⇒ no snapshot ⇒ base/head None ⇒ every gate notEvaluated (still evaluated:true).
                let expectedGates, expectedRoute =
                    projectExpected validCatalog [ gp "src/Lib/Thing.fs" ] None

                Expect.equal
                    model.Exit
                    Loop.Success
                    "explicit paths bypass git entirely ⇒ success despite not-a-repo git"

                Expect.equal
                    (writtenOf cap Loop.RouteArtifact)
                    (Some(req.RouteOut, expectedRoute))
                    "route.json = projection of the explicit paths"

                Expect.equal
                    (writtenOf cap Loop.GatesArtifact)
                    (Some(req.GatesOut, expectedGates))
                    "gates.json = full catalog"
            }
            ...
            test "twice-run over fixed inputs yields byte-identical artifacts and --json summary (US3, SC-002)" {
                ...
                Expect.equal
                    (writtenOf cap1 Loop.GatesArtifact)
                    (writtenOf cap2 Loop.GatesArtifact)
                    "gates.json byte-identical across runs"

                Expect.equal
                    (writtenOf cap1 Loop.RouteArtifact)
                    (writtenOf cap2 Loop.RouteArtifact)
                    "route.json byte-identical across runs"
            ...
            test
                "neither the written artifacts nor the summary carry an excluded verdict/clock/abs-path token (US3 AS3, SC-005)" {
                let git = gitWithChanges [ 'M', "src/Lib/Thing.fs" ]
                let _, cap, _ = runWith validCatalog git Loop.DefaultRange Loop.Json

                let gates =
                    writtenOf cap Loop.GatesArtifact |> Option.map snd |> Option.defaultValue ""
