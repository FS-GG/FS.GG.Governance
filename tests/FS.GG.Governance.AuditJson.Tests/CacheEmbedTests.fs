...
let private relInputs =
    { baseInputs with
        Check = CheckId "rel"
        Domain = DomainId "build"
    }

let private lintInputs =
    { baseInputs with
        Check = CheckId "lint"
...
let private recordedStore =
    storeOf [ shipInputs, refA; relInputs, refR; lintInputs, refL ]
...
let private causeKind (verdict: JsonElement) : string =
    strField (verdict.GetProperty "cause") "kind"
...
let private sectionIds (doc: JsonDocument) (name: string) : string list = section doc name |> List.map itemId
        ...
        [
            ...
            test "an inputsChanged gate item names its changed categories in report order (US2.1, SC-005)" {
                ...
                | Some(MustRecompute(InputsChanged cats)) ->
                    Expect.equal
                        (causeCategories v)
                        (cats |> List.map categoryToken)
                        "categories verbatim, in report order"
            ...
            test
                "a reusable verdict on a base-blocking gate leaves it a blocker with full six-field enforcement (US2.4, L7, SC-004)" {
                use doc = parse (AuditJson.ofShipDecision richDecision (Some mixedReport) [])
                ...
                Expect.equal
                    (enforcement item "effectiveSeverity")
                    "blocking"
                    "effective severity unchanged by the cache verdict"
            }

            test
                "additive: verdict / exitCodeBasis / sections / enforcement are byte-identical to the None projection (L7, FR-008)" {
                ...
                Expect.equal
                    (strField docSome.RootElement "verdict")
                    (strField docNone.RootElement "verdict")
                    "verdict unchanged"

                Expect.equal
                    (strField docSome.RootElement "exitCodeBasis")
                    (strField docNone.RootElement "exitCodeBasis")
                    "exitCodeBasis unchanged"

                Expect.equal
                    (strField docSome.RootElement "schemaVersion")
                    (strField docNone.RootElement "schemaVersion")
            ...
            test "an orphan report entry (GateId matching no item) adds nothing (L5, FR-006)" {
                ...
                for name in [ "blockers"; "warnings"; "passing" ] do
                    Expect.equal
                        (sectionIds docOrphan name)
                        (sectionIds docNone name)
                        (sprintf "%s unchanged by orphan" name)

                Expect.isFalse
                    (allItems docOrphan
                     |> List.exists (fun it -> hasField it "id" && itemId it = "ghost:gate"))
                    "orphan id absent"
            }
            ...
            test "no raw freshness input value / extra cache-derived field leaks; evidence verbatim (L8, SC-007)" {
                ...
                Expect.equal
                    (strField (cacheOf (gateItemById doc "build:ship")) "evidence")
                    "ev-A"
                    "evidence verbatim, never dereferenced"
            }
            ...
            test
                "a duplicate GateId in the report resolves to the first entry by report order, deterministically (L6, FR-007)" {
                let dup =
                ...
                Expect.equal
                    (kindOf (cacheOf (gateItemById doc "build:ship")))
                    expectedKind
                    "first-by-report-order verdict wins"

                Expect.equal
                    (AuditJson.ofShipDecision richDecision (Some dup) [])
                    (AuditJson.ofShipDecision richDecision (Some dup) [])
                    "deterministic"
            }
            ...
            test "the None case is evaluated:false with every gate item notEvaluated; Some _ is evaluated:true (L2, L9)" {
                ...
                Expect.isFalse
                    (docNone.RootElement.GetProperty("cacheEligibilityEvaluated").GetBoolean())
                    "evaluated false under None"
                ...
                Expect.isTrue
                    (docSome.RootElement.GetProperty("cacheEligibilityEvaluated").GetBoolean())
                    "evaluated true under Some"
            }

            test
                "totality: None / empty report / clean empty decision each yield a document with the section (L11, SC-006)" {
                let cases =
                    [
                ...
                for (label, json) in cases do
                    ...
                    Expect.equal
                        ...
                        (sprintf "%s yields a JSON object" label)

                    Expect.isTrue
                        (hasField doc.RootElement "cacheEligibilityEvaluated")
                        (sprintf "%s carries the cache section" label)
            }

            test "an evaluated-but-empty report is evaluated:true with every gate item notEvaluated (L9)" {
                use doc =
                    parse (AuditJson.ofShipDecision richDecision (Some(CacheEligibilityReport [])) [])

                Expect.isTrue
                    (doc.RootElement.GetProperty("cacheEligibilityEvaluated").GetBoolean())
                    "empty report is evaluated:true"
