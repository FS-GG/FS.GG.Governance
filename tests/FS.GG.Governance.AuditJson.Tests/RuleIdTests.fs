...
let private emit (decision: ShipDecision) : string =
    AuditJson.ofShipDecision decision None []
...
let tests =
    testList
        ...
        [
            ...
            test
                "no item emits an unattributed: marker; every ruleId is non-empty and source-prefixed (T014, FR-010, SC-006)" {
                let decision = decisionOf ruleIdRoute Gate Standard
                use doc = parse (emit decision)
            ...
            test
                "RuleIdInvariance: the ruleId set is byte-identical across every profile and run mode, no finding dropped (T011, SC-002/SC-004)" {
