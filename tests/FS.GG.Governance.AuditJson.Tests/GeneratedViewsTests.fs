    ...
    testList
        ...
        [
            test "empty ⇒ omitted entirely ⇒ byte-identical to ofShipDecision (FR-004)" {
                let withEmpty =
                    AuditJson.ofShipDecisionWithGeneratedViews emptyCleanDecision None [] []
            ...
            test
                "source-drift entries carry the full wire shape, sorted by viewId, both severities (gate+block-on-ship ⇒ blocking)" {
                let findings =
                    [
                ...
                let json =
                    AuditJson.ofShipDecisionWithGeneratedViews emptyCleanDecision None [] (findings |> List.map detail)
                ...
                Expect.stringContains
                    json
                    (sprintf "\"%s\"" (categoryToken CoveredArtifactsCat))
                    "drifted category token"
            ...
            test "an undeterminable entry carries cause + detail reason (FR-008), not drifted" {
                let json =
                    AuditJson.ofShipDecisionWithGeneratedViews
                        emptyCleanDecision
                        None
                        []
                        [ detail (finding "v" (CE.Undeterminable "no recorded provenance")) ]
