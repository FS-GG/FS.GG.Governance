    ...
    testList
        ...
        [
            testPropertyWithConfig
                fsCheckConfig
                "ofShipDecision d = ofShipDecision d byte-for-byte (AS1, SC-002)"
                (fun d -> AuditJson.ofShipDecision d None [] = AuditJson.ofShipDecision d None [])

            test
                "value-equal decisions assembled from differently-ordered route inputs project identically (AS2, SC-003)" {
                ...
                let fB =
                    mkFinding
                        ...
                        (GovernedPath "src/b.fs")
                        (ProtectedBoundaryUnknown(SurfaceId "s"))
            ...
            test
                "schemaVersion field equals AuditJson.schemaVersion and every object's field order is fixed (AS3, FR-013)" {
                ...
                for it in List.concat [ section doc "blockers"; section doc "warnings"; section doc "passing" ] do
                    ...
                    | "gate" ->
                        Expect.equal
                            (fieldOrder it)
                            [ "kind"; "id"; "ruleId"; "enforcement"; "cacheEligibility" ]
                            "gate item field order"
                    | "finding" ->
                        Expect.equal
                            (fieldOrder it)
                            [ "kind"; "id"; "ruleId"; "path"; "enforcement" ]
                            "finding item field order"
            ...
            test
                "exclusion sweep: only the contracted fields exist, and only the declared vocabulary is emitted (AS4, SC-007, FR-011/FR-012)" {
                ...
                let contracted =
                    set
                        [
                            ...
                            "ruleId"
                            "path"
                            "enforcement" // 068: additive per-finding ruleId
                            "baseSeverity"
                            "maturity"
                            "mode"
                            "profile"
                ...
                let foreign =
                    fieldNames doc.RootElement
                    |> List.filter (fun n -> not (Set.contains n contracted))
                ...
                let maturities =
                    set [ "observe"; "warn"; "blockOnPr"; "blockOnShip"; "blockOnRelease" ]
                ...
                let ruleIds =
                    set
                        [
                            ...
                            "boundary:unknownProtectedBoundaryPath"
                        ]
