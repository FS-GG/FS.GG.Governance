...
let tests =
    testList
        ...
        [
            test
                "a decision with >=1 blocker projects verdict:fail / exitCodeBasis:blocked with every blocker by identity + enforcement (AS1, SC-001)" {
            ...
            test
                "a Pass decision with empty blockers projects verdict:pass / exitCodeBasis:clean with a present empty blockers array (AS2, FR-009)" {
            ...
            test
                "warnings + passing each render in their own section; no item in more than one; union equals the decision's items (AS3, FR-005, SC-001)" {
                let d = richDecision
                use doc = parse (AuditJson.ofShipDecision d None [])
            ...
            test
                "verdict and exitCodeBasis are echoed VERBATIM from the value, never recomputed, and no numeric exitCode appears (FR-002/FR-003, SC-004)" {
            ...
            test
                "Synthetic: a reason with JSON-special characters round-trips exactly — escaping delegated to the writer (FR-012)" {
