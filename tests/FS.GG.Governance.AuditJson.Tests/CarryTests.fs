...
let private allItems (d: ShipDecision) =
    List.concat [ d.Blockers; d.Warnings; d.Passing ]

let private severityTok (s: Severity) =
    match s with
    | Advisory -> "advisory"
    | Blocking -> "blocking"
...
let tests =
    testList
        ...
        [
            test
                "a relaxed base-Blocking warning shows baseSeverity:blocking AND effectiveSeverity:advisory plus mode/profile/maturity/reason (AS1, FR-011, SC-005)" {
                ...
                for it in relaxed do
                    ...
                    Expect.equal
                        (enforcement it "effectiveSeverity")
                        "advisory"
                        "effective severity present and advisory"
            ...
            test
                "every blocker/warning/passing item carries all six enforcement fields verbatim from its F023 decision (AS2, FR-006, SC-005)" {
                let d = richDecision
                use doc = parse (AuditJson.ofShipDecision d None [])
                ...
                for it in List.concat [ section doc "blockers"; section doc "warnings"; section doc "passing" ] do
                    ...
                    Expect.equal
                        (enforcementFields it)
                        (Map.find id expected)
                        (sprintf "six enforcement fields verbatim + ordered for %s" id)
            }

            test
                "a finding item's identity carries both findingIdToken and governed path; a gate item's its gateIdValue, neither re-derived (AS3, FR-004/FR-010)" {
                let d = richDecision
                use doc = parse (AuditJson.ofShipDecision d None [])
            ...
            test "a GateId / governed-path containing the id separator renders verbatim — no re-parse (FR-008/FR-010)" {
                ...
                Expect.isTrue
                    (Set.contains "src/boundary/Api.fs" findingPaths)
                    "governed path with `/` rendered verbatim"
            }
        ]
