...
let private withTempDir body =
    let dir =
        Path.Combine(Path.GetTempPath(), "fsgg-command-host-" + Guid.NewGuid().ToString("N"))
    ...
    testList
        ...
        [
            ...
            test "executionPlan with a budget fold runs it (Ship/Verify branch) over empty gates" {
                ...
                let parms =
                    {
                        CommandHost.BudgetFold = Some(fun _ -> Map.empty, foldReport)
                    }

                let plan, inputs, report =
                    CommandHost.executionPlan
                        parms
                        (Some CommandHost.emptySensedFacts)
                        (Some EvidenceReuse.empty)
                        []
                        None
                        "."
            ...
            test "discoverHandoffs distinguishes absent and empty readiness state" {
                withTempDir (fun repo ->
                    Expect.equal
                        (CommandHost.discoverHandoffs repo)
                        CommandHost.HandoffsAbsent
                        "no directory is absent"

                    Directory.CreateDirectory(Path.Combine(repo, "readiness")) |> ignore

                    Expect.equal
                        (CommandHost.discoverHandoffs repo)
                        CommandHost.HandoffsEmpty
                    ...
                    | [ read ] ->
                        ...
                        | Error diagnostic ->
                            Expect.equal
                                diagnostic.Cause
                                Model.Malformed
                                "unreadable state becomes a malformed diagnostic"
