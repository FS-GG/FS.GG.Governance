    ...
    testList
        ...
        [
            test "references only Config + Routing + BCL + FSharp.Core (no network/host/CLI/git)" {
                ...
                Expect.isEmpty
                    offending
                    (sprintf "ProductSurfaces must depend on Config/Routing/BCL/FSharp.Core only; found: %A" offending)
                ...
                Expect.isEmpty
                    forbidden
                    (sprintf
                        "ProductSurfaces must not reference network/kernel/host/Snapshot/CLI/adapters; found: %A"
                        forbidden)
            }
            ...
            test "classify is a pure function of its three inputs (identical output for identical input)" {
                ...
                let paths =
                    [ "src/Api.fsi"; "docs/guide.md"; "release/notes.md" ] |> List.map normalizePath
