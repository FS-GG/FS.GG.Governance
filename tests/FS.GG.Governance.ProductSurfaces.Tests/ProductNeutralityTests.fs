...
let private surfaceIds (rep: ProductSurfaceReport) =
    rep.Classifications
    |> List.map (fun c -> let (SurfaceId s) = c.Surface in s)
    |> Set.ofList
    ...
    testList
        ...
        [
            test "each report's surface ids are exactly those the fixture declared (no fabrication)" {
                let declaredOf name =
                    (factsOf name).Capabilities.Surfaces
                    |> List.map (fun s -> let (SurfaceId i) = s.Id in i)
                    |> Set.ofList

                Expect.isTrue
                    (Set.isSubset (surfaceIds allKinds) (declaredOf "product-surface-all-kinds"))
                    "all-kinds surfaces are declared"

                Expect.isTrue
                    (Set.isSubset (surfaceIds standalone) (declaredOf "generated-product-standalone"))
                    "standalone surfaces are declared"
            }
            ...
            test "no surface id unique to one fixture appears in the other's report (no leaked identity)" {
                ...
                for uniqueToAllKinds in [ "extra-api"; "tokens"; "sample"; "ship-skill" ] do
                    Expect.isFalse
                        (standaloneIds.Contains uniqueToAllKinds)
                        (sprintf "'%s' must not leak into the standalone report" uniqueToAllKinds)
            }
            ...
            test "the report carries no template-profile/baseline/generator string from the catalog" {
                ...
                for catalogOnly in [ "fsharp-lib"; "public-api.baseline.txt"; "api-surface"; "design-tokens" ] do
                    Expect.isFalse
                        (rendered.Contains catalogOnly)
                        (sprintf "catalog-only token '%s' must not appear in the classification" catalogOnly)
            }
        ]
