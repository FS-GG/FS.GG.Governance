...
let private report =
    classifyPaths "product-surface-all-kinds" "standard" [ "release/notes.md"; "src/Api.fsi"; "docs/guide.md" ]
...
let tests =
    testList
        ...
        [
            ...
            test
                "co-kind tie: src/**/*.fsi under two package surfaces → ordinal-first SurfaceId (OrdinalSurfaceTiebreak)" {
            ...
            test "re-ordering the authored surfaces does not change any winner (SC-005)" {
                ...
                let paths =
                    [ "release/notes.md"; "src/Api.fsi"; "src/Internal.fs"; "docs/guide.md" ]

                let baseRep =
                    ProductSurfaces.classify
                        facts
                ...
                let revRep =
                    ProductSurfaces.classify
                        reversed
                        (Routing.route reversed (paths |> List.map normalizePath))
                        (ProfileId "standard")
