...
let private report =
    classifyPaths "product-surface-all-kinds" "standard" [ "src/Api.fsi"; "docs/guide.md"; "release/notes.md" ]
            ...
            test "none ≤ target ⇒ the cheapest declared tier is selected" {
                    ...
                    { facts with
                        Capabilities =
                            { facts.Capabilities with
                                Checks = extra :: facts.Capabilities.Checks
                            }
                    }
                ...
                let rep =
                    ProductSurfaces.classify
                        facts'
                        (Routing.route facts' [ normalizePath "samples/App/Program.fs" ])
                        (ProfileId "standard")

                let c =
                    rep.Classifications
                    |> List.find (fun c -> c.Path = normalizePath "samples/App/Program.fs")
