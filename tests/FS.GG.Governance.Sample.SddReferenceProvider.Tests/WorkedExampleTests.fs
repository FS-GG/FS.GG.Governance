...
let private runWorkedExample () =
    ...
    let req =
        runRequest target lifecycleReservedPaths (Some SddReferenceProvider.provider)
                ...
                try
                    ...
                    | Built(exitCode, output) ->
                        Expect.equal
                            exitCode
                            0
                            (sprintf "dotnet build MyApp.sln must succeed first-attempt:\n%s" output)
                finally
                    cleanup target
            }
            ...
            test "manifest projection matches the committed golden, byte-for-byte and deterministically" {
                ...
                try
                    ...
                    Expect.equal
                        actual
                        golden
                        "manifest projection drifted from the golden (BLESS_FIXTURES=1 to regenerate)"
