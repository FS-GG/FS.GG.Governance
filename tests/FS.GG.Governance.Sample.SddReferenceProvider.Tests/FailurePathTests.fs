    ...
    testList
        ...
        [ // FR-010 / SC-007: no provider selected ⇒ today's behavior, zero diff. `init` emits ZERO
            ...
            test "no provider selected → NoProvider, zero effects, nothing written" {
                ...
                try
                    let _, effects =
                        Loop.init (runRequest target lifecycleReservedPaths None).Request None
            ...
            test "collision at an emitted path → Refused(Collision), nothing overwritten" {
                ...
                try
                    ...
                    Expect.equal
                        (filesUnder target)
                        [ "README.md" ]
                        "only the seeded file is present — nothing else written"

                    Expect.equal
                        (File.ReadAllText seeded)
                        "PRE-EXISTING — must not be overwritten"
                        "seeded file unchanged"
                finally
                    cleanup target
            }
            ...
            test "cloned provider runs through the identical seam — only emitted files differ" {
                ...
                try
                    ...
                    Expect.equal
                        (File.ReadAllText(Path.Combine(target, "hello.txt")))
                        "from a clone\n"
