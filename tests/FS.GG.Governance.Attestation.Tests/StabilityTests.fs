        ...
        [
            ...
            test "changing ONLY a SensedDuration leaves Identity unchanged" {
                let fastPack =
                    {
                        Kind = Pack
                        Record = makeRecord 0 1L
                    }

                let slowPack =
                    {
                        Kind = Pack
                        Record = makeRecord 0 999999L
                    }
            ...
            test "changing a reproducible input (an artifact digest) changes Identity" {
                let id1 =
                    (Attestation.summarize (snapshotWith [ ArtifactHash "x" ] [ packRun ]) twoPacked).Identity

                let id2 =
                    (Attestation.summarize (snapshotWith [ ArtifactHash "y" ] [ packRun ]) twoPacked).Identity
