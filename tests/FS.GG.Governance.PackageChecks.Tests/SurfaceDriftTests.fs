...
// Reflective API surface-drift + dependency-scope guard (Principle II). Reflection lives ONLY in tests.

let private library = typeof<PackageFacts>.Assembly

let private baselinePath =
    Path.Combine(repoRoot, "surface", "FS.GG.Governance.PackageChecks.surface.txt")
...
let tests =
    testList
        "SurfaceDrift"
        [ test "PackageChecks public surface equals the committed baseline" {
          ...
          test "PackageChecks references only FS.GG.Governance.*/BCL/FSharp.Core (no network/SDK)" {
