...
let private allDeclaredIds = CP.packs |> List.collect CP.ruleIds |> Set.ofList
    ...
    testList
        ...
        [
            ...
            test "C1 every code the four real evaluators emit is declared by the composed profile" {
                ...
                let undeclared =
                    emitted |> List.filter (fun code -> not (Set.contains code allDeclaredIds))
            ...
            test "C2 the profile declares exactly CodeChecks' twelve closed rule tokens" {
                let fromDu =
                    ...
                    |> Array.map (fun case ->
                        ...
                        | other ->
                            failtestf "FindingId union case '%s' did not construct a FindingId: %A" case.Name other)
                    |> List.ofArray
                    |> List.sort
            ...
            test "C5 each pack's real findings carry the maturity the composed profile declares" {
                ...
                Expect.equal
                    (CP.declaredMaturity CP.EffectBoundary)
                    BlockOnPr
                    "#369 stays blocking for opted-in transitions"
            ...
            test "C6 #368 idiomatic-simplicity fires red through the profile and passes clean" {
                ...
                Expect.equal
                    ...
                    "the composed finding keeps the pack's own stable identity"
            ...
            test "C6 #369 effect-boundary fires red through the profile and passes clean" {
                ...
                Expect.equal
                    composed.Maturity
                    BlockOnPr
                    "at the profile's declared maturity for #369 — the one pack that blocks"
            ...
            test "C6 #370 evidence-boundary fires red through the profile and passes clean" {
                ...
                Expect.contains
                    (red |> List.map (fun f -> f.Code))
                    code
                    "dispatch-only evidence cannot satisfy an observed-outcome obligation"
            ...
            test "C9 the composed profile projects to exactly one gate per pack" {
                ...
                for c in CP.checks do
                    ...
                    Expect.isNone
                        c.Command
                        "composed gates are command-free — a tooling.yml binding would dangle in every repository that resolved the published set"
