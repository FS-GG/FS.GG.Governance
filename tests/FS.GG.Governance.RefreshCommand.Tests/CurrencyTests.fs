    ...
    testList
        ...
        [
            test "matching digests + generator ⇒ Current (by digest, SC-002)" {
                let status =
                    decide
                        (Some(digestsOf [ "d1" ], GeneratorVersion "g1"))
                        (Ok(digestsOf [ "d1" ], GeneratorVersion "g1"))
            ...
            test "a changed source digest ⇒ stale, drifted = [CoveredArtifactsCat]" {
                let status =
                    decide
                        (Some(digestsOf [ "d1" ], GeneratorVersion "g1"))
                        (Ok(digestsOf [ "d2" ], GeneratorVersion "g1"))

                Expect.equal
                    status
                    (WouldRegenerate [ CoveredArtifactsCat ])
                    "only the covered-artifacts category drifted"
            }
            ...
            test "a changed generator version ⇒ stale, drifted = [GeneratorVersionCat]" {
                let status =
                    decide
                        (Some(digestsOf [ "d1" ], GeneratorVersion "g1"))
                        (Ok(digestsOf [ "d1" ], GeneratorVersion "g2"))

                Expect.equal
                    status
                    (WouldRegenerate [ GeneratorVersionCat ])
                    "only the generator-version category drifted"
            }
            ...
            test "a source whose digest cannot be sensed ⇒ StaleUnresolved, NEVER Current (FR-010)" {
                let status =
                    decide (Some(digestsOf [ "d1" ], GeneratorVersion "g1")) (Error "source not found: src.txt")
