    ...
    testList
        ...
        [
            ...
            test "the projection contains exactly the supplied strings (two different manifests)" {
                let docA =
                    RefreshJson.ofRefreshDecision (decisionFor (parseYml (manifest "view-one" "baseline" "a/one.txt")))

                let docB =
                    RefreshJson.ofRefreshDecision (
                        decisionFor (parseYml (manifest "view-two" "totally-custom" "b/two.txt"))
                    )
