    ...
    testList
        ...
        [
            ...
            test "ManifestLoaded(Ok) emits SenseSource + ReadRecorded per in-scope entry" {
                ...
                let _, effects =
                    Loop.update (Loop.ManifestLoaded(Ok(parseYml refreshYmlTwoViews))) m0
            ...
            test "a stale entry emits RegenerateView, then Regenerated'(Ok) emits RecordProvenance" {
                ...
                let m2, _ =
                    Loop.update (Loop.Sensed("doc", Ok(digestsOf [ "d2" ], GeneratorVersion "g1"))) m1
                // Recorded differs ⇒ stale ⇒ decideSensing emits RegenerateView (write mode).
                let m3, effects3 =
                    Loop.update (Loop.RecordedRead("doc", Some(digestsOf [ "d1" ], GeneratorVersion "g1"))) m2
            ...
            test "a non-stale entry emits neither RegenerateView nor RecordProvenance" {
                ...
                let m2, _ =
                    Loop.update (Loop.Sensed("doc", Ok(digestsOf [ "d1" ], GeneratorVersion "g1"))) m1
            ...
            test "roll-up precedence: any unresolved ⇒ StaleUnresolved' even alongside a would-regenerate" {
                ...
                let m3, _ =
                    Loop.update (Loop.Sensed("cat", Ok(digestsOf [ "x" ], GeneratorVersion "g1"))) m2
