    ...
    testList
        ...
        [
            test "--view <id> senses only the selected view; the other is NotEvaluated" {
                ...
                let m2, _ =
                    Loop.update (Loop.Sensed("doc", Ok(digestsOf [ "d" ], GeneratorVersion "g1"))) m1

                let m3, _ =
                    Loop.update (Loop.RecordedRead("doc", Some(digestsOf [ "d" ], GeneratorVersion "g1"))) m2
            ...
            test "a not-evaluated view projects status `not-evaluated` in refresh.json" {
                ...
                Expect.stringContains
                    (RefreshJson.ofRefreshDecision decision)
