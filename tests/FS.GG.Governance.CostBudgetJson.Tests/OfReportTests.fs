    ...
    testList
        ...
        [
            ...
            test "recompute decision reuses the F042 cause shape (inputsChanged + categories)" {
                ...
                let cats =
                    [ for c in cause.GetProperty("categories").EnumerateArray() -> c.GetString() ]
            ...
            test "overBudget decision carries class, ceiling, and a reason string" {
                ...
                Expect.equal
                    (skip.GetProperty("reason").GetString())
                    "d:skip (exhaustive) exceeds the cheap budget"
                    "reason"
                ...
                Expect.equal
                    (defer.GetProperty("reason").GetString())
                    "e:defer (high) exceeds the medium budget"
                    "reason"
            }
            ...
            test "an agentReviewed review is labelled but its CacheKey is NEVER emitted" {
                ...
                Expect.isFalse
                    (json.Contains "SECRET-CACHE-KEY-DO-NOT-EMIT")
                    "the CacheKey is not a blocking signal in the document"
            }
            ...
            test "top-level field order is schemaVersion < decisions < findings" {
                Expect.isLessThan
                    (json.IndexOf "\"schemaVersion\"")
                    (json.IndexOf "\"decisions\"")
            ...
            test "findings render kind/baseSeverity, with categories ONLY for stale" {
                ...
                let cats =
                    [ for c in stale.GetProperty("categories").EnumerateArray() -> c.GetString() ]
