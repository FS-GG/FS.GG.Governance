    ...
    testList
        ...
        [
                    ...
                    for mode in modes do
                        for p in profiles do
                            ...
                            Expect.equal
                                viaLeaf
                                viaCore
                                (sprintf "leaf decision must equal the truth table at (%A, %A, %A)" m mode p)
            }
            ...
            test "block-on-pr: WARNING under verify+standard, BLOCKER under verify+strict, BLOCKER at ship (C1/FR-009)" {
                ...
                Expect.equal
                    (decisionOf f Verify Standard).EffectiveSeverity
                    Advisory
                    "standard profile: a warning under verify"

                Expect.equal
                    (decisionOf f Verify Strict).EffectiveSeverity
                    Blocking
                    "strict profile tightens the floor down to verify"
