    ...
    testList
        ...
        [
            ...
            test "a relaxing profile yields a visible warning while the carried Cause is unchanged (T035/US3)" {
                ...
                Expect.equal
                    relaxed.EffectiveSeverity
                    Advisory
                    "light profile under verify relaxes block-on-pr to a warning"

                Expect.equal
                    f.Cause
                    (Undeterminable "lock unreadable")
                    "the carried cause is never altered by relaxation"
            }
