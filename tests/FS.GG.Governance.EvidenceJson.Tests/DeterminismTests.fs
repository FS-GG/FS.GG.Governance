    ...
    testList
        ...
        [
            ...
            test "dependency input order does not change the bytes (sorted by (dependent, dependency))" {
                ...
                let pairs =
                    [
                        for d in root.GetProperty("dependencies").EnumerateArray() ->
                            strProp "dependent" d, strProp "dependency" d
                    ]
            ...
            test "disclosure input order does not change the bytes (sorted by (rule, justification))" {
                ...
                let pairs =
                    [
                        for d in root.GetProperty("disclosures").EnumerateArray() ->
                            strProp "rule" d, strProp "justification" d
                    ]
