        ...
        [
            ...
            test "every worked example re-decides identically on a second call" {
                for (f, _) in workedExamples do
                    Expect.equal
                        (AdvisoryPromotion.decide f)
                        (AdvisoryPromotion.decide f)
                        "decide is a pure function of its facts"
            }

            testPropertyWithConfig
                fscheckConfig
                "decide is a pure function of its facts (SC-005, L-D12)"
                (fun (f: PromotionFacts) -> AdvisoryPromotion.decide f = AdvisoryPromotion.decide f)
        ]
