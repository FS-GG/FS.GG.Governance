    ...
    testList
        ...
        [
            test "bare default — no basis, zero confirmations ⇒ StaysAdvisory NoPermittedBasis (L-D3)" {
                Expect.equal
                    (AdvisoryPromotion.decide (facts None 0 3 None))
                    (StaysAdvisory NoPermittedBasis)
                    "bare finding holds advisory with NoPermittedBasis"
            }
            ...
            testPropertyWithConfig
                fscheckConfig
                "advisory-by-default — no basis satisfied ⇒ never EligibleToBlock (SC-001, L-D4)"
