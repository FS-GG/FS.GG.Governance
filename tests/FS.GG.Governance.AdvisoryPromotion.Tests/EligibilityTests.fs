    ...
    testList
        ...
        [
            ...
            testPropertyWithConfig
                fscheckConfig
                "all-named — satisfiedBases (decide f) equals the bases f satisfies, in fixed order (SC-002, L-D6)"
                (fun (f: PromotionFacts) ->
                    AdvisoryPromotion.satisfiedBases (AdvisoryPromotion.decide f) = expectedBases f)

            testPropertyWithConfig
                fscheckConfig
                "eligible iff a basis — decide f is EligibleToBlock iff f satisfies ≥1 basis (L-D5)"
                (fun (f: PromotionFacts) ->
                    let eligible =
