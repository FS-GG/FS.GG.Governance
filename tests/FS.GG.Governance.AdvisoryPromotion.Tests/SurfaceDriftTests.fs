...
let private advisoryPromotionAsm =
    SurfaceDrift.assemblyNamed "FS.GG.Governance.AdvisoryPromotion"
    ...
    testList
        ...
        [
            ...
            test "the public surface is exactly the two modules (Model + AdvisoryPromotion), nothing else" {
                let typeNames =
                    advisoryPromotionAsm.GetExportedTypes()
                    |> Array.choose (fun t -> Option.ofObj t.FullName)

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.AdvisoryPromotion.ModelModule"))
                    "Model module is public"

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.AdvisoryPromotion.AdvisoryPromotionModule"))
