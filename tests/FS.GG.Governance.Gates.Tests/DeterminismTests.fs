...
let private sampleCommands =
    [ command "dotnet-test" 600; command "trivy" 120; command "schemathesis" 900 ]
    ...
    testList
        ...
        [
            test "assembling identical facts twice → structurally equal registry incl. order (SC-003, AS1)" {
                ...
                Expect.equal
                    (Gates.buildRegistry facts)
                    (Gates.buildRegistry facts)
                    "identical facts → identical registry"
            }
            ...
            <| (fun (cseed: int) (mseed: int) ->
                ...
                let permReg =
                    Gates.buildRegistry (factsOf (permute cseed sampleChecks) (permute mseed sampleCommands))
            ...
            test "the GateId order is unchanged when inputs are reversed (US5 AS2)" {
                ...
                let reversed =
                    Gates.buildRegistry (factsOf (List.rev sampleChecks) (List.rev sampleCommands))
