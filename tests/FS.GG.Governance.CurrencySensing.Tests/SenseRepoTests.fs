...
let private sha (s: string) =
    ...
    h.ComputeHash(Text.Encoding.UTF8.GetBytes s)
    |> Array.map (fun b -> b.ToString("x2"))
    |> String.concat ""

let private withTempRepo (body: string -> 'a) : 'a =
    let dir =
        Path.Combine(Path.GetTempPath(), "fsgg-cs-" + Guid.NewGuid().ToString("N"))
...
let private lockJson (sourceHashes: string list) (gen: string) =
    ...
    sprintf
        "{\"schemaVersion\":\"fsgg.refresh-lock/v1\",\"views\":{\"v\":{\"sources\":[%s],\"generatorVersion\":\"%s\",\"output\":\"x\"}}}"
        arr
        gen
...
let tests =
    testList
        ...
        [
            ...
            test "a missing provenance lock under a dial ⇒ Undeterminable finding, never a silent pass (FR-008)" {
                withTempRepo (fun dir ->
                    ...
                    | [ f ] ->
                        match f.Cause with
                        | Undeterminable _ ->
                            Expect.equal f.BaseSeverity Blocking "undeterminable still blocks when configured"
            ...
            test
                "a present-but-unparseable refresh.yml ⇒ one Blocking Undeterminable finding, never a silent pass (M-ADPT)" {
                withTempRepo (fun dir ->
                    ...
                    | [ f ] ->
                        ...
                        Expect.equal
                            f.Maturity
                            BlockOnPr
                            "fail-closed at the strictest dial (configured value unknowable)"
