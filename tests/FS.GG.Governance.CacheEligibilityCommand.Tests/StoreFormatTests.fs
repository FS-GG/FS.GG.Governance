...
let private sampleStore: ReuseStore =
    ...
    ReuseStore
        [
            ...
            {
                Inputs = inputs2
                Evidence = EvidenceRef "ev-2"
            }
        ]

let private withTempFile (content: string) (body: string -> 'a) : 'a =
    let path =
        Path.Combine(Path.GetTempPath(), "fsgg-store-" + Guid.NewGuid().ToString("N") + ".json")
        ...
        [
            test "a well-formed store round-trips to the expected ReuseStore (order preserved)" {
                ...
                withTempFile json (fun path ->
                    ...
                    | Ok(Some parsed) ->
                        Expect.equal
                            (EvidenceReuse.entries parsed)
                            (EvidenceReuse.entries sampleStore)
                            "round-trips, including absent command + sensed-empty covered set"
                    | other -> failtestf "expected Ok (Some store), got %A" other)
            }

            test "an absent store file ⇒ Ok None (FR-006)" {
                let missing =
                    Path.Combine(Path.GetTempPath(), "fsgg-store-absent-" + Guid.NewGuid().ToString("N") + ".json")
