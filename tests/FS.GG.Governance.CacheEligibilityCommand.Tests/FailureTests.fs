    ...
    testList
        ...
        [
            test "not-a-git-repo ⇒ InputUnavailable (exit 3), no artifact written" {
                let _, cap, model =
                    run validCatalog gitNotRepo fixedSensor (storeReaderOf (Ok None)) Set.empty
            ...
            test "declared catalog absent ⇒ InputUnavailable (exit 3), no artifact (C2: missing input)" {
                let _, cap, model =
                    run Map.empty git fixedSensor (storeReaderOf (Ok None)) Set.empty
            ...
            test "invalid catalog ⇒ ToolError (exit 4), no artifact, structured diagnostic" {
                let _, cap, model =
                    run invalidCatalog git fixedSensor (storeReaderOf (Ok None)) Set.empty
            ...
            test "malformed present store ⇒ ToolError (exit 4), no artifact" {
                let _, cap, model =
                    run validCatalog git fixedSensor (storeReaderOf (Error "unexpected end of JSON")) Set.empty
            ...
            test "unwritable output ⇒ ToolError (exit 4), NO partial artifact (FR-010)" {
                ...
                let _, cap, model =
                    run
                        ...
                        git
                        fixedSensor
                        (storeReaderOf (Ok None))
                        (Set.ofList [ req.CacheOut; req.UnresolvedOut ])
