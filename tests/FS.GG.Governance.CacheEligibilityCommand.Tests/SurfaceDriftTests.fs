...
let private commandAsm =
    SurfaceDrift.assemblyNamed "FS.GG.Governance.CacheEligibilityCommand"
    ...
    testList
        ...
        [
            ...
            test "the public API surface is exactly the Loop + Interpreter modules (plus the Exe entry)" {
                let typeNames =
                    commandAsm.GetExportedTypes() |> Array.choose (fun t -> Option.ofObj t.FullName)

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.Contains "FS.GG.Governance.CacheEligibilityCommand.LoopModule"))
                    "Loop module is public"

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.Contains "FS.GG.Governance.CacheEligibilityCommand.InterpreterModule"))
                ...
                Expect.isEmpty
                    unexpected
                    (sprintf "only Loop/Interpreter (+ Program entry) are public; found extra: %A" unexpected)
            }
