...
let private runDocs sensor store format =
    ...
    writtenOf cap Loop.UnresolvedArtifact |> Option.map snd,
    cap.Emits
...
let private cacheDocFor (gates: Gate list) =
    ...
    let _, effs =
        driveProjection (selectedModel gates (requestFor Loop.DefaultRange Loop.Human)) sensed EvidenceReuse.empty
