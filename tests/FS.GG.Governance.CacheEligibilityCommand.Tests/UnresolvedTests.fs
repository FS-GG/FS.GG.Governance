    ...
    testList
        ...
        [
            ...
            test "the unresolved gate is ABSENT from cache-eligibility.json (never reusable)" {
                ...
                Expect.isFalse
                    (cacheDoc.Contains "build:format")
                    "an unresolved gate never appears in cache-eligibility.json"
            }
            ...
            test "multiple missing facts are named in enum order" {
                ...
                | [ (_, facts) ] ->
                    Expect.equal
                        facts
                        [ "ruleHash"; "coveredArtifacts"; "baseRevision"; "headRevision" ]
                        "named in MissingFact enum order"
            ...
            test "the sidecar is ALWAYS written, empty when every gate resolves (A2)" {
                ...
                let sidecar =
                    sidecarOf (selectedModel gates req) (fullSensed gates) EvidenceReuse.empty
