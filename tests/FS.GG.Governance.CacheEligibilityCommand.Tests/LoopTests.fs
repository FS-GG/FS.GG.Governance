...
let private gateA =
    mkGate "build" "format" Cheap LocalOrCi (Some(CommandId "dotnet-format"))

let private gateB =
    mkGate "build" "tests" Medium LocalOrCi (Some(CommandId "dotnet-build"))
    ...
    testList
        ...
        [
            test "init: Since/DefaultRange emit SenseScope; ExplicitPaths emit LoadCatalog (no git)" {
                ...
                let _, expEff =
                    Loop.init (requestFor (Loop.ExplicitPaths [ gp "src/A.fs" ]) Loop.Human)
            ...
            test "empty store ⇒ every resolved gate mustRecompute NoPriorEvidence; cache doc = ofReport (L6/L7)" {
                ...
                for e in verdicts do
                    Expect.equal
                        (CacheEligibility.recomputeCause e.Verdict)
                        (Some NoPriorEvidence)
                        "noPriorEvidence with empty store"
            }
            ...
            test "a matching store ⇒ that gate reusable with its evidence ref (US1 AS1, SC-002)" {
                ...
                Expect.equal
                    (CacheEligibility.reusableEvidence aVerdict.Verdict)
                    (Some(EvidenceRef "ev-A"))
                    "gate A reusable with ev-A"

                let bVerdict = verdicts |> List.find (fun e -> e.Gate = gateB.Id)

                Expect.isFalse
                    (CacheEligibility.isReusable bVerdict.Verdict)
                    "gate B (no matching entry) must recompute"
            }
