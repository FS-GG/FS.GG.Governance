    ...
    testList
        ...
        [
            ...
            test "--out drives the sidecar stem, and --store/--format are honored" {
                let r =
                    parseOk [ "--out"; "out/ce.json"; "--store"; "s/store.json"; "--format"; "json" ]
            ...
            test "--format text is an additive synonym for human (CLI-3)" {
                Expect.equal
                    (parseOk [ "--format"; "text" ]).Format
                    Loop.Human
                    "text maps to the canonical Human format"
            ...
            test "--paths yields ExplicitPaths (normalized); --since yields Since" {
                ...
                | Loop.ExplicitPaths ps ->
                    Expect.equal ps [ normalizePath "src/A.fs"; normalizePath "src/B.fs" ] "explicit paths normalized"
