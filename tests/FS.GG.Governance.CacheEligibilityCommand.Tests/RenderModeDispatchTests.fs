...
let private runDispatch (capability: bool -> RenderMode.ColorCapability) (format) =
    ...
    let basePorts =
        fakePorts validCatalog git fixedSensor (storeReaderOf (Ok None)) cap0 req
    ...
    testList
        ...
        [
            ...
            test "--plain on a TTY ⇒ Plain even though the terminal is rich-capable (FR-012)" {
                ...
                let req =
                    { requestFor Loop.DefaultRange Loop.Human with
                        ExplicitPlain = true
                    }
            ...
            test "Json contract does NOT leak host output paths (CLI-4, FR-003)" {
                ...
                Expect.isFalse
                    (emitted.Contains "\"wrote\"")
                    "the `wrote` path object is dropped from the JSON contract"

                Expect.isFalse (emitted.Contains req.CacheOut) "the caller-supplied cache path never reaches the JSON"

                Expect.isFalse
                    (emitted.Contains req.UnresolvedOut)
                    "the caller-supplied unresolved path never reaches the JSON"
            }
