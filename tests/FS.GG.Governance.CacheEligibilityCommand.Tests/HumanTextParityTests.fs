...
let tests =
    testList
        ...
        [
            test
                "human summary contains the HumanText.ofCacheEligibilityReport projection of the resolved report, ANSI-free, with host `wrote` lines (US1)" {
                ...
                let req, cap, model =
                    runWith fixedSensor (storeReaderOf (Ok(Some store))) Loop.Human
                ...
                let report =
                    Expect.wantSome (cacheReportOf model) "both senses arrived ⇒ a recomputable report"
            ...
            test
                "all-must-recompute (empty store): the human summary is still the HumanText projection, ANSI-free (US1)" {
            ...
            test "JsonGolden: cache-eligibility.json is byte-identical to the genuine F041/F042 projection (SC-002)" {
                ...
                Expect.equal
                    (writtenOf cap2 Loop.CacheArtifact)
                    (writtenOf cap Loop.CacheArtifact)
                    "cache-eligibility.json byte-identical across runs"
            }
        ]
