    ...
    testList
        ...
        [
            test "empty findings ⇒ byte-identical to ofVerifyDecision (existing golden untouched)" {
                let withEmpty =
                    VerifyJson.ofVerifyDecisionWithSurfaceChecks richDecision (Some mixedReport) mixedOutcomes []
            ...
            test "non-empty findings ⇒ a surfaceChecks array with the documented element shape" {
                ...
                let json =
                    VerifyJson.ofVerifyDecisionWithSurfaceChecks emptyCleanDecision None [] findings
