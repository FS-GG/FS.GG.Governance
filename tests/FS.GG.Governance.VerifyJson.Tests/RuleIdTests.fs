...
let private surfaceFinding (code: string) : SC.SurfaceFinding =
    {
        ...
        Location =
            {
                File = normalizePath "src/Foo.fsi"
                Detail = "drift"
            }
        ...
        [
            test "every enforced item carries ruleId right after the id object (gate:/boundary:) (C2)" {
                ...
                for item in items do
                    ...
                    | "finding" ->
                        Expect.equal ruleId ("boundary:" + strField idObj "finding") "finding item → boundary:<token>"
            ...
            test "every surfaceChecks element carries ruleId (surface:<domain>:<code>) right after code (C2)" {
                let findings =
                    [
                        surfaceFinding "package.baseline-drift"
                        surfaceFinding "package.export-removed"
                    ]

                use doc =
                    parse (VerifyJson.ofVerifyDecisionWithSurfaceChecks emptyCleanDecision None [] findings)
                ...
                for e in elements do
                    ...
                    Expect.equal
                        (List.findIndex ((=) "ruleId") order)
                        (List.findIndex ((=) "code") order + 1)
                        "ruleId right after code"
            ...
            test
                "CrossSurfaceRuleId: a finding in both verify.json and audit.json carries an identical ruleId (T013, FR-006, SC-005)" {
