    ...
    testList
        ...
        [
            test "a Reusable candidate becomes Reuse and is NOT in recomputeGates (charges nothing)" {
                ...
                let report =
                    Budget.decide
                        (Budget.budgetFor Profile.Release RunMode.Release)
                        RunMode.Release
                        [ cc g Exhaustive verdict ]
            ...
            test "a NoPriorEvidence candidate ⇒ Recompute with the NoPriorEvidence cause — never a fabricated reuse" {
                ...
                Expect.equal
                    ...
                    "real F041 verdict is NoPriorEvidence against an empty store"
            ...
            test
                "a MustRecompute gate whose cost exceeds the ceiling ⇒ OverBudget — not silently reused, not silently recomputed" {
                let g = gid "build" "tests"

                let verdict =
                    verdictFor (baseInputs |> fun i -> { i with RuleHash = RuleHash "r2" }) baseStore
                // Inner ⇒ Cheap ceiling; an Exhaustive cost is over budget.
                let report =
                    Budget.decide (Budget.budgetFor Light Inner) Inner [ cc g Exhaustive verdict ]
