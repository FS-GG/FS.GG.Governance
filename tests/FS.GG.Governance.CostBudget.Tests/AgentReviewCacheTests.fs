    ...
    testList
        ...
        [
            test "matching F036 identity ⇒ Reusable ⇒ Reuse (reused on matching judge/prompt/check-artifact)" {
                ...
                let report =
                    Budget.decide
                        ...
                        RunMode.Release
                        [ ccReviewed (gid "ai" "review") Cheap verdict reviewKey ]
            ...
            test "one F036 identity changed ⇒ no match ⇒ Recompute (re-review)" {
                Expect.isFalse
                    (AgentReviewKey.matches reviewInputsBase reviewInputsChanged)
                    "a changed model version no longer matches"
                ...
                let report =
                    Budget.decide
                        (Budget.budgetFor Profile.Release RunMode.Release)
                        RunMode.Release
                        [ ccReviewed (gid "ai" "review") Cheap verdict reviewKeyChanged ]
            ...
            test "decide's budget arithmetic for an AgentReviewed candidate is IDENTICAL to the same Deterministic one" {
                for verdict in
                    [
                        Reusable refA
                        MustRecompute(InputsChanged [ RuleHashCat ])
                        MustRecompute NoPriorEvidence
                    ] do
                    for cost in costs do
                        for m in modes do
                            ...
                            Expect.equal
                                revDecision
                                detDecision
                                (sprintf "decision identical for %A cost %A mode %A" verdict cost m)
            }
        ]
