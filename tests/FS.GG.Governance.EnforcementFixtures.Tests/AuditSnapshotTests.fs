...
let private relPath (s: Scenario) : string =
    "audit-snapshots/" + s.Name + ".audit.json"
    ...
    testList
        ...
        [
            testList
                ...
                [
                    for s in scenarios -> test s.Name { blessOrCompare (relPath s) (snapshotFor s) }
                ]
                ...
                [
                    for s in scenarios ->
                        test s.Name {
                            ...
                            let total =
                                sectionCount doc "blockers"
                                + sectionCount doc "warnings"
                                + sectionCount doc "passing"
            ...
            test
                "no-hide — every relaxed-blocker warning carries both base and effective severity + reason (FR-009, SC-004)" {
                for s in scenarios do
                    if s.ExpectedSection = "warnings" then
                        ...
                        Expect.equal
                            baseSeverity
                            "blocking"
                            (sprintf "%s: a relaxed blocker carries base severity 'blocking'" s.Name)

                        Expect.equal
                            effectiveSeverity
                            "advisory"
                            (sprintf "%s: a relaxed blocker shows effective severity 'advisory'" s.Name)

                        Expect.notEqual
                            baseSeverity
                            effectiveSeverity
                            (sprintf "%s: base and effective severity must differ (no-hide)" s.Name)

                        Expect.isFalse
                            (System.String.IsNullOrWhiteSpace reason)
                            (sprintf "%s: the reason must be non-empty" s.Name)
            }
