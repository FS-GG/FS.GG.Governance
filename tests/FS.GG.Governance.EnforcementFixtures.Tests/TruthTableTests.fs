...
let private dataRows (table: string) : string list list =
    ...
    |> List.map (fun line ->
        ...
        inner.Split('|') |> Array.map (fun c -> c.Trim()) |> Array.toList)
    ...
    testList
        ...
        [
            test "completeness — exactly 240 rows and 240 distinct (base,maturity,mode,profile) keys (SC-001)" {
                ...
                let keys = rows |> List.map (fun r -> r[0], r[1], r[2], r[3]) |> Set.ofList

                Expect.equal
                    (Set.count keys)
                    240
                    "every (base,maturity,mode,profile) combination must appear exactly once — no missing, no duplicate"
            }
            ...
            test "determinism — regenerating twice yields identical bytes (SC-002, FR-004)" {
                Expect.equal
                    (renderTruthTable ())
                    (renderTruthTable ())
                    "generation must be a pure function of the cores — no clock/host/order influence"
            }
