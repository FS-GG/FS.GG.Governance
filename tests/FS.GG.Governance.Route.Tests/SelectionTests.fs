    ...
    testList
        ...
        [
            ...
            test "UnmatchedInRoot and OutOfScope paths select NO gate — no fallback (FR-003)" {
                ...
                let r =
                    FS.GG.Governance.Route.Route.select
                        (registryOf fixtureFacts)
                        report
                        (findingsOf fixtureFacts report)
            ...
            test "selection joins on declared Gate.Domain by id equality, not a re-parsed GateId (FR-010)" {
                ...
                let f =
                    facts
                        ...
                        []
                        [ check "build:extra" "weird" None Cheap; check "build" "tests" None Cheap ]
                        []
            ...
            test "an AmbiguousRoute diagnostic on a Routed path still selects its resolved domain (D7)" {
                ...
                let f =
                    facts
                        "src"
                        [ "src/app/*.fs", "build"; "src/*/Main.fs", "docs" ]
                        []
                        [ check "build" "tests" None Cheap; check "docs" "lint" None Cheap ]
                        []
                ...
                let r =
                    FS.GG.Governance.Route.Route.select (registryOf f) report (findingsOf f report)
