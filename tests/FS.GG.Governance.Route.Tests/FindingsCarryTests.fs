...
let private fixtureFacts =
    facts "src" [ "src/build/**", "build" ] [] [ check "build" "tests" None Cheap ] []
    ...
    testList
        ...
        [
            test "a non-empty F017 report is carried UNCHANGED alongside the selected gates (AS1, SC-003)" {
                ...
                let r =
                    FS.GG.Governance.Route.Route.select (registryOf fixtureFacts) report findings
            ...
            test "a routed gate and a finding-bearing UnmatchedInRoot path coexist (AS3)" {
                ...
                let selectingPaths =
                    r.SelectedGates
                    |> List.collect (fun sg -> sg.SelectingPaths |> List.map (fun p -> p.Path))
