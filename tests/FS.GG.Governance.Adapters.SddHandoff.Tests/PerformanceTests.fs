    ...
    testList
        ...
        [
            test "Synthetic M5/M6-like raw samples pass after independent nearest-rank recomputation" {
                ...
                let value =
                    evidence
                        ...
                        [ sample "headless" "linux-ci" durations (List.replicate 100 0) ]
                        (Some true)
                        []
            ...
            test "Synthetic headless samples cannot satisfy live compositor intent" {
                ...
                Expect.exists
                    result.Failures
                    (fun f -> f.Contains "live-compositor")
                    "limitation names the missing lane"
            }
            ...
            test "Synthetic readback-contaminated live proof is rejected" {
                ...
                let result =
                    evaluate (evidence (Some(intent "active" true)) [ contaminated ] None [])
            ...
            test "Synthetic not-applicable intent produces no applicable gate result" {
                let value =
                    evidence
                        (Some
                            { intent "not-applicable" false with
                                Rationale = Some "no render loop"
                            })
                        []
                        None
                        []
            ...
            test "Synthetic self-attested artifact without typed intent is rejected" {
                let value =
                    evidence None [ sample "headless" "linux-ci" [ 10m ] [ 0 ] ] (Some true) []
                ...
                Expect.exists
                    result.Failures
                    (fun f -> f.Contains "typed performance intent")
                    "missing authority is named"
            }
            ...
            test "Synthetic definition digests require exactly 64 lowercase hexadecimal digits" {
                ...
                for digest in malformed do
                    ...
                    let value =
                        evidence (Some declaration) [ sample "headless" "linux-ci" [ 10m ] [ 0 ] ] None []
            ...
            test "Synthetic duplicate declared workload ids are rejected" {
                ...
                Expect.exists
                    result.Failures
                    (fun f -> f.Contains "must not contain duplicates")
                    "duplicate is explicit"
            }

            test "Synthetic duration and catch-up arrays require paired sample cardinality" {
                let value =
                    evidence (Some(intent "active" false)) [ sample "headless" "linux-ci" [ 10m; 11m ] [ 0 ] ] None []
                ...
                Expect.exists
                    result.Failures
                    (fun f -> f.Contains "equal per-sample cardinality")
                    "pairing failure is explicit"
            }
        ]
