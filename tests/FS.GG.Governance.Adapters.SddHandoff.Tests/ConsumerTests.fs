    ...
    testList
        ...
        [
            ...
            test "a bad document ⇒ a blocking integrity gate + diagnostic and NO mapped evidence gate (FR-011)" {
                ...
                Expect.isFalse
                    (ids |> List.exists (fun id -> id.Contains "evidence"))
                    "no mapped evidence gate for a bad document"
            ...
            test "multiple handoffs load in <id> order with gates sorted by GateId (FR-012, research D7)" {
                ...
                let a =
                    { Fixtures.read "satisfied" with
                        Source = "readiness/aaa/governance-handoff.json"
                    }

                let b =
                    { Fixtures.read "failing" with
                        Source = "readiness/bbb/governance-handoff.json"
                    }
            ...
            test "governedReferences absent vs present does not change correctness (FR-010)" {
                ...
                let withoutRefs =
                    Consumer.consume
                        [
                            {
                                Source = "readiness/satisfied/governance-handoff.json"
                                Json = strippedJson
                            }
                        ]
            ...
            test "a synthetic-tainted handoff ⇒ a blocking evidence gate (taint closure, research D4)" {
                ...
                let r =
                    Consumer.consume
                        [
                            {
                                Source = "readiness/taint/governance-handoff.json"
                                Json = json
                            }
                        ]

                Expect.equal
                    (maturityOf r "evidence")
                    (Some BlockOnShip)
                    "a Real node resting on Synthetic taints AutoSynthetic ⇒ blocking"
            }
...
let private mk (id: string) (json: string) : Reader.HandoffRead =
    {
        Source = sprintf "readiness/%s/governance-handoff.json" id
        Json = json
    }
    ...
    testList
        ...
        [
            ...
            test "C3 — declared src/A/x, tests/A/y ⇒ normalized + sorted" {
                let doc =
                    mk "wi" (docDeclaring """[ { "workItem": "WI-1", "paths": [ "src/A/x", "tests/A/y" ] } ]""")
            ...
            test "C4 — two consumable docs with overlapping paths ⇒ union, de-duplicated" {
                let a =
                    mk "aaa" (docDeclaring """[ { "workItem": "WI-1", "paths": [ "src/A/x", "tests/A/y" ] } ]""")

                let b =
                    mk "bbb" (docDeclaring """[ { "workItem": "WI-2", "paths": [ "src/A/x", "src/B/z" ] } ]""")
            ...
            test "C5 — a consumable + a malformed doc ⇒ only the consumable's paths (FR-008)" {
                let good =
                    mk "good" (docDeclaring """[ { "workItem": "WI-1", "paths": [ "src/A/x" ] } ]""")
            ...
            test "C6 — a single version-mismatch doc ⇒ [] (FR-008)" {
                Expect.isEmpty
                    (Consumer.candidatePaths [ Fixtures.read "v2-major" ])
                    "an unsupported major contributes no candidates"
            }
            ...
            test "C7 — the same path declared twice across work items ⇒ one entry" {
                let doc =
                    mk
                        "wi"
                        (docDeclaring
                            """[ { "workItem": "WI-1", "paths": [ "src/A/x" ] }, { "workItem": "WI-2", "paths": [ "src/A/x" ] } ]""")

                Expect.equal
                    (Consumer.candidatePaths [ doc ])
                    [ GovernedPath "src/A/x" ]
                    "a path declared twice survives once"
            }

            test "C8 — a back-slash raw path ⇒ normalized via Reader.parse" {
                let doc =
                    mk "wi" (docDeclaring """[ { "workItem": "WI-1", "paths": [ "src\\A\\x" ] } ]""")

                Expect.equal
                    (Consumer.candidatePaths [ doc ])
                    [ GovernedPath "src/A/x" ]
                    "back-slashes normalize to forward-slashes"
            }
        ]
