        ...
        [
            ...
            test "ADR-0002 row: a stale node carries its underlying state PLUS a StaleEvidence diagnostic (FR-006)" {
                ...
                | Ok(ns, _), diags ->
                    ...
                    Expect.equal
                        (diags |> List.map (fun d -> d.Cause))
                        [ StaleEvidence ]
                        "exactly one StaleEvidence diagnostic"
            ...
            test
                "ADR-0002 row: Evidence.effective taints a Real node resting on Synthetic → AutoSynthetic (blocking-capable, research D4)" {
                ...
                | Ok states ->
                    ...
                    Expect.equal
                        states.["test:unit"]
                        EvidenceState.AutoSynthetic
                        "the dependent Real node is tainted AutoSynthetic"
            ...
            test "ADR-0002 row: a Failed effective state survives the closure (blocking-capable)" {
                ...
                match r with
                | Ok states ->
                    Expect.equal states.["test:unit"] EvidenceState.Failed "Failed is carried into the effective map"
