        ...
        [
            ...
            test "a direct exe→exe edge is flagged" {
                ...
                Expect.isTrue
                    (violations
                     |> List.exists (fun v ->
                         v.Detail.Contains "FS.GG.Governance.Cli"
                         && v.Detail.Contains "FS.GG.Governance.RouteCommand"))
            ...
            test "a transitive exe→exe edge through a library is flagged" {
                ...
                Expect.isNonEmpty (exeExeEdges graph) "a transitive exe→exe reach through a library must be caught"
            }
        ]
