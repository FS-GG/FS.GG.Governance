    ...
    testList
        ...
        [
            ...
            test "the duplicate built-in adapter seam stays consolidated" {
                ...
                let builtIn =
                    nodes |> List.find (fun n -> n.Name = "FS.GG.Governance.Adapters.BuiltIn")
            ...
            test "the generated classified graph equals the committed artifact" {
                let artifactPath =
                    Path.Combine(repoRoot, "tests", "FS.GG.Governance.DependencyFences.Tests", "dependency-graph.dot")
                ...
                Expect.equal
                    actual
                    expected
                    "regenerate the classified graph intentionally with BLESS_DEPENDENCY_GRAPH=1"
            }
        ]
