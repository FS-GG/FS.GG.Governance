...
let private genArtifact =
    gen {
        let! k = Gen.elements [ "token"; "essay" ]
...
let private propConfig =
    ...
    } // fixed seed → reproducible (cf. F01/F02)
    ...
    testList
        ...
        [
            ...
            test "V1 eval: atom outcomes map; All/Any/Not follow Kleene; order-independent (US1 AS1-4, INV-2/11)" {
                ...
                Expect.equal
                    (Check.eval noFacts (atomUnknown "p" "tone?"))
                    (Uncertain "tone?")
                    "Unknown r → Uncertain r"

                // All: Fail dominates; else Uncertain survives; else Pass.
                Expect.isTrue
                    (isFail (Check.eval noFacts (All [ atomMet "a"; atomUnknown "b" "?"; atomUnmet "c" "x" ])))
                    "a Fail dominates All"

                Expect.isTrue
                    (isUncertain (Check.eval noFacts (All [ atomMet "a"; atomUnknown "b" "?"; atomMet "c" ])))
                    "Uncertain survives All of passes"
                ...
                Expect.isTrue
                    (isPass (Check.eval noFacts (Any [ atomUnmet "a" "x"; atomMet "b" ])))
                    "a Pass dominates Any"

                Expect.isTrue
                    (isUncertain (Check.eval noFacts (Any [ atomUnmet "a" "x"; atomUnknown "b" "?" ])))
                    "Uncertain survives Any of fails"

                Expect.isTrue
                    (isFail (Check.eval noFacts (Any [ atomUnmet "a" "x"; atomUnmet "b" "y" ])))
                    "all fail → Fail"
                ...
                for seed in [ 1; 7; 42; 99 ] do
                    Expect.equal
                        (Check.eval noFacts (All xs))
                        (Check.eval noFacts (All(shuffle seed xs)))
                        "All eval order-independent (verdict+reason)"

                    Expect.equal
                        (Check.eval noFacts (Any xs))
                        (Check.eval noFacts (Any(shuffle seed xs)))
                        "Any eval order-independent (verdict+reason)"
            }
            ...
            test "V2 eval: (a ==> b) = Any [Not a; b] for representative a,b (US1 AS5, FR-006)" {
                ...
                for a, b in cases do
                    Expect.equal
                        (Check.eval noFacts (a ==> b))
                        (Check.eval noFacts (Any [ Not a; b ]))
                        "implication desugars to Any [Not a; b]"
            }
            ...
            test "V3 eval: Opaque maps its function's outcome (US1 AS6, FR-006)" {
                ...
                Expect.equal
                    (Check.eval noFacts (Opaque("o", fun _ -> Unknown "dunno")))
                    (Uncertain "dunno")
                    "opaque Unknown → Uncertain"
            }

            // ── User Story 2: inspect without running — render and hash ──

            test
                "V4 never-executes: a throwing-Eval probe still renders/hashes/reads/isReified; only eval throws (US2 AS1, SC-001, INV-1)" {
                let boom =
                    Check.probe "boom" [ { Kind = "token"; Key = "t" } ] [ NumberArg 4.5 ] (fun (_: FactSet<string>) ->
                        failwith "executed")
                ...
                Expect.equal
                    (Check.reads composed)
                    [
                        { Kind = "token"; Key = "t" }
                        { Kind = "token"; Key = "t" }
                        { Kind = "token"; Key = "t" }
                    ]
                    "reads succeeds (no Eval): boom, Not boom, boom in the implication"
            ...
            test "V5 render: deterministic, execution-free, authoring order preserved (US2 AS1, FR-007)" {
                ...
                Expect.equal
                    (Check.render chk)
                    "all of [contrastRatio(token:text, 4.5); toneIsProfessional]"
                    "render uses structure only, in authoring order"
                ...
                Expect.notEqual
                    (Check.render (All [ contrast; tone ]))
                    (Check.render (All [ tone; contrast ]))
                    "render preserves authoring order"
            }

            testPropertyWithConfig
                propConfig
                "V6 hash: permuting members of All/Any is hash-invariant (US2 AS3, SC-002, INV-3)"
            ...
            test "V6b hash: duplicate members in a commutative node hash deterministically (edge case, INV-3)" {
                ...
                Expect.equal
                    (Check.hash (All [ a; a; b ]))
                    (Check.hash (All [ b; a; a ]))
                    "duplicates: position-independent"

                Expect.equal
                    (Check.hash (Any [ a; b; a ]))
                    (Check.hash (Any [ a; a; b ]))
                    "duplicates: position-independent (Any)"
            }
            ...
            test "V7 hash: positional structure changes the key (US2 AS4/AS5, SC-002, INV-4)" {
                ...
                let p1 =
                    Check.probe "p" [] [ LiteralArg "x"; NumberArg 1.0 ] (fun (_: FactSet<string>) -> Met)

                let p2 =
                    Check.probe "p" [] [ NumberArg 1.0; LiteralArg "x" ] (fun (_: FactSet<string>) -> Met)
            ...
            test "V8 hash: re-hash identical ⇒ identical; Opaque hashes by name only (US2 AS2/AS6, INV-5)" {
                ...
                let o2 =
                    Opaque("judge", fun (_: FactSet<string>) -> Unmet "different function entirely")
            ...
            testPropertyWithConfig
                propConfig
                "V9 explain: Explanation.verdict (explain f c) = eval f c (US3 AS1, SC-004, INV-6)"
            <| fun (c: Check<string>) -> Explanation.verdict (Check.explain noFacts c) = Check.eval noFacts c
            ...
            test "V10 explain: structure mirrors the check and records each atom's outcome (US3 AS2, INV-7)" {
                let chk =
                    All
                        [
                            atomMet "a"
                            Any [ atomUnmet "b" "x"; atomUnknown "c" "?" ]
                            Not(atomMet "d")
                        ]

                match Check.explain noFacts chk with
                | AllExplained([ AtomExplained("a", Met, _)
                                 AnyExplained([ AtomExplained("b", Unmet "x", _); AtomExplained("c", Unknown "?", _) ],
                                              _)
                                 NotExplained(AtomExplained("d", Met, _), _) ],
                               rootV) ->
            ...
            test
                "V11 isReified false iff Opaque present; reads = exactly declared refs, Opaque adds none (US4 AS1-3, SC-005, INV-8/9)" {
                let r1 = { Kind = "token"; Key = "text" }
                let r2 = { Kind = "essay"; Key = "intro" }

                let structural =
                    All [ mkProbe "a" [ r1 ] [] Met; Not(mkProbe "b" [ r2 ] [] (Unmet "x")) ]
            ...
            test
                "V12 totality: empty All/Any and combinator mixes fold through all six interpreters (FR-013, SC-006, INV-10)" {
