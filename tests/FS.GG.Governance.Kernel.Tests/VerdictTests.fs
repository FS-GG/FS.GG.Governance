    ...
    let genReason =
        Gen.oneof
            [
                ...
                |> Gen.map (fun s -> if System.String.IsNullOrEmpty s then "" else s)
...
let private propConfig =
    ...
    } // fixed seed → reproducible (cf. F01 V7)
    ...
    testList
        ...
        [
            ...
            testPropertyWithConfig
                propConfig
                "V6 permuting the input list ⇒ identical verdict and reason (FR-005/006, SC-001)"
            ...
            test "V8 duplicate/shuffled reasons ⇒ dedup'd, ordinal-sorted, position-independent reason (US2 AS4)" {
                ...
                Expect.equal
                    (Verdict.all [ Fail "a"; Fail "z" ])
                    (Verdict.all [ Fail "z"; Fail "a" ])
                    "reorder ⇒ identical Fail"

                Expect.equal
                    (Verdict.all [ Fail "a"; Fail "z" ])
                    (Fail "a; z")
                    "reasons ordinal-sorted and joined on \"; \""
                ...
                Expect.equal
                    (Verdict.all [ Fail "z"; Fail "a"; Fail "z"; Fail "a" ])
                    (Fail "a; z")
                    "duplicates collapse"
            }
            ...
            test "V8b \"; \" is RESERVED inside a single leaf reason — it is split and reordered (CORE-3)" {
                ...
                Expect.equal
                    (Verdict.all [ Fail "spacing; tone" ])
                    (Fail "spacing; tone")
                    "already-ordinal components pass through unchanged"

                Expect.equal
                    (Verdict.all [ Fail "tone; spacing" ])
                    (Fail "spacing; tone")
                    "a leaf reason's \"; \"-parts are split and reordered, not preserved verbatim"

                Expect.equal
                    (Verdict.any [ Uncertain "z; a; z" ])
                    (Uncertain "a; z")
                    "duplicate components within one leaf reason collapse too"
                ...
                Expect.equal
                    (Verdict.negate (Uncertain "tone; spacing"))
                    (Uncertain "tone; spacing")
                    "an un-aggregated reason is carried verbatim, unsplit"
            }
            ...
            test "V9 negate swaps pass/fail tags, fixes Uncertain; double-negate recovers tags (US3)" {
                ...
                Expect.equal
                    (Verdict.negate (Verdict.negate (Uncertain "y")))
                    (Uncertain "y")
                    "Uncertain recovers exactly"
                ...
                Expect.isTrue
                    (isFail (Verdict.negate (Verdict.negate (Fail "x"))))
                    "reasoned Fail recovers its TAG (reason dropped at the flip)"
            }
