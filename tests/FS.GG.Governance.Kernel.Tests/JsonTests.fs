...
let private unmet name r : Check<string> =
    Check.probe name [] [] (fun _ -> Unmet r)
...
let rec private checkNode (el: JsonElement) =
    ...
    | "all"
    | "any" ->
        for p in el.GetProperty("parts").EnumerateArray() do
            checkNode p
    ...
    testList
        ...
        [
            ...
            test "V32 ofExplanation is byte-for-byte deterministic" {
                ...
                Expect.equal
                    (Json.ofExplanation expl)
                    (Json.ofExplanation expl)
                    "same explanation ⇒ identical JSON (SC-002)"
            }
            ...
            test "V34 serialization runs no probe; OpaqueExplained emits name + recorded outcome only" {
                ...
                Expect.equal
                    (Json.toExplanation (Json.ofExplanation boom))
                    boom
                    "opaque-by-data serializes with no probe to run"
            }
            ...
            test "V39 six distinct evidence-state tokens round-trip; effective map round-trips" {
                ...
                Expect.equal
                    (Json.ofEvidenceState AutoSynthetic)
                    "\"autoSynthetic\""
                    "AutoSynthetic gets its own visible token"
                ...
                Expect.equal
                    (Json.ofEffective id eff)
                    (Json.ofEffective id eff)
                    "ofEffective is byte-for-byte deterministic (SC-002)"
            }
            ...
            test "V40 the four readers throw fast on malformed input, per the documented contract" {
                ...
                let badKind =
                    captureThrow (fun () -> Json.toExplanation "{\"kind\":\"bogus\"}" |> ignore)
                ...
                Expect.stringStarts
                    badState.Message
                    "Json:"
                    "an unknown evidence-state token ⇒ a 'Json:'-tagged failure"
            }
        ]
