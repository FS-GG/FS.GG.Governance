...
let private blockingRule id s c =
    CheckRule.rule (RuleId id) Deterministic s c
...
let private propConfig =
    { FsCheckConfig.defaultConfig with
        maxTest = 300
        replay = Some(4242UL, 2425UL, None)
    } // fixed seed → reproducible (cf. F02 V6)
...
let tests =
    testList
        ...
        [
            ...
            test
                "V40 light by default: no matching fence (and empty fence set) ⇒ Routine + empty Blocking in every mode (FR-006, SC-001)" {
                ...
                let rule =
                    blockingRule "peer-review" (spec "constitution.md" "I") (chk "peer-reviewed")
            ...
            test "V41 single fence trips ⇒ Fenced carrying its name (FR-004, SC-002)" {
                Expect.equal
                    (Route.stakesOf fences srcChange)
                    (Fenced "merge-boundary")
                    "one matching fence ⇒ Fenced with its name"
            }

            test
                "V42 fenced gate explained: non-empty Blocking; renderRoute names rule + fence + rendered check (FR-012, SC-006/010)" {
                let rule =
                    blockingRule "peer-review" (spec "constitution.md" "I") (chk "peer-reviewed")
                ...
                Expect.stringContains
                    rendered
                    (Check.render rule.Check)
                    "render contains the rendered check text (no drift)"
            }

            test
                "V46 drift-proof gate: gate.Statement = Check.render rule.Check; route + render run NO probe/review (FR-012, SC-006/010)" {
                ...
                Expect.equal
                    gate.Statement
                    (Check.render rule.Check)
                    "gate Statement is byte-for-byte Check.render (drift-proof, SC-006)"
            ...
            test
                "V44 run-mode matrix: advisory in Sandbox/Inner, blocking only in Gate; stakes identical across modes (FR-008/009, SC-004)" {
                let rule =
                    blockingRule "peer-review" (spec "constitution.md" "I") (chk "peer-reviewed")

                let routes =
                    allModes |> List.map (fun m -> m, Route.route fences [ rule ] m srcChange)

                for mode, r in routes do
                    Expect.equal
                        r.Stakes
                        (Fenced "merge-boundary")
                        (sprintf "stakes identical (Fenced) in %A — mode changes enforcement, not classification" mode)
                    ...
                    | Inner ->
                        ...
                        Expect.equal
                            (List.length r.Advisory)
                            1
                            (sprintf "the requirement surfaces as advisory in %A" mode)
            ...
            testPropertyWithConfig
                propConfig
                "V43 order-independent: permuting fences ⇒ identical Stakes, Route, and render (FR-005, SC-003)"
            <| fun (paths: string list) (mode: RunMode) ->
                ...
                |> List.forall (fun p ->
                    ...
                    && Route.renderRoute (Route.route p [ rule ] mode change) =
                        Route.renderRoute (Route.route fences [ rule ] mode change))

            test
                "V43b multi-match ⇒ Fenced carrying deduped, ordinal-sorted, \"; \"-joined names; order-independent (FR-005, SC-002)" {
                ...
                Expect.equal
                    (Route.stakesOf (List.rev fences) bothChange)
                    expected
                    "reordering fences ⇒ identical Fenced name"
            ...
            test
                "V45 reason mandatory: every route (fenced-at-gate, advisory, routine over empty sets) has a non-empty Reason (FR-011, SC-005)" {
                let rule =
                    blockingRule "peer-review" (spec "constitution.md" "I") (chk "peer-reviewed")
                ...
                Expect.isTrue
                    (nonEmpty (Route.renderRoute routine))
                    "renderRoute of a bare routine route is still a non-empty block"
            }

            test
                "V47 short, filterable, deterministic: Blocking = exactly the blocking gates, bounded by applicable rules; render deterministic & execution-free; total over empty rules (FR-013/014/015, SC-007/008/009/010)" {
                let executed = ref false
                ...
                Expect.equal
                    (r.Blocking |> List.map (fun e -> e.Id))
                    [ RuleId "b1"; RuleId "b2" ]
                    "Blocking is exactly the blocking-severity entries, in catalog order"

                Expect.equal (r.Advisory |> List.map (fun e -> e.Id)) [ RuleId "a1" ] "Advisory is exactly the rest"

                Expect.isTrue
                    (r.Blocking |> List.forall (fun e -> e.Severity = Blocking))
                    "every Blocking entry has Blocking severity"
                ...
                Expect.equal
                    (List.length union)
                    (List.length rules)
                    "the partition is bounded by the applicable rules (FR-013, SC-007)"
                ...
                Expect.equal
                    bare.Stakes
                    (Fenced "merge-boundary")
                    "stakes still classified for a high-stakes change with no applicable rule"

                Expect.isTrue
                    (nonEmpty bare.Reason)
                    "a bare fenced route still carries a reason — the partition is total"
            }
        ]
