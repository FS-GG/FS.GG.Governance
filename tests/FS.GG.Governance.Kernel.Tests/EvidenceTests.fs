    ...
    testList
        ...
        [
            ...
            test "V21 synthetic root taints its real chain; a no-synthetic graph is untouched" {
                ...
                let declared = [ "a", Real; "b", Real; "c", Pending; "d", Failed; "e", Skipped ]
            ...
            test "V23 re-declaring the synthetic root Real clears the taint; selective with two roots" {
                ...
                let cleared =
                    eff [ "data", Real; "analysis", Real; "report", Real ] [ "analysis", "data"; "report", "analysis" ]
                ...
                let edges2 =
                    [ "onlyA", "rootA"; "onlyB", "rootB"; "both", "rootA"; "both", "rootB" ]
                ...
                let g =
                    Evidence.build (nodes2 Real) edges2
                    |> function
                        | Ok x -> x
            ...
            test "V27 non-real states are inert; declared synthetic outranks inherited taint; domain-neutral" {
                ...
                Expect.equal
                    (stateOf m2 "s2")
                    Synthetic
                    "declared Synthetic reported verbatim — root cause outranks inheritance"

                // (c) domain-neutral: a research finding resting on simulated data is AutoSynthetic (US4 AS3, INV-12).
                let m3 =
                    eff [ "simulated-data", Synthetic; "finding", Real ] [ "finding", "simulated-data" ]
