...
let chainRule (ruleId: string) (inputs: string list) (output: string) : Rule<string> =
    {
        ...
        Apply =
            fun facts ->
                let present name =
                    facts |> List.exists (fun f -> f.Value = name)
...
let private fixedRules =
    [
        ...
        chainRule "r4" [ "D" ] "E"
        chainRule "r5" [ "A" ] "D"
    ] // D reachable by two chains (r3 and r5)
        ...
        [
            ...
            test "V1 chained rules produce exact transitive closure" {
                ...
                let r =
                    FixedPoint.evaluate
                        identify
                        [ chainRule "r1" [ "A" ] "B"; chainRule "r2" [ "B" ] "C" ]
                        (supplied [ "A" ])

                Expect.equal
                    (factIds r)
                    [ FactId "A"; FactId "B"; FactId "C" ]
                    "supplied + closure, no spurious/missing"
            }

            test "V2 unmet preconditions and the no-rules case derive nothing" {
                let unmet =
                    FixedPoint.evaluate identify [ chainRule "r" [ "X" ] "Y" ] (supplied [ "A" ])
                ...
                Expect.equal
                    (factIds noRules)
                    [ FactId "A"; FactId "B" ]
                    "no rules ⇒ exactly the supplied facts (canonical order)"
            ...
            test "V5 an asserted fact carries empty provenance" {
                let r =
                    FixedPoint.evaluate identify [ chainRule "r1" [ "A" ] "B" ] (supplied [ "A" ])
            ...
            testPropertyWithConfig propConfig "V7 result is invariant under rule-order permutation"
            <| fun (seed: int) ->
            ...
            test "V8 facts sharing an identify id collapse to one entry" {
                ...
                let count id =
                    r.Facts |> List.filter (fun f -> f.Id = id) |> List.length
            ...
            test "V9 repeated evaluation is byte-for-byte identical" {
                let run () =
                    FixedPoint.evaluate identify fixedRules fixedSupplied
