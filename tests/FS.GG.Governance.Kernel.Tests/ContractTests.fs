        ...
        [
            test "V35 ofRules: one entry per rule (catalog order), Statement = Check.render, drift-proof" {
                ...
                Expect.notEqual
                    mutated.[0].Statement
                    eA.Statement
                    "changing the check changes the Statement (drift-proof, SC-005)"
            ...
            test "V36 ofRules total over the empty catalog; deterministic; contract JSON round-trips" {
                Expect.equal
                    (Contract.ofRules ([]: CheckRule<string> list))
                    []
                    "empty catalog ⇒ empty contract (FR-007, SC-006)"
