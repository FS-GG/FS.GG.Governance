...
let private mk name reads outcome : Check<Gov> =
    Check.probe name reads [] (fun _ -> outcome)
...
let private propConfig =
    { FsCheckConfig.defaultConfig with
        ...
        replay = Some(1234UL, 5679UL, None)
    } // fixed seed → reproducible (cf. F01/F02/F03)
...
let tests =
    testList
        ...
        [
            ...
            test
                "V13 rule: Deterministic refuses an opaque check iff non-reified; every other tier accepts (US3 AS1-3, FR-006, SC-001, INV-1)" {
                let id = RuleId "judge"
                ...
                Expect.isTrue
                    (refusedWith (CheckRule.rule id Deterministic spec opaqueCheck))
                    "Deterministic over an Opaque check is refused with the rule id"
                // A nested Opaque is refused too (isReified walks the whole structure).
                Expect.isTrue
                    (refusedWith (CheckRule.rule id Deterministic spec (All [ metCheck; opaqueCheck ])))
                    "Deterministic over a check containing an Opaque anywhere is refused"
                // The same opaque check at AgentReviewed / HumanOnly is accepted.
                Expect.isTrue
                    (accepted (CheckRule.rule id AgentReviewed spec opaqueCheck))
                    "AgentReviewed accepts an opaque check"

                Expect.isTrue
                    (accepted (CheckRule.rule id HumanOnly spec opaqueCheck))
                    "HumanOnly accepts an opaque check"
                // A fully reified check at Deterministic is accepted.
                Expect.isTrue
                    (accepted (CheckRule.rule id Deterministic spec metCheck))
                    "Deterministic accepts a reified check"
            }
            ...
            testPropertyWithConfig
                propConfig
                "V14 cacheKey: identical ingredients ⇒ identical key; any one ingredient changing ⇒ a different key (US2 AS1/2, FR-011/012, SC-002, INV-2)"
            <| fun (modelId: string) (version: string) (checkHash: string) (arts: string list) (prompt: string) ->
                ...
                let diffModel =
                    CheckRule.cacheKey { judge with ModelId = modelId + "!" } checkHash arts q <> k

                let diffVersion =
                    CheckRule.cacheKey { judge with Version = version + "!" } checkHash arts q <> k
            ...
            test
                "V15 cacheKey: artifact half is order- and duplicate-independent; empty set still stable and varies with check hash + judge (US2 edge, FR-012, INV-3)" {
                let judge = bridge.Judge
                let arts = [ "h1"; "h2"; "h3" ]
                let k = CheckRule.cacheKey judge "ch" arts (Some "q")

                Expect.equal
                    (CheckRule.cacheKey judge "ch" [ "h3"; "h1"; "h2" ] (Some "q"))
                    k
                    "permuted artifact hashes ⇒ same key"

                Expect.equal
                    (CheckRule.cacheKey judge "ch" [ "h1"; "h1"; "h2"; "h3"; "h3" ] (Some "q"))
                    k
                    "duplicated artifact hashes ⇒ same key"
                ...
                Expect.notEqual
                    (CheckRule.cacheKey judge "ch2" [] (Some "q"))
                    e
                    "empty set: key still varies with the check hash"

                Expect.notEqual
                    (CheckRule.cacheKey { judge with ModelId = "other" } "ch" [] (Some "q"))
                    e
                    "empty set: key still varies with the judge"
            ...
            test
                "V16 AgentReviewed: a recorded review with the matching key ⇒ cache HIT — Decided, zero NeedsReview, provenance carries the review's FactId (US2 AS3, FR-009, SC-003, INV-4)" {
                let id = RuleId "judge"

                let r =
                    okRule (CheckRule.rule id AgentReviewed spec opaqueCheck)
                    |> CheckRule.asking "Is the tone professional?"

                let k = keyOf [] r

                let recorded =
                    mkFact "rev1" (GovOut(Reviewed { Rule = id; Key = k; Verdict = Pass }))
                ...
                Expect.isEmpty
                    (outs
                     |> List.filter (function
                         | NeedsReview _ -> true
                         | _ -> false))
                    "cache hit ⇒ zero NeedsReview (no agent call)"

                let inputs =
                    emitted
                    |> List.collect (fun f -> f.Provenance)
                    |> List.collect (fun p -> p.Inputs)

                Expect.equal
                    inputs
                    [ recorded.Id ]
                    "cache hit ⇒ provenance Inputs = the consumed RecordedReview's FactId"
            }

            test
                "V17 AgentReviewed: no matching recorded review ⇒ cache MISS — exactly one NeedsReview carrying the key (US2 AS4, FR-009, SC-003, INV-5)" {
                let id = RuleId "judge"
                ...
                for facts in [ []; [ nonMatching ] ] do
                    ...
                    Expect.equal
                        ...
                        [
                            {
                                Rule = id
                                Question = Some "Is the tone professional?"
                                Key = keyOf facts r
                            }
                        ]
                        "cache miss ⇒ exactly one NeedsReview carrying the key"
                    ...
                    Expect.isEmpty
                        (outs
                         |> List.filter (function
                             | Decided _ -> true
                             | _ -> false))
                        "cache miss ⇒ no Decided"
            }

            test
                "V18 AgentReviewed: a verdict recorded under an old judge/prompt no longer matches once the judge or the question changes ⇒ fresh NeedsReview (US2 AS5, FR-013, SC-004, INV-6)" {
                let id = RuleId "judge"
                ...
                let recorded =
                    mkFact "rev" (GovOut(Reviewed { Rule = id; Key = k; Verdict = Pass }))
                ...
                let outs2 =
                    (CheckRule.toRule bridge2 r).Apply [ recorded ]
                    |> List.choose (fun f -> bridge2.Project f.Value)
                ...
                Expect.equal
                    ...
                    "stale verdict under an old judge ⇒ fresh NeedsReview"
                ...
                Expect.equal
                    ...
                    "stale verdict under an old prompt ⇒ fresh NeedsReview"
            }

            // ── User Story 1: bridge a tiered rule into the executable kernel ──

            test
                "V19 Deterministic: Apply asserts Decided(id, eval) verbatim (Uncertain not coerced); Description = render for every rule (US1 AS1/2, FR-007/008, SC-005/006, INV-7/8)" {
                let cases =
                    [ metCheck, Pass; failCheck, Fail "nope"; uncertainCheck, Uncertain "tbd" ]
                ...
                for chk, expected in cases do
                    ...
                    Expect.equal
                        (outcomes r [])
                        [ Decided(id, Check.eval [] chk) ]
                        "Deterministic ⇒ Decided(id, eval) verbatim (Uncertain preserved)"
                ...
                Expect.isEmpty
                    (outs
                     |> List.filter (function
                         | Decided _ -> true
                         | _ -> false))
                    "HumanOnly ⇒ never a Decided verdict"
            }

            // ── User Story 4: severity and spec provenance, orthogonal to tier ──

            test
                "V21 severity ⟂ tier: defaults Advisory; blocking flips severity leaving tier; every tier×severity recorded independently; HumanOnly escalates either way; Spec recoverable (US4 AS1-4, FR-002/003/010, SC-008, INV-9)" {
                let id = RuleId "r"
                ...
                Expect.equal
                    (outcomes (CheckRule.blocking h) [])
                    [ Escalated id ]
                    "HumanOnly Blocking ⇒ Escalated (severity-independent)"
            ...
            test
                "V22 totality: toRule + Apply over every tier, empty All/Any, empty facts, and an unknown artifact never throw; the AgentReviewed miss emits NeedsReview as data (FR-015/017, SC-007, INV-10/11)" {
                let id = RuleId "t"

                let reading =
                    Check.probe "r" [ { Kind = "token"; Key = "absent" } ] [] (fun (_: FactSet<Gov>) -> Met)
                ...
                let factSets: FactSet<Gov> list =
                    [ []; [ mkFact "a" (Art("token", "other", "h")) ] ]
                ...
                Expect.isTrue
                    (outs
                     |> List.exists (function
                         | NeedsReview _ -> true
                         | _ -> false))
                    "AgentReviewed miss emits a NeedsReview value (no I/O, no agent call)"
            }
        ]
