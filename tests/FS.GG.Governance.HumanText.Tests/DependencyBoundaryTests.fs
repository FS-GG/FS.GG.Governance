...
let private humanTextAssembly =
    SurfaceDrift.assemblyNamed "FS.GG.Governance.HumanText"
    ...
    testList
        ...
        [
            test "HumanText does not reference Spectre.Console or any presentation package" {
                ...
                Expect.isEmpty
                    offending
                    (sprintf "HumanText must not reference a presentation package; found: %A" offending)
            }
            ...
            test "HumanText references only report cores + BCL + FSharp.Core" {
                ...
                Expect.isEmpty
                    forbidden
                    (sprintf "HumanText must not reference host/render/Spectre; found: %A" forbidden)
            }
        ]
