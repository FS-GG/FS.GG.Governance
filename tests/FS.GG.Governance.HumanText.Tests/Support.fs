...
let emptyEvidenceReport: CacheEligibilityReport =
    CacheEligibility.evaluate [] EvidenceReuse.empty
...
let blockedReleaseReport: ReleaseReport =
    releaseReportOf ((VersionBump, Unmet) :: (allFamilies |> List.tail |> List.map (fun k -> k, Met)))
