    ...
    testList
        ...
        [
            ...
            test "selectMode: Rich iff interactive, else Plain" {
                Expect.equal
                    (RenderMode.selectMode false (cap true false false))
                    RenderMode.Rich
                    "tty no-nocolor no-plain ⇒ rich"
            ...
            test "every HumanText.of* renders a non-empty string over a real report" {
                Expect.isGreaterThan
                    (HumanText.ofRouteResult routeWithFindings (Some evidenceReport) mixedOutcomes).Length
                    0
                    "route"
            ...
            test "every ReportView.viewOf* yields a titled view over a real report" {
                let nonEmpty (v: ReportView.ReportView) =
                    Expect.isGreaterThan v.Title.Length 0 "title"
