    ...
    testList
        ...
        [
            test "route: the same gate ids surface in both the text and route.json" {
                let text =
                    HumanText.ofRouteResult routeWithFindings (Some evidenceReport) mixedOutcomes

                let json =
                    RouteJson.ofRouteResult routeWithFindings (Some evidenceReport) mixedOutcomes
            ...
            test "verify: the verdict + each blocker gate surface in both the text and verify.json" {
                let text =
                    HumanText.ofVerifyDecision blockedDecision (Some evidenceReport) mixedOutcomes

                let json =
                    VerifyJson.ofVerifyDecision blockedDecision (Some evidenceReport) mixedOutcomes
