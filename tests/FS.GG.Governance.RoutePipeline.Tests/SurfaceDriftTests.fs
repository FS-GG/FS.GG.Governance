...
let private routePipeline =
    SurfaceDrift.assemblyNamed "FS.GG.Governance.RoutePipeline"
    ...
    testList
        ...
        [
            ...
            test "the public API surface is exactly the Loop + Interpreter modules" {
                let typeNames =
                    routePipeline.GetExportedTypes()
                    |> Array.choose (fun t -> Option.ofObj t.FullName)

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.Contains "FS.GG.Governance.RouteCommand.LoopModule"))
                    "Loop module is public"

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.Contains "FS.GG.Governance.RouteCommand.InterpreterModule"))
                ...
                let unexpected =
                    ...
                    |> Array.filter (fun n ->
                        not (
                            ...
                            || n.Contains "RouteCommand.Interpreter+"
                        )) // nested types of Interpreter
            ...
            test "RoutePipeline references only the eight cores + BCL + FSharp.Core (no kernel/host/cli, D1)" {
                ...
                Expect.isEmpty
                    offending
                    (sprintf "RoutePipeline must depend on the eight cores/BCL/FSharp.Core only; found: %A" offending)
                ...
                Expect.isEmpty
                    forbidden
                    (sprintf "RoutePipeline must not reference kernel/host/cli/adapters; found: %A" forbidden)
            }
        ]
