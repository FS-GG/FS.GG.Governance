        ...
        [
                ...
                let registry =
                    catalog
                        [
                            ...
                            gate "build" "rel-cheap" Cheap Release
                        ] // cheaper, same domain, but Release
            ...
            test "local-permission truth table: Local/LocalOrCi qualify; Ci/Release do not (D6)" {
                ...
                for env, qualifies in [ Local, true; LocalOrCi, true; Ci, false; Release, false ] do
                    ...
                    | outcome, _ ->
                        failtestf "env %A: unexpected outcome %A (expected qualifies=%b)" env outcome qualifies
            }
            ...
            test "tie-break: among equally-cheapest candidates the least GateId ordinal is named (L5, FR-007)" {
                ...
                | CheaperLocalAlternative g ->
                    Expect.equal (gateIdValue g.Id) "build:aaa" "least GateId breaks the cost tie"
                ...
                let registry =
                    catalog
                        [
                            ...
                            gate "build" "cheap-local" Cheap Local
                        ] // the cheapest candidate

                match alternativeFor h registry with
                | CheaperLocalAlternative g ->
                    Expect.equal (gateIdValue g.Id) "build:cheap-local" "the cheapest candidate is named"
                | NoCheaperLocalAlternative -> failtest "expected build:cheap-local"
            }

            testPropertyWithConfig
                fscheckConfig
                "every finding carries a present Alternative — never omitted (L3, SC-003)"
                (fun (route: RouteResult) (registry: GateRegistry) ->
                    let explanation = RouteExplain.explain route registry
