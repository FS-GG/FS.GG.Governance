...
let private routeExplainAsm =
    SurfaceDrift.assemblyNamed "FS.GG.Governance.RouteExplain"
    ...
    testList
        ...
        [
            ...
            test "the public surface is exactly the two modules (Model + RouteExplain), nothing else" {
                let typeNames =
                    routeExplainAsm.GetExportedTypes()
                    |> Array.choose (fun t -> Option.ofObj t.FullName)

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.RouteExplain.ModelModule"))
                    "Model module is public"

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.RouteExplain.RouteExplainModule"))
