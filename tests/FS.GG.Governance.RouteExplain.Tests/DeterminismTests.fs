...
let private route =
    routeOf [ selGate gFull paths; selGate gHigh [ sp "src/a.fs" "src/**" ] ]
    ...
    testList
        ...
        [
            testPropertyWithConfig
                fscheckConfig
                "explain is referentially transparent — twice yields equal results (L6, SC-005)"
                (fun (r: RouteResult) (reg: GateRegistry) -> RouteExplain.explain r reg = RouteExplain.explain r reg)

            test "reordering the selected gates does not change the explanation (L6)" {
                let reordered = routeOf (route.SelectedGates |> List.rev)

                Expect.equal
                    (RouteExplain.explain reordered registry)
                    canonical
                    "findings re-sorted by GateId — order-invariant"
            }
            ...
            test "reordering the registry gates does not change the explanation (L6)" {
                ...
                Expect.equal
                    (RouteExplain.explain route reordered)
                    canonical
                    "alternative resolution sorts candidates — order-invariant"
            }
            ...
            test "duplicating the registry gates does not change the explanation (L6, dup-invariance)" {
                ...
                Expect.equal
                    (RouteExplain.explain route duplicated)
                    canonical
                    "duplicate candidates resolve to the same head"
            }
