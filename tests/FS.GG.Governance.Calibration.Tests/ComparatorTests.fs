    ...
    testList
        ...
        [
            test "sample gate for min >= 2: below / at / above (agreement satisfied)" {
                ...
                Expect.isFalse
                    (Calibration.isCalibrated (Calibration.decide (thresholds 3 80) (evidenceOf 2 100)))
                    "observed 2 < min 3"

                Expect.isTrue
                    (Calibration.isCalibrated (Calibration.decide (thresholds 3 80) (evidenceOf 3 100)))
                    "observed 3 = min 3"

                Expect.isTrue
                    (Calibration.isCalibrated (Calibration.decide (thresholds 3 80) (evidenceOf 4 100)))
                    "observed 4 > min 3"
            }
            ...
            test "agreement gate: below / at / above (sample gate satisfied)" {
                ...
                Expect.isFalse
                    (Calibration.isCalibrated (Calibration.decide (thresholds 3 80) (evidenceOf 3 79)))
                    "obs 79 < req 80"

                Expect.isTrue
                    (Calibration.isCalibrated (Calibration.decide (thresholds 3 80) (evidenceOf 3 80)))
                    "obs 80 = req 80 (inclusive)"

                Expect.isTrue
                    (Calibration.isCalibrated (Calibration.decide (thresholds 3 80) (evidenceOf 3 81)))
                    "obs 81 > req 80"
            }
            ...
            testPropertyWithConfig
                fscheckConfig
                "reason precedence: NoCalibrationEvidence ▸ TooFewSamples ▸ AgreementBelowThreshold"
            <| fun (t: CalibrationThresholds) (e: CalibrationEvidence) ->
                let observed = List.length e.Samples
                let effectiveMin = max (Calibration.sampleCountValue t.MinimumSamples) 2
                let obs = Calibration.agreementValue e.ObservedAgreement
                ...
                | Uncalibrated(AgreementBelowThreshold(o, r)) ->
                    observed >= effectiveMin
                    && obs < req
                    && o = e.ObservedAgreement
                    && r = t.MinimumAgreement
