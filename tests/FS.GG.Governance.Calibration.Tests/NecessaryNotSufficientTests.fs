...
let tests =
    testList
        ...
        [
            ...
            test
                "no-hide: a Calibrated decision always names its metrics; an Uncalibrated always names its reason (L-D8/L-D9, FR-001)" {
                for (t, e, _) in workedExamples do
                    match Calibration.decide t e with
                    | Calibrated m ->
                        Expect.equal
                            (Calibration.calibrationMetrics (Calibrated m))
                            (Some m)
                            "Calibrated ⇒ metrics named"

                        Expect.isNone (Calibration.calibrationReason (Calibrated m)) "Calibrated ⇒ no reason"
                    | Uncalibrated r ->
                        Expect.equal
                            (Calibration.calibrationReason (Uncalibrated r))
                            (Some r)
                            "Uncalibrated ⇒ reason named"
