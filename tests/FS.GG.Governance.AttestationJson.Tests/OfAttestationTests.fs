...
let tests =
    testList
        ...
        [
            ...
            test
                "complianceToken is the canonical compatible-shape marker (M-JSON-1: referenced by Release/Verify embeds)" {
            ...
            test
                "fixed field order: schemaVersion < compliance < complianceNote < identity < builder < subjects < materials < invocation" {
                let json = AttestationJson.ofAttestation baseSummary
