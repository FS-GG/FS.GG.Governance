        ...
        [ // (a) Verbatim carriage of all nine facts (SC-001, SC-002, US1 #1/#2).
            test "build carries all nine facts back verbatim" {
                ...
                Expect.equal
                    p.ArtifactDigests
                    baseArtifactDigests
                    "artifact digests (same elements, same order — verbatim)"

                Expect.equal
                    p.CommandRecords
                    baseCommandRecords
                    "command records (same elements, same order, each whole)"
            ...
            test "every supplied artifact digest is present on the value (no dedup at build time)" {
                ...
                Expect.equal
                    p.ArtifactDigests
                    [ ArtifactHash "a1"; ArtifactHash "a1"; ArtifactHash "a2" ]
                    "duplicate carried verbatim — canonicalization is canonicalId's job"
            }
            ...
            test "a failed / timed-out embedded command record is an ordinary complete provenance" {
                ...
                Expect.equal
                    p.CommandRecords.Head.Reproducible.ExitCode
                    (ExitCode 137)
                    "failed run carried whole, not rejected"
            }
