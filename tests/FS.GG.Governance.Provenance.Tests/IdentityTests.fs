    ...
    testList
        ...
        [ // (a) Sensed split — durations reachable, distinct, and identity-invariant (SC-003/SC-004, US2 #1/#3).
            test "embedded record durations are reachable as sensed metadata, distinct from reproducible facts" {
                Expect.equal
                    baseProvenance.CommandRecords.Head.Duration
                    (SensedDuration 123_456L)
                    "record.Duration reachable via the provenance"

                Expect.equal
                    baseProvenance.CommandRecords.Head.Reproducible.Executable
                    (Executable "gcc")
                    "reproducible facts reachable separately"
            }

            test "two provenances differing only in an embedded record's duration have equal canonicalId" {
                let p1 =
                    { baseProvenance with
                        CommandRecords = [ makeCommandRecord 1L ]
                    }

                let p2 =
                    { baseProvenance with
                        CommandRecords = [ makeCommandRecord 9_999_999L ]
                    }
                ...
                Expect.equal
                    (Provenance.canonicalId (rebuild p1))
                    (Provenance.canonicalId (rebuild p2))
                    "duration excluded from identity"
            }
            ...
            test "changing any one reproducible fact changes canonicalId" {
                ...
                for (label, variant) in allReproducibleVariants do
                    ...
                    Expect.notEqual
                        (Provenance.canonicalId perturbed)
                        baseId
                        (sprintf "flipping '%s' must change the identity" label)
            }
            ...
            test "the same revision string in two fields yields different identities" {
                ...
                let p2 =
                    rebuild
                        { baseProvenance with
                            SourceCommit = Revision "x"
                            Base = Revision "dup"
                        }

                Expect.notEqual
                    (Provenance.canonicalId p1)
                    (Provenance.canonicalId p2)
                    "src vs base are distinct tagged segments"
            }
            ...
            test "the same string as rule hash vs generator version yields different identities" {
                let p1 =
                    rebuild
                        { baseProvenance with
                            RuleHash = RuleHash "shared"
                            GeneratorVersion = GeneratorVersion "g"
                        }
                ...
                let p2 =
                    rebuild
                        { baseProvenance with
                            RuleHash = RuleHash "g"
                            GeneratorVersion = GeneratorVersion "shared"
                        }

                Expect.notEqual
                    (Provenance.canonicalId p1)
                    (Provenance.canonicalId p2)
                    "rule vs gen are distinct tagged segments"
            }
            ...
            test "worked example renders to the contract's exact canonical block" {
                ...
                Expect.equal
                    actual
                    workedExampleBlock
                    "worked-example identity equals contracts/provenance-identity-format.md"
            ...
            testPropertyWithConfig fscheckConfig "duration-only difference never changes the identity"
            <| fun (p: Provenance) ->
                ...
                let bumped =
                    ...
                    |> List.map (fun r ->
                        ...
                        CommandRecord.build
                            r.Reproducible.Executable
                            r.Reproducible.Arguments
                            r.Reproducible.WorkingDirectory
                            r.Reproducible.Environment
                            r.Reproducible.Timeout
                            r.Reproducible.ExitCode
                            r.Reproducible.StdoutDigest
                            r.Reproducible.StderrDigest
                            r.Reproducible.CapturedOutput
                            (SensedDuration(n + 1L)))
