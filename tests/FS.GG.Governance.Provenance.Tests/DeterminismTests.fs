    ...
    testList
        ...
        [ // (a) build then canonicalId twice ⇒ structurally / byte equal.
            ...
            test "reordering and duplicating artifact digests leaves canonicalId unchanged" {
                ...
                let permuted =
                    { p with
                        ArtifactDigests = permuteAndDuplicateDigests digests
                    }
            ...
            testPropertyWithConfig fscheckConfig "artifact-digest order/dup permutation is identity-invariant"
            <| fun (p: Provenance) ->
                let permuted =
                    { p with
                        ArtifactDigests = permuteAndDuplicateDigests p.ArtifactDigests
                    }
            ...
            test "reordering command records DOES change canonicalId" {
                let forward =
                    { baseProvenance with
                        CommandRecords = [ baseCommandRecord; secondRecord ]
                    }

                let reversed =
                    { baseProvenance with
                        CommandRecords = [ secondRecord; baseCommandRecord ]
                    }
