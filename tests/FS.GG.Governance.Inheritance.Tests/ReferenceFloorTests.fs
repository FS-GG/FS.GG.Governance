...
let private gateIds (gates: Gate list) =
    gates |> List.map (fun g -> gateIdValue g.Id)
    ...
    testList
        ...
        [
            ...
            test "productTemplateProfiles reads distinct, sorted profiles off the surfaces (FR-002)" {
                ...
                let profiles =
                    productTemplateProfiles facts |> List.map (fun (TemplateProfile p) -> p)
            ...
            test "an inherited-only gate carries an EMPTY selection trace (FR-005)" {
                ...
                let inheritedSel =
                    out.SelectedGates
                    |> List.find (fun sg -> gateIdValue sg.Gate.Id = "gameplay:fr-covered")
            ...
            test "applyInheritance preserves the local gate's selection trace" {
                ...
                let localSel =
                    out.SelectedGates
                    |> List.find (fun sg -> gateIdValue sg.Gate.Id = "build:compile")
