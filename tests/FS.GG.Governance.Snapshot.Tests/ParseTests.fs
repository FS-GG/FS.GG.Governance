...
let private findPath p (cs: ChangedPath list) =
    cs |> List.find (fun c -> c.Path = gp p)
    ...
    testList
        ...
        [ // ── diff --name-status -z -M ──
            ...
            test "rename/copy three-NUL records carry Path=new, OldPath=Some old" {
                let cs =
                    changedOf (znul [ "R096"; "old.fs"; "new.fs" ] + znul [ "C100"; "src.fs"; "copy.fs" ])
            ...
            test "a non-ASCII / spaced path survives -z verbatim (FR-012)" {
                ...
                Expect.equal
                    (cs |> List.map (fun c -> c.Path))
                    [ gp "docs/föö bar.md" ]
                    "path preserved, not git-quoted"
            }
