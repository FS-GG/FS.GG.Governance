        ...
        [ // ── US1: committed changed-path set ──
            ...
            test "a two-file diff yields both as normalized GovernedPaths sorted by Path, with Some Range" {
                let snap =
                    Snapshot.assemble
                        { baseRaw with
                            DiffRaw = Ok(znul [ "M"; "./src/Kernel/Eval.fs" ] + znul [ "A"; "docs/intro.md" ])
                        }
            ...
            test "each SensingDiagnosticId is reachable from its matching RawSensing error shape" {
                ...
                only
                    ...
                    (Snapshot.assemble
                        { baseRaw with
                            BaseResolved = Error "no such ref"
                        })
                ...
                only
                    ...
                    (Snapshot.assemble
                        { baseRaw with
                            MergeBaseResolved = Error "no merge base"
                        })
                ...
                only
                    ...
                    (Snapshot.assemble
                        { baseRaw with
                            StatusRaw = Error "permission denied"
                        })
            ...
            test "diagnostics are sorted by (id token, operation) and non-empty exactly on failure" {
                ...
                let sorted =
                    ...
                    |> List.sortWith (fun a b ->
                        let c =
                            System.String.CompareOrdinal(sensingDiagnosticIdToken a.Id, sensingDiagnosticIdToken b.Id)

                        if c <> 0 then
                            c
                        else
                            System.String.CompareOrdinal(a.Operation, b.Operation))
            ...
            test "empty-success and failure are structurally distinct (FR-011)" {
                ...
                Expect.isTrue
                    (success.Diagnostics.IsEmpty && success.Range.IsSome)
                    "success: no diagnostics, Some range"
