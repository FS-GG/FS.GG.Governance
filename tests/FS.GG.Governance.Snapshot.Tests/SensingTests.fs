...
let private portsWithCi dir ci : Ports =
    {
        Git = (Interpreter.realPorts dir).Git
        Ci = fun () -> ci
    }

let private changedPaths (snap: RepoSnapshot) =
        ...
        [ // ── US1: committed changed-path set over real git ──
            ...
            test "senses the committed changed-path set between base and head (US1 AS1/AS3)" {
                withTempRepo (fun dir ->
                    ...
                    let snap =
                        Interpreter.senseSnapshot ports (opts None (Some(GitRef "HEAD~1")) (Some(GitRef "HEAD")))

                    Expect.equal
                        (changedPaths snap)
                        [ gp "docs/intro.md"; gp "src/Eval.fs" ]
                        "two normalized paths, sorted"
            ...
            test "senses dirty + untracked working-tree state, distinct from committed Changed (US2)" {
                withTempRepo (fun dir ->
                    ...
                    Expect.isEmpty
                        (changedPaths snap)
                        "Default range ⇒ empty committed Changed (uncommitted work only)")
            }
            ...
            test "a non-repository directory ⇒ NotARepository, no throw, not an empty success (US5 AS1)" {
                withTempDir (fun dir ->
                    ...
                    Expect.equal
                        (snap.Diagnostics |> List.map (fun d -> d.Id))
                        [ NotARepository ]
                        "NotARepository diagnostic"
                ...
                withTempRepo (fun dir ->
                    ...
                    let snap =
                        Interpreter.senseSnapshot
                            (Interpreter.realPorts dir)
                            (opts None (Some(GitRef "no-such-ref")) (Some(GitRef "HEAD")))

                    Expect.contains
                        (snap.Diagnostics |> List.map (fun d -> d.Id))
                        UnknownRef
                        "UnknownRef diagnostic raised"
            ...
            test "sensing is byte-for-byte read-only (US5 AS3, SC-005)" {
                withTempRepo (fun dir ->
                    ...
                    Interpreter.senseSnapshot
                        (Interpreter.realPorts dir)
                        (opts None (Some(GitRef "HEAD~1")) (Some(GitRef "HEAD")))
                    |> ignore
