...
let tests =
    testList
        ...
        [ // ── US1 (SC-001) ──
            test
                "V1 the adapter is fully specified by the five SPI components + the Bridge — 15 rules, 1 fence (token-surface only, NO merge fence)" {
                ...
                Expect.equal
                    adapter.Fences.Length
                    1
                    "exactly one fence — the token surface; there is NO merge fence (no lifecycle)"

                Expect.equal
                    (adapter.Fences |> List.map (fun f -> f.Name))
                    [ "token-surface" ]
                    "the single fence is the token-surface fence"
            ...
            test "V1 toRef is injective over all five DesignArtifactRef cases (a faithful-lift precondition, FR-002)" {
                ...
                Expect.equal
                    (List.distinct refs).Length
                    allArtifacts.Length
                    "distinct artifact kinds map to distinct ArtifactRefs"
            }

            test
                "V1 identify is injective on value-bearing facts, and keys entity facts so a later fact supersedes (law L0)" {
                // value-bearing facts are distinguished by full value …
                Expect.notEqual
                    (DesignSystem.identify (DesignRule "a"))
                    (DesignSystem.identify (DesignRule "b"))
                    "DesignRule keyed by value"
            ...
            // ── US3 (SC-003) ──
            test
                "V3 the full catalog evaluates AND explains over the fixture token tree (no rendering library on the path)" {
                ...
                for r in deterministic do
                    ...
                    Expect.isTrue
                        (isPass (Check.eval conformingFacts r.Check))
                        (sprintf "%s passes over the conforming fixtures" id)
            }

            test
                "V3 the probes are three-valued over fixtures — Met / Unmet / Unknown, never a silent Met for a missing observation" {
                ...
                Expect.isTrue
                    (isPass (
                        Check.eval
                            [ fact (SurfaceObservation("surface-matches", GeneratedTokenSurface, true)) ]
                            surfaceMatches
                    ))
                ...
                Expect.isTrue
                    (isFail (
                        Check.eval
                            [ fact (SurfaceObservation("surface-matches", GeneratedTokenSurface, false)) ]
                            surfaceMatches
                    ))
                ...
                Expect.isTrue
                    (isUncertain (Check.eval [] surfaceMatches))
                    "surface-matches: a missing observation ⇒ Uncertain"

                Expect.isTrue
                    (isUncertain (Check.eval [] contrast))
                    "contrast: a missing contrast fixture ⇒ Uncertain (never a silent Pass)"
