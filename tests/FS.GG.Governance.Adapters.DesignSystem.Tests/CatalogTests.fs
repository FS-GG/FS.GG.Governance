...
let tests =
    testList
        ...
        [ // ── US2 (SC-002) — deterministic block ──
            test
                "V2 the deterministic token/contrast/surface/evidence rules give DEFINITE verdicts and are the Blocking set" {
                ...
                Expect.isTrue
                    (isPass (Check.eval (metFacts "surface-matches" GeneratedTokenSurface) Catalog.tokenDrift.Check))
                    "token-drift Pass on a matching surface"

                Expect.isTrue
                    (isFail (Check.eval (unmetFacts "surface-matches" GeneratedTokenSurface) Catalog.tokenDrift.Check))
                    "token-drift Fail on drift"

                Expect.isTrue
                    (isUncertain (Check.eval [] Catalog.tokenDrift.Check))
                    "token-drift Uncertain when the fixture is absent (never a silent Pass)"

                Expect.isTrue
                    (isPass (Check.eval (metFacts "contrast-meets" GeneratedTokenSurface) Catalog.contrastPolicy.Check))
                    "contrast-policy Pass when the ratio is met"

                Expect.isTrue
                    (isUncertain (Check.eval [] Catalog.contrastPolicy.Check))
                    "contrast-policy Uncertain when the contrast fixture is missing"

                Expect.isTrue
                    (isPass (
                        Check.eval (metFacts "token-surface-gate" GeneratedTokenSurface) Catalog.tokenSurfaceGate.Check
                    ))
                    "token-surface-gate Pass when blessed"
                ...
                Expect.isTrue
                    (isFail (Check.eval tainted Catalog.evidenceMeasured.Check))
                    "evidence-measured Fail when a verdict rests on synthetic evidence"

                Expect.isTrue
                    (isPass (Check.eval conformingFacts Catalog.evidenceMeasured.Check))
                    "evidence-measured Pass over the conforming (all-Real) fixtures"
            ...
            test
                "V2 each AgentReviewed judgement rule is Opaque, never resolves, and routes to a review carrying its Question" {
                let judgementRules =
                    [
                ...
                for r in judgementRules do
                    ...
                    Expect.isTrue
                        (isUncertain (Check.eval conformingFacts r.Check))
                        (sprintf "%s never resolves to Pass/Fail — Uncertain for any facts" id)
            ...
            test "V2 adopt-new-policy is HumanOnly — escalates to a person and never resolves deterministically" {
                ...
                Expect.isTrue
                    (isUncertain (Check.eval conformingFacts Catalog.adoptNewPolicy.Check))
                    "adopt-new-policy never resolves by engine"
            ...
            test
                "V2 advisory by default; only a token-surface change trips the single fence and makes the Blocking set bite" {
                let plainChange =
                    {
                ...
                Expect.isEmpty
                    (blockingIds plainChange)
                    "a change not touching the token surface blocks nothing (advisory default)"
            ...
            test
                "V4 every rule renders to a non-empty sentence, explains itself, and its published Statement equals Check.render" {
                ...
                for (r, entry) in List.zip Catalog.catalog contract do
                    ...
                    Expect.equal
                        entry.Statement
                        rendered
                        (sprintf "%s: published Statement = Check.render (advertised = enforced)" id)
            ...
            test
                "V5 a deterministic rule's hash is invariant under commutative re-ordering while positional nodes stay positional" {
            ...
            test "V5 two structurally-equal judgement rules produce the same agent-review cache key" {
                ...
                let twin =
                    DesignSystem.reviewing
                        [ RenderedCapture ]
                        (Opaque("colour-informational", fun _ -> Unknown "requires judgement"))
                ...
                let key1 =
                    CheckRule.cacheKey judge (Check.hash r.Check) (Check.reads r.Check |> List.map artHash) r.Question

                let key2 =
                    CheckRule.cacheKey
                        judge
                        (Check.hash twin)
                        (Check.reads twin |> List.map artHash)
                        (Some "Does colour carry information here, or is it decoration?")

                Expect.equal
                    key1
                    key2
                    "identical reviewed artifacts + Opaque name + judge + prompt ⇒ identical cache key (no spurious re-review)"
            }
            ...
            test "V5b changing a reviewed artifact's content hash moves the agent-review key (M-ADPT-2)" {
                ...
                let changed (ref: ArtifactRef) =
                    if ref = renderedRef then "h-CHANGED" else "h0"

                let k0 =
                    CheckRule.cacheKey judge (Check.hash r.Check) (Check.reads r.Check |> List.map baseHash) r.Question

                let k1 =
                    CheckRule.cacheKey judge (Check.hash r.Check) (Check.reads r.Check |> List.map changed) r.Question
