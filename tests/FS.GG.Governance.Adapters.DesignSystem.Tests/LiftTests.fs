...
let tests =
    testList
        ...
        [ // ── SC-006 ──
            test
                "V6 render / hash / reads / isReified are invariant under the lift for 100% of the catalog (the cache key does not move)" {
                for r in Catalog.catalog do
                    ...
                    Expect.equal
                        ...
                        (sprintf "%s: reified-ness invariant (a lifted Opaque stays opaque)" id)
            }

            test
                "V6 verdict & explanation (provenance proof tree) are identical standalone vs lifted, for 100% of the catalog" {
                for r in Catalog.catalog do
                    let (RuleId id) = r.Id
                    let lifted = Lift.check (|DesignP|_|) r.Check
            ...
            test
                "V6 the executable lifted rule preserves (verdict, provenance) — Id, Provenance, and value verbatim (law L2/L3) for 100% of the catalog" {
                for r in Catalog.catalog do
                    let (RuleId id) = r.Id
            ...
            test
                "V7 the design-system adapter composes alongside the REAL F10 Spec Kit adapter and drops out cleanly (independence)" {
                ...
                Expect.equal
                    withoutDesign.Catalog.Length
                    specKitAdapter.Rules.Length
                    "dropping the design domain leaves only the Spec Kit domain"
