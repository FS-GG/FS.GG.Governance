    ...
    testList
        ...
        [
            ...
            test "identity is Provenance.canonicalId verbatim" {
                let expected =
                    Provenance.identityValue (Provenance.canonicalId baseSnapshot.Provenance)
            ...
            test "artifactDigests rendered SORTED with duplicates collapsed by sort (set semantics)" {
                let arts =
                    [
                        for a in root.GetProperty("artifactDigests").EnumerateArray() -> a.GetString()
                    ]

                Expect.equal
                    arts
                    [ "a1"; "a2"; "a2" ]
                    "sorted ordinal (carriage is set in identity; rendering is sorted)"
            }
            ...
            test "commandRuns in carried order, each {kind, identity, exitCode, durationNanos}" {
                ...
                Expect.equal
                    (first.GetProperty("identity").GetString())
                    (Audit.runIdentity runBuild)
                    "per-run identity is CommandRecord.canonicalId"
                ...
                Expect.isLessThan
                    (raw.IndexOf "\"exitCode\"")
                    (raw.IndexOf "\"durationNanos\"")
                    "exitCode < durationNanos"
            }
            ...
            test "two snapshots differing ONLY in durations share the top-level and per-run identity" {
                ...
                Expect.equal
                    (slowDoc.RootElement.GetProperty("identity").GetString())
                    (root.GetProperty("identity").GetString())
                    "top-level identity unchanged"

                let slowFirst = slowDoc.RootElement.GetProperty("commandRuns").[0]

                Expect.equal
                    (slowFirst.GetProperty("identity").GetString())
                    (root.GetProperty("commandRuns").[0].GetProperty("identity").GetString())
                    "per-run identity unchanged"

                Expect.notEqual
                    (slowFirst.GetProperty("durationNanos").GetInt64())
                    111L
                    "only the sensed duration differs"
            }
