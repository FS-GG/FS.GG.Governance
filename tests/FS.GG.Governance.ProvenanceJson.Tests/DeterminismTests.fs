    ...
    testList
        ...
        [
            test "ofSnapshot is byte-identical for identical input" {
                Expect.equal
                    (ProvenanceJson.ofSnapshot baseSnapshot)
                    (ProvenanceJson.ofSnapshot baseSnapshot)
                    "identical"
            }
            ...
            test "a duration-only change leaves the identity fields unchanged (only durationNanos differs)" {
                ...
                let slow =
                    ProvenanceJson.ofSnapshot (
                        snapshotOf
                            [
                                { runBuild with
                                    Record = makeRecord 0 7_000_000L
                                }
                                runTest
                                runFailed
                            ]
                ...
                let idOf (s: string) =
                    (JsonDocument.Parse s).RootElement.GetProperty("identity").GetString()
