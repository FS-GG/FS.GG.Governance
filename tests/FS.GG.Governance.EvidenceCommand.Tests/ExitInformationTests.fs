    ...
    testList
        ...
        [
            ...
            test "an absent/unreadable input maps to InputUnavailable (3) with a diagnostic, and writes NOTHING" {
                ...
                Expect.equal
                    (List.head model.Diagnostics).Category
                    Loop.InputUnavailable
                    "diagnostic categorized as input-unavailable"
            }
            ...
            test "an interpreter/tool defect maps to ToolError (4), distinct from missing input" {
                ...
                Expect.equal
                    (List.head model.Diagnostics).Category
                    Loop.ToolError
                    "diagnostic categorized as tool-error"
            }
            ...
            test "a write failure maps to ToolError (4)" {
                let r =
                    report
                        [
                            reportNode "a" FS.GG.Governance.Kernel.Real FS.GG.Governance.Kernel.Real None "speckit"
                        ]
                        []
