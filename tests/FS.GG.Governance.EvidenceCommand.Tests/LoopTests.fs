    ...
    testList
        ...
        [
            ...
            test "Reported(Ok report) emits the write effect carrying the projected document; no I/O in update" {
                ...
                Expect.equal
                    effects
                    [ Loop.WriteArtifact("out.json", expectedDoc) ]
                    "single write effect with the projected bytes"
            ...
            test
                "toDocument runs the taint closure: a Real node resting on Synthetic becomes effective AutoSynthetic (SC-004)" {
