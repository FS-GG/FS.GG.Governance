    ...
    testList
        ...
        [
            test "an Unrecoverable fact ⇒ Violated with the distinct 'no recoverable evidence' reason" {
                ...
                let f =
                    List.exactlyOne (Release.evaluate [ rule ] (factsOf [ Provenance, Unrecoverable ]))
            ...
            test "Unmet and Unrecoverable produce distinct reason text" {
                ...
                let unmet =
                    List.exactlyOne (Release.evaluate [ rule ] (factsOf [ PublishPlan, Unmet ]))

                let unrec =
                    List.exactlyOne (Release.evaluate [ rule ] (factsOf [ PublishPlan, Unrecoverable ]))
