    ...
    testList
        ...
        [
            test "one finding per rule of every kind, all Satisfied when every fact is Met" {
                ...
                Expect.isTrue (findings |> List.forall (fun f -> f.Outcome = Satisfied)) "every met fact ⇒ Satisfied"
            ...
            test "the carried BaseSeverity + Maturity equal the rule's declared values (no re-derivation)" {
                ...
                let f =
                    List.exactlyOne (Release.evaluate [ rule ] (factsOf [ PackageMetadata, Met ]))
            ...
            test "releaseRuleKindToken returns the exact seven wire tokens" {
                ...
                Expect.equal
                    (Release.releaseRuleKindToken ApiCompatibility)
                    "apiCompatibility"
                    "apiCompatibility token (088)"
            }
