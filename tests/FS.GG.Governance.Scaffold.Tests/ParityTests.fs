    ...
    testList
        ...
        [
            test "two distinct Synthetic providers differ ONLY in emitted files + recorded id; safety identical" {
                ...
                try
                    ...
                    | Some ma, Some mb ->
                        ...
                        Expect.notEqual
                            (fst (Option.get ma.Provider))
                            (fst (Option.get mb.Provider))
                            "different provider ids"

                        Expect.equal
                            (ma.Generated |> List.map (fun g -> g.RelativePath))
                            [ "src/A.fs" ]
                            "A emits its own files"

                        Expect.equal
                            (mb.Generated |> List.map (fun g -> g.RelativePath))
                            [ "src/B.fs"; "src/C.fs" ]
                            "B emits its own files"
            ...
            test "a {Major=2} provider ⇒ init yields Done(Refused(ContractMismatch)) WITHOUT invoking, no writes" {
                ...
                | Some {
                           Outcome = Refused(ContractMismatch declared)
                           Generated = []
                       } ->
                    Expect.equal
                        declared
                        { Major = 2; Minor = 0 }
                        "carries the declared version (actionable diagnostic)"
            ...
            test "a {Major=1;Minor=1} provider ⇒ refused (minor beyond supported), no invocation" {
                ...
                | Some {
                           Outcome = Refused(ContractMismatch declared)
                       } -> Expect.equal declared { Major = 1; Minor = 1 } "declared minor carried"
                | other -> failtestf "expected Refused(ContractMismatch), got %A" other
            }
