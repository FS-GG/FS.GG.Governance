        ...
        [
            test "Synthetic provider: a successful run writes every emitted file under target ⇒ Scaffolded" {
                withTemp (fun target ->
                    ...
                    let p =
                        fakeProvider "fixture.lib" [ "src/App/Program.fs", "// hello"; "README.md", "# hi" ]

                    let model =
                        Interpreter.run (Interpreter.realPorts target) (runRequest target [] (Some p))
                    ...
                    Expect.equal
                        (filesUnder target)
                        [ "README.md"; "src/App/Program.fs" ]
                        "every emitted file exists under target"
            ...
            test
                "Synthetic provider: a pre-existing file at an emitted path refuses with Collision, overwriting nothing" {
                withTemp (fun target ->
                    ...
                    match model.Manifest with
                    | Some { Outcome = Refused(Collision paths) } ->
                        Expect.contains paths "README.md" "names the colliding path"
            ...
            test "Synthetic provider: an out-of-target emission rejects with OutOfTarget, touching nothing" {
                withTemp (fun target ->
                    ...
                    | Some { Outcome = Refused(OutOfTarget paths) } ->
                        Expect.contains paths "../escape.fs" "names the escaping path"
            ...
            test
                "Synthetic provider: a Write port returning Error leaves zero new files and is reified to a Msg (never throws)" {
                withTemp (fun target ->
                    ...
                    let faultyWrite =
                        { real with
                            Write = fun _ -> Error "injected write fault"
                        }

                    let model = Interpreter.run faultyWrite (runRequest target [] (Some p))

                    match model.Manifest with
                    | Some { Outcome = Refused(ProviderErrored d) } ->
                        Expect.stringContains d "injected" "carries the fault"
            ...
            test "Synthetic provider: a Write port that THROWS is caught and reified — no partial tree, never throws" {
                withTemp (fun target ->
                    ...
                    match model.Manifest with
                    | Some { Outcome = Refused(ProviderErrored d) } ->
                        Expect.stringContains d "kaboom" "thrown exception reified"
                    | other -> failtestf "expected Refused(ProviderErrored), got %A" other
            ...
            test
                "Synthetic provider: a mid-batch rename clash rolls back the in-flight temp too — ZERO residue (SC-005)" {
                withTemp (fun target ->
            ...
            test
                "Synthetic provider: a mid-batch failure rolls back the directories it created too — ZERO new dirs (ADPT-4)" {
                withTemp (fun target ->
                    ...
                    let p =
                        fakeProvider
                            "fixture.lib"
                            [ "src/App/Program.fs", "// hi"; "dup.fs", "// first"; "dup.fs", "// second" ]
                    ...
                    Expect.isFalse
                        (Directory.Exists(Path.Combine(target, "src", "App")))
                        "the created leaf directory is rolled back"

                    Expect.isFalse
                        (Directory.Exists(Path.Combine(target, "src")))
                        "the created parent directory is rolled back too")
            }
            ...
            test "Synthetic provider: rollback does NOT delete a pre-existing directory it wrote into (ADPT-4)" {
                withTemp (fun target ->
                    ...
                    Expect.isTrue
                        (Directory.Exists(Path.Combine(target, "src")))
                        "the pre-existing directory survives rollback"

                    Expect.equal (File.ReadAllText seeded) "OPERATOR" "the operator's file under it is untouched"

                    Expect.isFalse
                        (Directory.Exists(Path.Combine(target, "src", "App")))
                        "only the batch-created nested dir is rolled back"

                    Expect.equal (filesUnder target) [ "src/keep.txt" ] "only the operator's file remains")
            }

            test
                "Synthetic provider: re-running over an already-scaffolded target reports Collision and writes nothing new" {
                withTemp (fun target ->
                    let p =
                        fakeProvider "fixture.lib" [ "src/App/Program.fs", "// hello"; "README.md", "# hi" ]
                    ...
                    | Some { Outcome = Refused(Collision paths) } ->
                        Expect.isNonEmpty paths "re-run reports the prior files as collisions"
                ...
                withTemp (fun target ->
                    ...
                    let model =
                        Interpreter.run
                            ...
                            (runRequest target [ ".fsgg/governance.yml" ] (Some p))

                    match model.Manifest with
                    | Some { Outcome = Refused(Collision paths) } ->
                        Expect.contains paths ".fsgg/governance.yml" "reserved existing path is a collision"
