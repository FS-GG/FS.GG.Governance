...
let private emissionOf (files: (string * string) list) : ProviderEmission =
    {
        Files = files |> List.map (fun (p, c) -> { RelativePath = p; Contents = c })
    }
    ...
    testList
        ...
        [
            test "compatible provider: init emits exactly InvokeProvider and enters Invoking" {
                ...
                match effects with
                | [ Loop.InvokeProvider(prov, _req) ] ->
                    Expect.equal prov.Id (ProviderId "fixture.lib") "invokes the selected provider"
            ...
            test "in-bounds emission ⇒ ProbeCollisions over resolved ∪ reserved, no write yet" {
                let p =
                    fakeProvider "fixture.lib" [ "src/App/Program.fs", "// p"; "src/App/App.fsproj", "<p/>" ]

                let model, _ = invoking p [ ".fsgg/governance.yml" ]

                let emission =
                    emissionOf [ "src/App/Program.fs", "// p"; "src/App/App.fsproj", "<p/>" ]
            ...
            test "write Ok ⇒ Done(Scaffolded) folding a provider-attributed, ascending, providerOwned manifest" {
                ...
                | Some m ->
                    ...
                    Expect.equal
                        m.Provider
                        (Some(ProviderId "fixture.lib", { Major = 1; Minor = 0 }))
                        "provider attributed"
            ...
            test "out-of-target emission ⇒ Refused(OutOfTarget) after Emit, before any probe" {
                ...
                let emission =
                    emissionOf [ "../escape.fs", "x"; "/etc/rooted.fs", "y"; "src/ok.fs", "z" ]
            ...
            test "probed pre-existing/reserved path ⇒ Refused(Collision) with NO WriteAll" {
                ...
                let model3, effects3 =
                    Loop.update (Loop.CollisionsProbed(Ok [ "src/App/Program.fs" ])) model2
            ...
            test "EmitFailed ⇒ Refused(ProviderErrored)" {
                ...
                let model2, effects2 =
                    Loop.update (Loop.ProviderEmitted(Error(EmitFailed "boom"))) model
                ...
                match model2.Manifest with
                | Some {
                           Outcome = Refused(ProviderErrored d)
                           Generated = []
                           Provider = Some _
            ...
            test "Unresolvable ⇒ Refused(ProviderUnavailable)" {
                ...
                let model2, _ =
                    Loop.update (Loop.ProviderEmitted(Error(Unresolvable "missing"))) model

                match model2.Manifest with
                | Some {
                           Outcome = Refused(ProviderUnavailable d)
                           Generated = []
                       } -> Expect.equal d "missing" "carries the detail"
            ...
            test "write Error ⇒ recoverable Refused(ProviderErrored), no partial recorded" {
                ...
                | Some {
                           Outcome = Refused(ProviderErrored d)
                           Generated = []
                       } -> Expect.equal d "disk full" "carries the write fault"
                | other -> failtestf "expected Refused(ProviderErrored), got %A" other
            }
        ]
