    ...
    testList
        ...
        [
            ...
            test "versionDelta classifies a major bump distinctly from a minor/patch bump" {
                ...
                Expect.equal
                    (Pack.versionDelta (Some "1.2.0") (Some "1.2.0"))
                    NoForwardChange
                    "equal ⇒ no forward change"

                Expect.equal
                    (Pack.versionDelta (Some "2.0.0") (Some "1.9.9"))
                    NoForwardChange
                    "downgrade ⇒ no forward change"
            ...
            test "end-to-end SC-002: a real removed member under a patch bump grades Unmet" {
                ...
                let signal =
                    ApiBreakSignal.BreakingChanges
                        [
                            {
                                Member = "PublicApi.removed"
                                Kind = MemberRemoved
                                Origin = ApiBreakOrigin.Local
                            }
                        ]
