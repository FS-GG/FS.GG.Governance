    ...
    testList
        ...
        [
            test "one PackVerdict per outcome, sorted by (SurfaceId, ArtifactPath)" {
                let outcomes =
                    [ packed "B" "out/B.nupkg" "1.1.0" "dB"; packed "A" "out/A.nupkg" "1.1.0" "dA" ]

                let result = Pack.evaluatePack bs outcomes

                let surfaces =
                    result.Verdicts |> List.map (fun v -> let (SurfaceId s) = v.Surface in s)
            ...
            test "a failed pack's run is present in Runs with its sentinel exit — never dropped" {
                ...
                let exits =
                    result.Runs
                    |> List.map (fun r ->
                        let (FS.GG.Governance.CommandRecord.Model.ExitCode c) =
                            r.Record.Reproducible.ExitCode in
            ...
            test "ArtifactUnreadable carries its message in the reason" {
                let result =
                    Pack.evaluatePack bs [ packedNoArtifact "A" (ArtifactUnreadable "bad zip") ]
