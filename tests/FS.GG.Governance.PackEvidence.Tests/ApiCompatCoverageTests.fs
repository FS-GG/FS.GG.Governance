    ...
    testList
        ...
        [
            test "mixed package set ⇒ every package covered-or-reported, SurfaceId-sorted (SC-001)" {
                ...
                | NotCovered reason ->
                    Expect.stringContains reason "indeterminate" "tool error ⇒ NotCovered, reason carried"
            ...
            test "T028/FR-013: an inherited break is attributed distinctly from a local break" {
                ...
                Expect.equal
                    (Pack.apiCompatibilityFact signal MinorOrPatchBump)
                    (Some Unmet)
                    "inherited+local under minor ⇒ Unmet"
            }
