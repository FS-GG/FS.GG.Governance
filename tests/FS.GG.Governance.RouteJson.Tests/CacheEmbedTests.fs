...
let private embedResult =
    resultOf embedFacts [ "src/build/Core.fs"; "src/docs/Guide.md"; "src/loose/x.fs" ]
...
let private noPriorReport =
    reportOf [ candidate "build:tests" buildInputs ] EvidenceReuse.empty
        ...
        [
            ...
            test "the None case renders cacheEligibilityEvaluated:false and every gate notEvaluated (L2, L9)" {
                ...
                Expect.isFalse
                    ...
                    "evaluated flag false under None"

                for g in selectedGates doc do
                    Expect.equal
                        (kindOf (cacheOf g))
                        "notEvaluated"
                        (sprintf "%s is notEvaluated under None" (strField g "id"))
            }
            ...
            test "Some _ renders cacheEligibilityEvaluated:true (L9)" {
                ...
                Expect.isTrue
                    (doc.RootElement.GetProperty("cacheEligibilityEvaluated").GetBoolean())
                    "evaluated flag true under Some"
            }
                ...
                for (gs, gn) in List.zip (selectedGates docSome) (selectedGates docNone) do
                    ...
                    for name in fieldOrder gs do
                        if name <> "cacheEligibility" then
                            Expect.equal
                                (gs.GetProperty(name).GetRawText())
                                (gn.GetProperty(name).GetRawText())
                                (sprintf "gate field %s unchanged" name)
            }
            ...
            test "no raw freshness input value / cache-derived severity leaks; evidence is verbatim (L8, SC-007)" {
                ...
                for token in
                    ...
                    Expect.isFalse
                        ...
                        (sprintf "no raw input value / enforcement token %s leaks into the cache verdict" token)
                // the evidence reference is the exact opaque token, never dereferenced/expanded.
                use doc = parse json

                Expect.equal
                    (strField (cacheOf (gateById doc "build:tests")) "evidence")
                    "ev-A"
            ...
            test
                ...
                Expect.equal
                    (RouteJson.ofRouteResult embedResult (Some dup) [])
                    (RouteJson.ofRouteResult embedResult (Some dup) [])
            ...
            test
                "totality: None / empty report / empty route / finding-only route each yield a document with the section (L11, SC-006)" {
            ...
            test "an evaluated-but-empty report still reports evaluated:true with every gate notEvaluated (L9)" {
                use doc =
                    parse (RouteJson.ofRouteResult embedResult (Some(CacheEligibilityReport [])) [])

                Expect.isTrue
                    (doc.RootElement.GetProperty("cacheEligibilityEvaluated").GetBoolean())
                    "empty report is evaluated:true (distinct from None)"
