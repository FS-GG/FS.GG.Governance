        ...
        Classifications =
            [
                ...
                classification
                    ...
                    FocusedTests
                    true
                    (CheaperLocalTier StructuralScan)
    ...
    testList
        ...
        [
            test "an EMPTY product-surface report ⇒ byte-identical to ofRouteResult (additive, non-breaking)" {
                let withEmpty =
                    RouteJson.ofRouteResultWithProductSurfaces emptyRoute None [] { Classifications = [] }
                ...
                Expect.equal
                    withEmpty
                    legacy
                    "empty report writes NO productSurfaces field — existing goldens untouched"
            }

            test "schemaVersion is unchanged at fsgg.route/v2" {
                use doc =
                    parse (RouteJson.ofRouteResultWithProductSurfaces emptyRoute None [] report)
            ...
            test "the productSurfaces golden is byte-stable (T041)" {
                let goldenDir =
                    Path.Combine(repoRoot, "tests", "FS.GG.Governance.RouteJson.Tests", "golden")
