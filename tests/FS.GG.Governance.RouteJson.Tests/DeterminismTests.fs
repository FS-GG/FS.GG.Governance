...
let private fixtureFacts =
    facts
        "src"
        [ "src/build/**", "build"; "src/docs/**", "docs"; "src/api/**", "api" ]
    ...
    testList
        ...
        [
            testPropertyWithConfig
                config
                "twice-identical: ofRouteResult is byte-identical for identical input (AS1, SC-002)"
            ...
            test "fixed-fixture twice-identical equality (SC-002)" {
                Expect.equal
                    (RouteJson.ofRouteResult populated None [])
                    (RouteJson.ofRouteResult populated None [])
                    "same result → same bytes"
            }
            ...
            <| Prop.forAll (Arb.fromGen genPaths) (fun paths ->
                ...
                let b =
                    RouteJson.ofRouteResult (resultOf fixtureFacts (List.sortDescending paths)) None []
            ...
            <| Prop.forAll (Arb.fromGen genPaths) (fun paths ->
                ...
                let a =
                    RouteJson.ofRouteResult (FS.GG.Governance.Route.Route.select real report findings) None []

                let b =
                    RouteJson.ofRouteResult (FS.GG.Governance.Route.Route.select reversed report findings) None []
            ...
            test
                "exclusion sweep: the emitted text contains no enforcement/verdict/raw-YAML/clock token (AS4, SC-007, FR-011/FR-012)" {
            ...
            test "positive path-allowlist: every emitted path/matchedGlob is a declared GovernedPath (FR-012)" {
                ...
                let declared =
                    let globs =
                        fixtureFacts.Capabilities.PathMap
                        |> List.map (fun e -> let (GovernedPath g) = e.Glob in g)

                    let changed =
                        pool |> List.map (fun p -> let (GovernedPath n) = normalizePath p in n)
                ...
                for p in allEmittedPaths doc do
                    Expect.isTrue
                        (declared.Contains p)
                        (sprintf "emitted path %A is a declared GovernedPath (no host/absolute path)" p)
                ...
                for p in allEmittedPaths doc do
                    ...
                    Expect.isFalse
                        ...
                        (sprintf "%A is not a drive path" p)
            }
        ]
