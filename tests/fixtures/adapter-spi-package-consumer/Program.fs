...
let policyCheck: Check<ProductFact> =
    Check.probe "policy-present" [ policyRef ] [] (fun facts ->
        if facts |> List.exists (fun fact -> fact.Value = PolicyPresent true) then
            Met
        else
            Unmet "policy missing")
...
let adapter: Adapter<ProductFact, ProductArtifact, ProductChange> =
    {
        ...
        Fences =
            [
                {
                    Name = "rules-corpus"
                    Trips = fun change -> change.Paths.Contains "rules.fs"
                }
            ]
        Bridge =
            {
                ...
                Project =
                    function
                    | Governance outcome -> Some outcome
                    | PolicyPresent _ -> None
            }
    }
...
let supplied: FactSet<ProductFact> =
    [
        {
            Id = FactId "policy:true"
            Value = PolicyPresent true
            Provenance = []
        }
    ]

let evaluation =
    FixedPoint.evaluate adapter.Identify (Adapter.toRules adapter) supplied

let verdict = Check.eval supplied policyCheck
let explanation = Check.explain supplied policyCheck

let route =
    Route.route adapter.Fences adapter.Rules Gate { Paths = set [ "rules.fs" ] }
