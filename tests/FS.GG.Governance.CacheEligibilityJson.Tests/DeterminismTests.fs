...
let private keyPos (doc: string) (key: string) : int =
    doc.IndexOf("\"" + key + "\"", StringComparison.Ordinal)
    ...
    testList
        ...
        [
            testPropertyWithConfig
                fscheckConfig
                "byte-for-byte determinism: ofReport r = ofReport r (L-T2)"
                (fun (r: CacheEligibilityReport) -> CacheEligibilityJson.ofReport r = CacheEligibilityJson.ofReport r)
            ...
            test "order-independence at the source: any permutation projects byte-identically (L-T3)" {
                ...
                let canonical =
                    CacheEligibilityJson.ofReport (orderingReport [ "z", "a"; "a", "b"; "a", "a" ])
                ...
                for p in perms do
                    Expect.equal
                        (CacheEligibilityJson.ofReport (orderingReport p))
                        canonical
                ...
                Expect.equal
                    (entriesOf doc |> List.map entryGate)
                    [ "a:a"; "a:b"; "z:a" ]
                    "entries in GateId-ordinal order"
            }

            testPropertyWithConfig
                fscheckConfig
                "versioned schema present + equals the constant (L-V1)"
            ...
            test "schemaVersion constant is the fixed literal (L-V1)" {
                Expect.equal
                    CacheEligibilityJson.schemaVersion
                    "fsgg.cache-eligibility/v1"
                    "fixed contract-version constant"
            }
            ...
            test "stable field order on the raw text: top-level, entry, verdict, cause (FR-007)" {
                ...
                let moved =
                    ...
                    |> (fun i ->
                        { i with
                            RuleHash = FS.GG.Governance.FreshnessKey.Model.RuleHash "r2"
                        })
                ...
                Expect.isLessThan
                    (keyPos text "schemaVersion")
                    (keyPos text "entries")
                    "schemaVersion precedes entries in raw text"
                ...
                for e in entriesOf doc do
                    ...
                    | "mustRecompute" ->
                        ...
                        | "inputsChanged" ->
                            Expect.equal (fieldOrder c) [ "kind"; "categories" ] "inputsChanged cause order"
