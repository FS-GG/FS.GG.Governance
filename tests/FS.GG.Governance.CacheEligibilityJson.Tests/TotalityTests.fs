    ...
    testList
        ...
        [
            testPropertyWithConfig
                fscheckConfig
                "totality: every well-typed report renders to a parseable document, never throws (L-T1)"
            ...
            test "all-reusable report renders every entry with its verdict" {
                ...
                let r =
                    report [ candidate (gid "a" "x") baseInputs; candidate (gid "b" "y") baseInputs ] store
            ...
            test "all-must-recompute report renders every entry with its verdict" {
                let r =
                    report
                        [ candidate (gid "a" "x") baseInputs; candidate (gid "b" "y") baseInputs ]
                        EvidenceReuse.empty
            ...
            test "duplicate GateId kept — two entries under build:tests, neither merged nor deduplicated (L-R8)" {
                ...
                Expect.equal
                    (entries |> List.map entryGate)
                    [ "build:tests"; "build:tests" ]
                    "both under the same gate id, in report order"
            }
        ]
