        ...
        [
            test "inputsChanged names exactly the categories, in order, never truncated (L-R4)" {
                ...
                let expected =
                    ...
                    | other ->
                        failtestf "expected InputsChanged, got %A" other
                        []

                Expect.equal
                    (causeCategories (verdictCause v))
                    expected
                    "categories named exactly, in report order, none truncated"
            ...
            test "noPriorEvidence ≠ inputsChanged [] — structurally distinct, never collapsed (L-R5)" {
                ...
                Expect.notEqual
                    (CacheEligibilityJson.ofReport noPrior)
                    (CacheEligibilityJson.ofReport emptyChanged)
                    "the two never render identically"
            }

            testPropertyWithConfig
                fscheckConfig
                "every mustRecompute entry carries exactly one cause from the closed vocabulary (L-R4)"
