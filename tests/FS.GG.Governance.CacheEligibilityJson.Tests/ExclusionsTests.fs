    ...
    testList
        ...
        [
            testPropertyWithConfig
                fscheckConfig
                "only the declared keys appear anywhere (L-R11)"
            ...
            testPropertyWithConfig
                fscheckConfig
                "verdict.kind ∈ {reusable, mustRecompute}; cause.kind ∈ {noPriorEvidence, inputsChanged}; categories ⊆ F029 vocabulary (FR-011)"
                (fun (r: CacheEligibilityReport) ->
                    use doc = parse (CacheEligibilityJson.ofReport r)
            ...
            testPropertyWithConfig
                fscheckConfig
                "no string value is anything but a gate id / closed token / category token / opaque evidence ref"
                (fun (r: CacheEligibilityReport) ->
                    use doc = parse (CacheEligibilityJson.ofReport r)
                    ...
                    let evidenceVals =
                        ...
                        |> List.choose (fun e ->
                            ...
                            | FS.GG.Governance.CacheEligibility.Model.Reusable ref ->
                                Some(EvidenceReuse.referenceValue ref)
                            | _ -> None)
