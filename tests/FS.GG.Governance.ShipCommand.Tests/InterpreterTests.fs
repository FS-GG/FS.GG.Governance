...
let tests =
    testList
        ...
        [ // ── US1: roll up + persist; bytes = F025(F024 rollup); blocked/clean exit ──
            ...
            test
                "base-blocking change ⇒ audit bytes = F025(F024 rollup), verdict fail, exit 1 (US1 AS1, SC-001, SC-007)" {
                let git = gitWithChanges [ 'M', "src/Lib/Thing.fs" ]

                let req, cap, model =
                    runWith validCatalog git Loop.DefaultRange Loop.Text Gate Standard
                ...
                Expect.equal
                    (writtenAudit cap)
                    (Some(req.AuditOut, expected))
                    "audit.json = AuditJson.ofShipDecision (Ship.rollup …)"
            ...
            test
                "same change, two lever sets ⇒ two verdicts/exit codes; relaxed lands in warnings (US2 AS1/AS2, SC-003, FR-011)" {
                let git = gitWithChanges [ 'M', "src/Lib/Thing.fs" ]
                ...
                let _, _, relaxed =
                    runWith validCatalog git Loop.DefaultRange Loop.Text Inner Standard
            ...
            test "the applied mode/profile flow through F024→F025 and are recorded per item (US2 AS1, T023)" {
                ...
                Expect.equal
                    docA
                    (auditExpected validCatalog candidates Gate Standard (Some(snapshotOf git defaultOpts)))
                    "gate/standard bytes match the rollup"
                ...
                Expect.equal
                    docB
                    (auditExpected validCatalog candidates Inner Strict (Some(snapshotOf git defaultOpts)))
                    "inner/strict bytes match the rollup"
            ...
            test "--json stdout equals the persisted audit content exactly; text suppressed (US3 AS2)" {
                ...
                let _, capJson, _ =
                    runWith validCatalog git Loop.DefaultRange Loop.Json Gate Standard
            ...
            test "an empty changed-path set ⇒ valid empty-partition audit, verdict pass, exit 0 (SC-006)" {
                let _, cap, model =
                    runWith validCatalog gitEmpty Loop.DefaultRange Loop.Json Gate Standard
            ...
            test "a valid empty catalog ⇒ empty partition, verdict pass, exit 0 (SC-006)" {
                ...
                let expected =
                    auditExpected emptyCatalog candidates Gate Standard (Some(snapshotOf git defaultOpts))

                let req, cap, model =
                    runWith emptyCatalog git Loop.DefaultRange Loop.Json Gate Standard

                Expect.equal
                    (writtenAudit cap)
                    (Some(req.AuditOut, expected))
                    "audit.json = projection of the empty registry rollup"
            ...
            test "a routine-only change is a clean pass; blocking is decided by the cores, not the host (FR-012, T030a)" {
                ...
                Expect.equal
                    ...
                    (Some expected)
                    "host edge never decides blocking — bytes = cores' output"
            }
        ]
