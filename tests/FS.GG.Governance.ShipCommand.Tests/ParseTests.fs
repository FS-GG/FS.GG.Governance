    ...
    testList
        ...
        [ test "--paths a b ⇒ ExplicitPaths of the normalized paths (tolerates leading ship verb)" {
          ...
          test "CLI-5: a stray non-`--` positional ⇒ UnexpectedArgument, not UnknownFlag" {
              Expect.equal (parse [ "ship"; "junk" ]) (Error(Loop.UnexpectedArgument "junk")) "stray positional rejected as UnexpectedArgument"
          }
          ...
          test "--store with no value ⇒ a UsageError VALUE, never a throw" {
              Expect.equal (parse [ "ship"; "--store" ]) (Error(Loop.MissingValue "--store")) "missing --store value is a value"
          } ]
