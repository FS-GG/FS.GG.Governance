...
let private runLevers (g) (mode: RunMode) (profile: Profile) =
    ...
    let model =
        Interpreter.run (fakePortsExec validCatalog g fakeSensor absentStoreReader fakeExecPortPass cap req) req
    ...
    testList
        ...
        [
            ...
            test "audit.json is byte-identical with wiring active; the two sidecars are written beside it" {
                ...
                Interpreter.run
                    (fakePortsExec validCatalog gitSrcChange fakeSensor absentStoreReader fakeExecPort cap req)
                    req
                |> ignore
                ...
                | Some(_, doc) ->
                    Expect.equal
                        doc
                        (auditExpected validCatalog candidates Gate Standard snap)
                        "audit.json bytes unchanged (sidecars never fold in)"
            ...
            test "cost/cache findings enforce as Advisory and never enter audit.json (FR-008)" {
                ...
                | Some(_, adoc), Some _ ->
                    Expect.isFalse (adoc.Contains "syntheticTaint") "findings never leak into audit.json"
            ...
            test "an unreadable store still writes well-formed sidecars with no fabricated reuse" {
                ...
                let model =
                    Interpreter.run
                        (fakePortsExec validCatalog gitSrcChange fakeSensor malformedStoreReader fakeExecPort cap req)
                        req
