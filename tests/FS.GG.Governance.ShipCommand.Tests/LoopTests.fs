    ...
    testList
        ...
        [
            test "Sensed Ok sets Candidates + Snapshot and emits LoadCatalog (US2)" {
                ...
                Expect.equal
                    m1.Candidates
                    (Some(snap.Changed |> List.map (fun c -> c.Path)))
                    "candidates = snapshot changed paths"
            ...
            test "Loaded Valid rolls up the verdict and emits SenseFreshness + LoadStore (no write yet) (US2)" {
                ...
                Expect.equal
                    ...
                    "Decision = Ship.rollup of the same inputs/levers (verdict decided here)"
            ...
            test
                "the join fires once both senses arrive ⇒ auditDoc = ofShipDecision decision (Some report), one write (SC-002)" {
                let git = gitWithChanges [ 'M', "src/Lib/Thing.fs" ]
                let req = requestFor Loop.DefaultRange Loop.Text
                ...
                match e4 with
                | Loop.WriteArtifact(Loop.AuditArtifact, p, doc) :: Loop.WriteArtifact(Loop.CostBudgetArtifact, _, _) :: [ Loop.WriteArtifact(Loop.ProvenanceArtifact,
                                                                                                                                              _,
                                                                                                                                              _) ] ->
            ...
            test
                "Wrote Ok then Emitted reach Done; ExitCodeBasis Blocked ⇒ Blocked exit, UNCHANGED by cache (US2 AS1, SC-003)" {
                let git = gitWithChanges [ 'M', "src/Lib/Thing.fs" ]
                let req = requestFor Loop.DefaultRange Loop.Text
                let snap, m2, _ = toSelected git req
                ...
                match e5 with
                | [ Loop.EmitSummary(text, _, _) ] ->
                    Expect.equal text (Loop.render m4 req.Format) "write ack ⇒ EmitSummary with the rendered text"
            ...
            test
                "ExitCodeBasis Clean ⇒ Success exit; finding-only audit carries no cache verdict on any item (US2 AS2, E1)" {
            ...
            test "render Json equals the (cache-bearing) audit document verbatim and is pure (US2)" {
                ...
                Expect.equal
                    (Loop.render m4 Loop.Json)
                    (Option.get m4.AuditDoc)
                    "Json render = the F025 audit doc verbatim"

                Expect.stringContains
                    (Loop.render m4 Loop.Json)
                    "\"cacheEligibilityEvaluated\":true"
                    "the persisted doc carries the evaluated cache section"
