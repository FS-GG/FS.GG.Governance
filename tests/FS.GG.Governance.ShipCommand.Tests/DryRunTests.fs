...
let private dryReq format =
    { requestFor Loop.DefaultRange format with
        DryRun = true
    }
    ...
    testList
        ...
        [ // ── US1: parse ──
            ...
            test "--dry-runn (typo) ⇒ UnknownFlag, writes nothing" {
                Expect.equal
                    (parse [ "ship"; "--dry-runn" ])
                    (Error(Loop.UnknownFlag "--dry-runn"))
                    "typo is a usage error"
            }
            ...
            test "a real run over the SAME ports WOULD execute — proving dry-run's suppression is real" {
                ...
                Interpreter.run
                    (fakePortsExec validCatalog git fakeSensor absentStoreReader (countingExecPort counter 0) cap req)
                    req
                |> ignore
            ...
            test
                "classify: real⇒satisfied, pending/failed/skipped⇒absent, stale⇒absent, deferred/accepted/synthetic⇒not-required" {
            ...
            test "an all-absent handoff surfaces allNotEvaluated=true (the notEvaluated failure mode)" {
                ...
                Expect.stringContains
                    (List.head capT.Emits)
                    "all-not-evaluated"
                    "the text form surfaces the absence explicitly"
            }
            ...
            test "a malformed handoff surfaces a diagnostic and does NOT read as a bare Pass (FR-008)" {
                ...
                runDry Loop.Json fakeExecPort cap [ handoffRead "{ this is not valid json" ]
                |> ignore
            ...
            test
                "dry-run --json emits the DISTINCT dryrun schema + simulated marker; never the real audit schema (G1/G2/G5)" {
                let cap = newCapture ()
                runDry Loop.Json fakeExecPort cap [ handoffRead mixedJson ] |> ignore
                let json = List.head cap.Emits
                ...
                Expect.isFalse
                    (json.Contains "fsgg.audit/v2")
                    "the real audit schema id never appears in simulated output"

                Expect.equal
                    SimulateProjection.schemaVersion
                    "fsgg.audit.dryrun/v1"
                    "the projection's schema id is the distinct one"
            }
        ]
