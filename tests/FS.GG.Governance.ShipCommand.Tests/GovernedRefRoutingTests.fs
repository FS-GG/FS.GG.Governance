    ...
    testList
        ...
        [ // ── US1 V3 (SC-004): a declared surface flips the ship verdict to non-shippable ──
                ...
                let baseline =
                    runWith
                        ...
                        [
                            handoffRead (
                                sprintf
                                    """{ "contractVersion": "2.0.0", "schemaVersion": 1, "evidence": { "nodes": [ { "id": "build:lib", "state": "real" } ], "dependencies": [] }, "governedReferences": [] }"""
                            )
                        ]

                Expect.equal
                    (Option.get baseline.Decision).Verdict
                    Pass
                    "with no declared surface and no sensed change ⇒ Pass"
            ...
            test "V5a — no handoff ⇒ the audit doc is byte-identical to the pre-F082 projection (SC-002)" {
                ...
                let expected =
                    auditExpected validCatalog candidates Gate Standard (Some(snapshotOf gitSrcChange defaultOpts))

                Expect.equal
                    (Option.get m.AuditDoc)
                    expected
                    "the absent-handoff audit doc matches the pre-seam pipeline (the identity merge)"
            }
