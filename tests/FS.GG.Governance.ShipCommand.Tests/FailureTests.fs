    ...
    testList
        ...
        [
            ...
            test "unresolved --since revision ⇒ InputUnavailable (exit 3), distinct diagnostic, no write (US4 AS1)" {
                ...
                let model =
                    Interpreter.run (fakePorts validCatalog (gitUnknownRev "nope") cap req) req
            ...
            test
                "unwritable output after a valid rollup ⇒ ToolError (exit 4), never Blocked, no partial artifact (US4 AS4)" {
                ...
                let model =
                    Interpreter.run (fakePortsFailingWrites validCatalog gitEmpty cap failing req) req
            ...
            test "the four tool-failure codes are mutually distinct and none equals the blocked code 1 (FR-009, SC-004)" {
                ...
                for c in codes do
                    Expect.notEqual
                        c
                        blocked
                        (sprintf "tool-failure code %d must differ from the blocked code %d" c blocked)
            ...
            test "the failure diagnostics are DISTINCT across categories (US4, SC-004)" {
                ...
                let badCatalog =
                    diagOf Map.empty gitEmpty (Loop.ExplicitPaths [ gp "src/Lib/Thing.fs" ]) Set.empty

                let unknownRev =
                    diagOf validCatalog (gitUnknownRev "nope") (Loop.Since "nope") Set.empty
                ...
                let unwritable =
                    diagOf
                        validCatalog
                        gitEmpty
                        (Loop.ExplicitPaths [ gp "src/Lib/Thing.fs" ])
                        (Set.ofList [ writeReq.AuditOut ])
