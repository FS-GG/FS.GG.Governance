        ...
        [
            test "SC-001: a seeded non-empty v1 store round-trips losslessly through the real reader" {
                withTempRepo (fun dir ->
                    ...
                    let req =
                        parseOrFail [ "ship"; "--repo"; dir; "--since"; "HEAD~1"; "--persist-store" ]
                    ...
                    Expect.equal
                        (readStore req.StorePath)
                        (Some(expectedPersistedRepo dir store))
                        "persisted store = pipeline over the grown store")
            }
            ...
            test "AC-2: an absent store ⇒ a well-formed empty v1 is written, re-read as the empty store" {
                withTempRepo (fun dir ->
                    ...
                    Expect.equal
                        (readStore req.StorePath)
                        (Some(expectedPersistedRepo dir EvidenceReuse.empty))
                        "re-reads as the grown store")
            }
            ...
            test "SC-003: an over-bound store persists within defaultRetentionBound, newest-first, survivors ⊆ loaded" {
                withTempRepo (fun dir ->
                    let entries =
                        [
                            for n in 1 .. (EvidenceReuseStore.defaultRetentionBound + 7) ->
                    ...
                    Expect.isLessThanOrEqual
                        (List.length persistedEntries)
                        EvidenceReuseStore.defaultRetentionBound
                        "within bound"

                    Expect.equal
                        persisted
                        (expectedPersistedRepo dir loaded)
                        "persisted = F047 pipeline over the grown store")
            }
            ...
            test "SC-003: a strictly-superseded entry is pruned" {
                withTempRepo (fun dir ->
                    let newer =
                        {
                            Inputs = persistInputs "format" "h1"
                            Evidence = syntheticRef "new"
                        }

                    let older =
                        {
                            Inputs = persistInputs "format" "h1"
                            Evidence = syntheticRef "old"
                        }

                    writeFile
                        dir
                        "readiness/evidence-reuse.json"
                        (EvidenceReuseStore.serialise (ReuseStore [ newer; older ]))
                    ...
                    Expect.isFalse
                        (EvidenceReuse.entries persisted |> List.contains older)
                        "the superseded (older) entry is pruned"

                    Expect.equal
                        persisted
                        (expectedPersistedRepo dir (ReuseStore [ newer; older ]))
                        "persisted = F047 pipeline over the grown store")
            }
            ...
            test "SC-004: verdict, exit, and audit.json are byte-for-byte identical with vs without --persist-store" {
                withTempRepo (fun dir ->
                    ...
                    Expect.equal
                        auditOn
                        auditOff
                        "audit.json is byte-identical (verdict decoupled from the store write)"
                    ...
                    Expect.equal
                        modelOn.Decision
                        modelOff.Decision
                        "the ship verdict/partition/enforcement is unchanged")
            }

            test
                "SC-005: an unwritable store target is non-fatal — exit unchanged, audit.json intact, no partial file, a note" {
                withTempRepo (fun dir ->
                    ...
                    Expect.equal
                        (File.ReadAllText req.AuditOut)
                        auditBase
                        "audit.json is unchanged by the failed store write"
    ...
    testList
        ...
        [
            test "ship.json byte-identical to the frozen pre-wiring golden (5a0cb28)" {
                let tmp =
                    Path.Combine(Path.GetTempPath(), "fsgg-golden-ship-" + System.Guid.NewGuid().ToString("N"))
