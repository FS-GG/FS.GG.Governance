...
let tests =
    testList
        ...
        [
            test
                "real git + real catalog: --since HEAD~1 under gate/standard ⇒ fail/blocked, exit 1, bytes = F025 rollup, re-run identical" {
                withTempRepo (fun dir ->
                    ...
                    let expected =
                        auditExpected
                            ...
                            Gate
                            Standard
                            (Some(snapshotOfRepo dir (sinceOpts "HEAD~1")))
                    ...
                    Expect.equal
                        ...
                        expected
                        "audit.json bytes = AuditJson.ofShipDecision (Ship.rollup …) (SC-001)"
            ...
            test "real git: a relaxing run mode lands the same change clean (exit 0), no-hide warnings" {
                withTempRepo (fun dir ->
                    ...
                    Expect.isNonEmpty
                        (Option.get model.Decision).Warnings
                        "the base-blocking gates relax to warnings (no-hide)"
            ...
            test "F070: a configured stale generated view ⇒ Fail/Blocked + a generatedViews blocker in audit.json" {
                withTempRepo (fun dir ->
                    ...
                    Expect.equal
                        (Option.get model.Decision).Verdict
                        Fail
                        "a stale generated view at block-on-ship ⇒ Fail"
                    ...
                    Expect.stringContains
                        ...
                        "the stale view blocks at the gate boundary")
            }
        ]
