...
let tests =
    testList
        ...
        [
            test
                "SC-003: Some report vs None — every NON-cache byte of audit.json is identical (verdict/partition/enforcement/basis)" {
                ...
                Expect.equal
                    ...
                    "every non-cache byte (verdict/partition/enforcement/basis) is identical (SC-003)"
            }

            test
                "SC-003: a REUSABLE verdict on a base-blocking gate leaves it a blocker (cache never moves the partition)" {
                let decision, selectedGates, baseHead = fixture ()
                ...
                let store =
                    EvidenceReuse.record (List.head candidates).Inputs (EvidenceRef "ev-reuse-1") EvidenceReuse.empty
                ...
                Expect.equal
                    (withoutCache withReusable)
                    (withoutCache withNone)
                    "a reusable verdict changes NOTHING the merge decision depends on (SC-003)"
            }
            ...
            test "FR-013: no raw freshness input / hash / freshness key leaks into audit.json (C1, L6)" {
                ...
                for tok in [ "rule-synthetic"; "gen-synthetic"; "art-synthetic"; "cmd-synthetic" ] do
                    Expect.isFalse
                        ...
                        (sprintf "raw freshness input %s must not appear in audit.json" tok)
