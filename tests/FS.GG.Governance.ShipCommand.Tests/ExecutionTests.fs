    ...
    testList
        ...
        [
            test "US1: a selected blocking gate that EXITS 0 is relocated to Passing ⇒ verdict Pass / exit 0 (SC-002)" {
                ...
                let model =
                    Interpreter.run
                        (fakePortsExec validCatalog git fakeSensor absentStoreReader fakeExecPortPass cap req)
                        req
            ...
            test "US1: a selected blocking gate that EXITS non-zero stays a Blocker ⇒ verdict Fail / exit 1 (SC-001)" {
                ...
                let model =
                    Interpreter.run
                        (fakePortsExec validCatalog git fakeSensor absentStoreReader fakeExecPort cap req)
                        req
            ...
            test "US2: a reusable gate is NOT spawned a second time (the cache payoff, SC-003)" {
                withTempRepo (fun dir ->
                    ...
                    Expect.equal
                        (Option.get model2.Decision).Verdict
                        Pass
                        "the reused passing outcome drives the verdict")
            }
    ...
    testList
        ...
        [
            ...
            test "relocation never MOVES a finding and never CREATES a blocker (FR-006)" {
                ...
                let findingsBefore =
                    (decision.Blockers @ decision.Warnings) |> List.filter isFinding
                ...
                Expect.isLessThanOrEqual
                    (List.length relocated.Blockers)
                    (List.length decision.Blockers)
                    "blockers only shrink"
                // Findings that were blockers/warnings are untouched (never relocated).
                let findingsAfter =
                    (relocated.Blockers @ relocated.Warnings) |> List.filter isFinding
