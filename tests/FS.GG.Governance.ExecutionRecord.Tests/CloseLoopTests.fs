    ...
    testList
        ...
        [ // (1) canonicalId + F049 referenceOf reproducible (SC-001, FR-007, US1 acceptance 1)
            ...
            test "a one-byte change to either output stream flips the reference (changed output never served as fresh)" {
                ...
                let outChanged =
                    Build.outcome (stdout = System.Text.Encoding.UTF8.GetBytes "out-bytez")

                let errChanged =
                    Build.outcome (stderr = System.Text.Encoding.UTF8.GetBytes "err-bytez")
            ...
            testPropertyWithConfig
                fscheckConfig
                "distinct reproducible facts give distinct references (injectivity over arbitrary outcomes)"
