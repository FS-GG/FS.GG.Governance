    ...
    testList
        ...
        [ // (1) content agreement — SC-002 / FR-002 / US2 acceptance 1
            ...
            test "a binary (non-UTF-8) buffer digests to a fixed-form lowercase-hex digest, never throwing" {
                ...
                Expect.isTrue
                    (s |> Seq.forall (fun c -> System.Char.IsDigit c || (c >= 'a' && c <= 'f')))
                    "lowercase hex"
            }
            ...
            <| fun (bytes: byte[]) -> digestOf bytes = digestOf bytes && digestOf bytes = digestOf (Array.copy bytes)
            ...
            test "equal stdout and stderr bytes yield equal digests (content alone)" {
                ...
                Expect.equal
                    (digestOf shared)
                    (digestOf (Array.copy shared))
                    "equal content on both streams => equal digests"
            }
        ]
