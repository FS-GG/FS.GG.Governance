...
let tests =
    testList
        ...
        [
            test
                "worked example B: command-less gate, fully sensed ⇒ Resolved with Command=None and CommandVersion=None (SC-003)" {
            ...
            test
                "command-less gate resolves even when CommandVersions carries OTHER commands (never MissingCommandVersion)" {
            ...
            test "command-less gate with EMPTY CommandVersions is never MissingCommandVersion" {
                ...
                | Unresolved facts ->
                    Expect.isFalse
                        (List.contains MissingCommandVersion facts)
                        "MissingCommandVersion impossible for a command-less gate"
            }

            testPropertyWithConfig
                fscheckConfig
                "command-less gate: fully sensed ⇒ Resolved, CommandVersion=None, never MissingCommandVersion"
            <| fun (g: Gate) ->
                // Force the gate command-less (identity + key), keeping everything else.
                let gLess =
                    { g with
                        FreshnessKey = { g.FreshnessKey with Command = None }
                    }
