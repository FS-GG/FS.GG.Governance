...
let tests =
    testList
        ...
        [
                ...
                | [ e ] ->
                    ...
                    | Resolved i ->
                        Expect.equal
                            ...
                            (Some { Gate = e.Gate; Inputs = i })
                            "candidate pairs the entry's gate with its resolved inputs"
            ...
            test
                "resolved candidates are accepted by real CacheEligibility.evaluate without adaptation ⇒ one verdict per resolved gate (SC-007)" {
            ...
            test "a recorded resolved candidate is deemed Reusable by F041 (the bridge round-trips to a real verdict)" {
                ...
                | [ c ] ->
                    ...
                    | [ e ] ->
                        Expect.isTrue
                            (CacheEligibility.isReusable e.Verdict)
                            "an exact-match resolved candidate is Reusable in F041"
            ...
            testPropertyWithConfig
                fscheckConfig
                "choose candidate over a mixed report has exactly as many elements as there are Resolved entries (FR-004)"
            ...
            testPropertyWithConfig
                fscheckConfig
                "every produced candidate is accepted by F041 ⇒ one verdict per candidate (no adaptation)"
            <| fun (gs: Gate list) (s: SensedFacts) ->
                let cands =
