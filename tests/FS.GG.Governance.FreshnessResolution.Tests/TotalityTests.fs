    ...
    testList
        ...
        [
            testPropertyWithConfig
                fscheckConfig
                "resolve returns a well-formed report and never throws over the full cross-product (SC-004, L-total)"
            <| fun (gs: Gate list) (s: SensedFacts) ->
                ...
                && es
                   |> List.forall (fun e ->
                       FreshnessResolution.isResolved e.Outcome
                       || not (List.isEmpty (FreshnessResolution.missingFacts e.Outcome)))

            test "empty gate list ⇒ empty report, a valid success not an error (Edge)" {
                Expect.equal
                    (FreshnessResolution.resolve [] fullSensed)
                    (FreshnessResolutionReport [])
                    "resolve [] sensed = FreshnessResolutionReport []"

                Expect.equal
                    (FreshnessResolution.entries (FreshnessResolution.resolve [] emptySensed))
                    []
            ...
            test "single gate path equals the many-gate path (one gate ⇒ one-entry report)" {
                let one =
                    FreshnessResolution.entries (FreshnessResolution.resolve [ gBuildTests ] fullSensed)
