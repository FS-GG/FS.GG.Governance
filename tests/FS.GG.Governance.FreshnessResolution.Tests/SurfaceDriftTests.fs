...
let private freshnessResolutionAsm =
    SurfaceDrift.assemblyNamed "FS.GG.Governance.FreshnessResolution"
    ...
    testList
        ...
        [
            ...
            test "the public surface is exactly the two modules (Model + FreshnessResolution), nothing else" {
                let typeNames =
                    freshnessResolutionAsm.GetExportedTypes()
                    |> Array.choose (fun t -> Option.ofObj t.FullName)

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.FreshnessResolution.ModelModule"))
                    "Model module is public"

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n ->
                         n.EndsWith "FS.GG.Governance.FreshnessResolution.FreshnessResolutionModule"))
