    ...
    testList
        ...
        [
            test "present-empty covered key ⇒ Resolved with CoveredArtifacts = [] (a legitimate sensed value)" {
                let presentEmpty =
                    { senseFully gBuildTests with
                        CoveredArtifacts = Map.ofList [ gBuildTests.Id, [] ]
                    }
            ...
            test "absent covered key ⇒ Unresolved [MissingCoveredArtifacts] (not sensed)" {
                ...
                Expect.equal
                    (onlyOutcome [ gBuildTests ] absent)
                    (Unresolved [ MissingCoveredArtifacts ])
                    "an absent key is unresolved on covered artifacts only"
            }
