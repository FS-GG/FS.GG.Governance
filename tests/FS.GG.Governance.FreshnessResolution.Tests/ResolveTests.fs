    ...
    testList
        ...
        [
            test "worked example A: fully-sensed build:tests ⇒ Resolved with carried identity + sensed facts (SC-001)" {
                Expect.equal
                    (onlyOutcome [ gBuildTests ] fullSensed)
                    (Resolved expectedA)
                    "carry: identity (cost dropped) + sensed facts, verbatim"
            }
            ...
            test "the four identity fields equal the carried FreshnessKey verbatim, Command option preserved" {
                ...
                | Resolved i ->
                    ...
                    Expect.equal
                        i.Environment
                        gBuildTests.FreshnessKey.Environment
                        "Environment from carried FreshnessKey"
            ...
            test "the six sensed fields equal the supplied SensedFacts verbatim" {
                ...
                | Resolved i ->
                    ...
                    Expect.equal
                        (Some i.GeneratorVersion)
                        fullSensed.GeneratorVersion
                        "GeneratorVersion sensed verbatim"
            ...
            test "Cost is DROPPED: two gates differing ONLY in Cost resolve to the same FreshnessInputs (SC-001)" {
                ...
                Expect.equal
                    (onlyOutcome [ cheap ] fullSensed)
                    (onlyOutcome [ exhaustive ] fullSensed)
                    "Cost does not affect the resolved FreshnessInputs"
            }

            testPropertyWithConfig
                fscheckConfig
                "carry property: any fully-sensed gate resolves to its carried identity + sensed facts, zero fabrication"
            <| fun (g: Gate) ->
                let s = senseFully g
            ...
            testPropertyWithConfig
                fscheckConfig
                "Cost-drop property: changing only Cost never changes the resolved value"
            <| fun (g: Gate) ->
