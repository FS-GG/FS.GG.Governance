    ...
    testList
        ...
        [
            testPropertyWithConfig
                fscheckConfig
                "exactly one entry per input gate; the GateId multiset equals the input multiset (L-complete)"
            ...
            test "worked example E: two gates sharing a GateId ⇒ TWO entries, neither merged nor dropped" {
                ...
                let es =
                    FreshnessResolution.entries (FreshnessResolution.resolve [ g1; g2 ] fullSensed)

                Expect.equal (List.length es) 2 "two same-id gates ⇒ two entries"

                Expect.allEqual
                    (es |> List.map (fun e -> e.Gate))
                    (gid "build" "tests")
                    "both entries attributed to the shared GateId"
            }

            test
                "tiebreak is a total order: same GateId, DISTINCT outcomes ⇒ byte-identical report for either input order" {
                ...
                let unresolvedGate =
                    gateWith "build" "tests" Medium Ci (Some(CommandId "absent-cmd"))
            ...
            testPropertyWithConfig
                fscheckConfig
                "duplicate-inducing inputs are never deduplicated — entry count always equals input count"
            <| fun (gs: Gate list) (s: SensedFacts) ->
