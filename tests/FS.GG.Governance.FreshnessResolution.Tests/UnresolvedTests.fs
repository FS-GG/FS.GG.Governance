    ...
    testList
        ...
        [
            test "single dropped fact ⇒ Unresolved [thatFact] and NO FreshnessInputs (SC-002, no-fabricate)" {
                ...
                for (fact, drop) in gapTable gLintStyle do
                    ...
                    Expect.equal
                        ...
                        [ fact ]
                        (sprintf "dropping %A names exactly that fact" fact)
            }
            ...
            test "worked example C tokens are ruleHash/coveredArtifacts/commandVersion/baseRevision" {
                ...
                Expect.equal
                    tokens
                    [ "ruleHash"; "coveredArtifacts"; "commandVersion"; "baseRevision" ]
                    "no-hide tokens for example C"
            }
            ...
            test "an Unresolved outcome carries ONLY the MissingFact list — no fabricated hash/version/revision" {
                ...
                Expect.equal
                    ...
                    None
                    "no candidate ⇒ no fabricated inputs"
            ...
            test "all six missingFactToken values are exact and pairwise distinct (stable, injective wire vocabulary)" {
                ...
                Expect.equal
                    (FreshnessResolution.missingFactToken MissingCoveredArtifacts)
                    "coveredArtifacts"
                    "coveredArtifacts"

                Expect.equal
                    (FreshnessResolution.missingFactToken MissingCommandVersion)
                    "commandVersion"
                    "commandVersion"

                Expect.equal
                    (FreshnessResolution.missingFactToken MissingGeneratorVersion)
                    "generatorVersion"
                    "generatorVersion"
            ...
            testPropertyWithConfig
                fscheckConfig
                "no-hide property: missingFacts equals EXACTLY the omitted facts (enum order), neither subset nor superset"
            <| fun (g: Gate) (s: SensedFacts) ->
                FreshnessResolution.missingFacts (onlyOutcome [ g ] s) = expectedMissing g s

            testPropertyWithConfig
                fscheckConfig
                "a partially-sensed gate is Unresolved iff at least one required fact is missing, and then names ≥1 gap"
            <| fun (g: Gate) (s: SensedFacts) ->
