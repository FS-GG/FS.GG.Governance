...
let rec private copyDirectory source destination =
    ...
    for file in Directory.GetFiles source do
        let name =
            ...
            | null -> failwith "fixture file has no name"
            | value -> value
    ...
    for directory in Directory.GetDirectories source do
        let name =
            ...
            | null -> failwith "fixture directory has no name"
            | value -> value
...
let private writeNuGetConfig path feed =
    let escapedFeed =
        match SecurityElement.Escape feed with
        | null -> failwith "feed path could not be escaped"
        | value -> value
...
let private packagePath feed id version =
    Path.Combine(feed, $"{id}.{version}.nupkg")
...
let private inspectSpiPackage nupkg =
    ...
    Expect.contains
        entries
        "lib/net10.0/FS.GG.Governance.Adapters.Spi.dll"
        "SPI assembly is a compile-time package asset"

    Expect.isFalse
        (entries
         |> List.exists (fun name -> name.Contains("CommandHost", StringComparison.OrdinalIgnoreCase)))
        "command-host assemblies are absent"
    ...
    let dependencies =
        ...
        |> Seq.map (fun node ->
            let id =
                match node.Attribute(XName.Get "id") with
                | null -> failtest "nuspec dependency has no id"
                | value -> value.Value

            let version =
                match node.Attribute(XName.Get "version") with
                | null -> failtestf "nuspec dependency %s has no version" id
                | value -> value.Value

            id, version)
        |> Seq.toList

    let governanceDependencies =
        dependencies
        |> List.filter (fun (id, _) -> id.StartsWith("FS.GG.Governance.", StringComparison.Ordinal))

    Expect.equal
        governanceDependencies
        [ "FS.GG.Governance.Kernel", "[0.1.1]" ]
        "Kernel is the sole exact Governance package dependency"

    Expect.isFalse
        (dependencies
         |> List.exists (fun (id, _) -> id.Contains("Command", StringComparison.OrdinalIgnoreCase)))
        "command packages are absent"

    Expect.isFalse
        (dependencies
         |> List.exists (fun (id, _) -> id.Contains("Host", StringComparison.OrdinalIgnoreCase)))
        "host packages are absent"
...
let packageTests =
    testList
        "PackageConsumer"
        [
            testCase "package-only locked consumer and negative package mutation"
            <| fun _ ->
                let repoRoot = Path.GetFullPath(Path.Combine(__SOURCE_DIRECTORY__, "../.."))

                let fixtureSource =
                    Path.Combine(repoRoot, "tests/fixtures/adapter-spi-package-consumer")

                let tempRoot =
                    Path.Combine(Path.GetTempPath(), "fsgg-adapter-spi-package-" + Guid.NewGuid().ToString("N"))
                ...
                try
                    ...
                    let kernelProject =
                        Path.Combine(repoRoot, "src/FS.GG.Governance.Kernel/FS.GG.Governance.Kernel.fsproj")

                    let spiProject =
                        Path.Combine(repoRoot, "src/FS.GG.Governance.Adapters.Spi/FS.GG.Governance.Adapters.Spi.fsproj")

                    requireGreen
                        "pack Kernel"
                        (run repoRoot [] [ "pack"; kernelProject; "-c"; "Release"; "--no-restore"; "-o"; feed ])

                    requireGreen
                        "pack adapter SPI"
                        (run repoRoot [] [ "pack"; spiProject; "-c"; "Release"; "--no-restore"; "-o"; feed ])
                    ...
                    requireGreen
                        ...
                        (run
                            consumer
                            [ "NUGET_PACKAGES", seedPackages ]
                            [ "restore"; project; "--use-lock-file"; "--configfile"; config ])

                    Expect.isTrue
                        (File.Exists(Path.Combine(consumer, "packages.lock.json")))
                        "consumer lock was generated"
                    ...
                    requireGreen
                        "cold locked consumer restore"
                        (run
                            consumer
                            [ "NUGET_PACKAGES", lockedPackages ]
                            [ "restore"; project; "--locked-mode"; "--configfile"; config ])
                    ...
                    requireGreen
                        ...
                        (run
                            consumer
                            [ "NUGET_PACKAGES", lockedPackages ]
                            [ "build"; project; "--no-restore"; "-c"; "Release" ])
                    ...
                    let expected =
                        File.ReadAllText(Path.Combine(consumer, "expected.txt")).Replace("\r\n", "\n").Trim()

                    let actual = execution.Output.Replace("\r\n", "\n").Trim()
                    Expect.equal actual expected "package-only consumer projection is stable"

                    let kernelName =
                        match Path.GetFileName kernelPackage with
                        | null -> failwith "Kernel package has no file name"
                        | value -> value

                    let spiName =
                        match Path.GetFileName spiPackage with
                        | null -> failwith "SPI package has no file name"
                        | value -> value

                    requireGreen
                        "pack non-matching Kernel"
                        (run
                            repoRoot
                            []
                            [
                    ...
                    let substitution =
                        run
                            substitutionConsumer
                            [ "NUGET_PACKAGES", substitutionPackages ]
                            [
                                "restore"
                                Path.Combine(substitutionConsumer, "AdapterSpiConsumer.fsproj")
                                "--use-lock-file"
                                "--configfile"
                                substitutionConfig
                            ]
                    ...
                    Expect.notEqual
                        substitution.ExitCode
                        0
                        "Kernel 0.1.2 must not substitute for the exact 0.1.1 dependency"

                    Expect.stringContains
                        substitution.Output
                        "NU1102"
                        "exact Kernel resolution rejects a feed without version 0.1.1"
                    ...
                    let negative =
                        run
                            negativeConsumer
                            [ "NUGET_PACKAGES", negativePackages ]
                            [
                                "restore"
                                negativeProject
                                "--use-lock-file"
                    ...
                    Expect.notEqual
                        negative.ExitCode
                        0
                        "a package whose compile-time SPI assembly was removed must fail fresh restore"

                    Expect.stringContains
                        negative.Output
                        "NU1202"
                        "the subject mutation is rejected because the SPI has no compatible compile surface"
                    ...
                    requireGreen
                        "reattached compile control"
                        (run
                            negativeConsumer
                            [ "NUGET_PACKAGES", negativePackages ]
                            [ "build"; negativeProject; "--no-restore"; "-c"; "Release" ])
                finally
