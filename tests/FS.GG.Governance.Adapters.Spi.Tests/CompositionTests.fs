...
let private factKey (fs: FactSet<ProjectFact>) =
    fs
    |> List.map (fun f -> factIdStr f.Id, f.Value, f.Provenance)
    ...
    testList
        ...
        [
            ...
            test "V67 cross-domain Implies couples two domains and a blocking result wins under F07 — Synthetic" {
                ...
                Expect.contains
                    outcomes
                    (Decided(RuleId "doc-implies-task-gov", Pass))
                    "cross-domain rule decides Pass when coupled"
            ...
            <| fun (docTitled: bool) (taskClosed: bool) ->
                let supplied: FactSet<ProjectFact> =
                    [
                        {
                            Id = FactId "d"
                            Value = Doc(HasTitle docTitled)
                            Provenance = []
                        }
                        {
                            Id = FactId "t"
                            Value = Task(TaskClosed taskClosed)
                            Provenance = []
                        }
                    ]
            ...
            test "V73 composition edge cases — duplicate fences, single adapter, minimal adapter — Synthetic" {
                ...
                let fenceA: Fence<ProjectChange> =
                    {
                        Name = "shared"
                        Trips = fun c -> c.Docs.Contains "x"
                    }

                let fenceB: Fence<ProjectChange> =
                    {
                        Name = "shared"
                        Trips = fun c -> c.Tasks.Contains "y"
                    }
                ...
                let route =
                    Route.route composedDup.Fences [] Gate { Docs = set [ "x" ]; Tasks = set [] }
                ...
                let bigSupplied: FactSet<ProjectFact> =
                    [
                        {
                            Id = FactId "f"
                            Value = Doc(HasTitle true)
                ...
                let composedEmpty =
                    Composition.compose [ Composition.lift (|DocP|_|) narrowDoc emptyAdapter ] []
            ...
            test "V69 removal — drop one adapter; remaining intact; cross-domain rule inert — Synthetic" {
                ...
                let result =
                    FixedPoint.evaluate projIdentify (Composition.toRules projBridge withoutDoc) supplied
                ...
                Expect.contains
                    outcomes
                    (Decided(RuleId "doc-implies-task-gov", Pass))
                    "cross-domain rule is inert (vacuous Pass), not an error"
            }
            ...
            test "V70 two unrelated domains each govern themselves with their own vocabulary — Synthetic" {
                ...
                Expect.isTrue
                    (docResult.Facts
                     |> List.exists (fun f -> f.Value = DocGov(Decided(RuleId "doc-titled", Pass))))
                ...
                Expect.isTrue
                    (taskResult.Facts
                     |> List.exists (fun f -> f.Value = TaskGov(Decided(RuleId "task-closed", Pass))))
            ...
            test "V71 two unrelated domains compose at one root without reshaping — Synthetic" {
                ...
                Expect.contains
                    outcomes
                    (Decided(RuleId "doc-titled", Pass))
                    "the document rule fires in the composed root"

                Expect.contains
                    outcomes
                    (Decided(RuleId "task-closed", Pass))
                    "the task rule fires in the composed root"
            }
        ]
