...
let private sortFacts (fs: FactSet<'a>) =
    fs |> List.sortBy (fun f -> factIdStr f.Id)
    ...
    testList
        ...
        [
            ...
            test "V62 standalone adapter governs itself through kernel entry points — Synthetic" {
                ...
                let outcomes =
                    result.Facts |> List.choose (fun f -> docAdapter.Bridge.Project f.Value)
            ...
            test "V64 lifting is render/hash/reads/isReified invariant — Synthetic" {
                for rule in docAdapter.Rules do
                    ...
                    Expect.equal
                        (Check.hash lc)
                        (Check.hash c)
                        (sprintf "hash invariant (cache key stable) for %A" rule.Id)
            ...
            test "V63 lifting is verdict+provenance faithful for 100% of rules — Synthetic" {
                ...
                Expect.equal
                    (sortFacts liftedResult.Facts)
                    (sortFacts expectedBig)
                    "lifted facts equal injected standalone facts"
            ...
            test "V65 lifted Opaque/AgentReviewed stays out of Deterministic and routes to review — Synthetic" {
                ...
                Expect.isFalse
                    (Check.isReified lifted.Check)
                    "lifted Opaque check stays opaque (cannot be Deterministic)"
