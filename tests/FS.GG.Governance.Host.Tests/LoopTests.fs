        ...
        [
            ...
            test "V51 accept freezes a policy-meeting sample set" {
                ...
                Expect.equal
                    (Loop.accept
                        (Agreement 2)
                        [ { Verdict = Pass; Confidence = 0.5 }; { Verdict = Pass; Confidence = 0.5 } ])
                    (Freeze Pass)
                    "agreement met"
            ...
            test "V52 accept stays pending below policy and never launders noise" {
                ...
                Expect.equal
                    (Loop.accept (Agreement 2) [ { Verdict = Pass; Confidence = 0.9 } ])
                    StayPending
                    "agreement short"
            ...
            test "V52-update a below-policy Reviewed records nothing and stays pending" {
                ...
                let m3, eff3 =
                    Loop.update config (Reviewed(key, Ok [ { Verdict = Pass; Confidence = 1.0 } ])) m2

                Expect.isFalse
                    (eff3
                     |> List.exists (function
                         | RecordVerdict _ -> true
                         | _ -> false))
                    "no RecordVerdict"
                ...
                let m3b, eff3b =
                    Loop.update
                        ...
                        (Reviewed(
                            key,
                            Ok [ { Verdict = Pass; Confidence = 1.0 }; { Verdict = Pass; Confidence = 1.0 } ]
                        ))
                        m2
            ...
            test "V57 an injection-laden artifact never alters the reviewer instruction" {
                ...
                Expect.isFalse
                    (honest.Task.Instruction.Contains "ignore your instructions")
                    "no injection in instruction"

                let taintedData =
                    tainted.Task.Data |> List.map (fun d -> d.Content) |> String.concat "\n"
