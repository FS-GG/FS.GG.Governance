    ...
    testList
        ...
        [
            ...
            testPropertyWithConfig
                fsCheckConfig
                "rollup never throws — returns a decision for every input (SC-005)"
            ...
            testPropertyWithConfig
                fsCheckConfig
                "partition law: |B|+|W|+|P| = N+M, disjoint, exhaustive (SC-006)"
                (fun (route, mode, profile) ->
                    let d = rollup route mode profile
            ...
            testPropertyWithConfig
                fsCheckConfig
                "each output list is sorted ascending by the composite key (FR-009)"
                (fun (route, mode, profile) ->
                    let d = rollup route mode profile
                    let keys (items: EnforcedItem list) = items |> List.map expectedKey
                    let sortedAsc (xs: string list) = xs = List.sort xs

                    sortedAsc (keys d.Blockers)
                    && sortedAsc (keys d.Warnings)
                    && sortedAsc (keys d.Passing))
        ]
