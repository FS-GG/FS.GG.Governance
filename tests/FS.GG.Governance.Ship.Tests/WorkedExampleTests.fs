...
let private gate id maturity =
    mkSelectedGate (mkGate (GateId id) maturity)
    ...
    testList
        ...
        [
            test "worked example at inner/light is a self-explaining warning, not a blocker (SC-002)" {
                ...
                let shipItem =
                    d.Warnings |> List.find (fun i -> i.Id = GateItem(GateId "build:ship"))
                ...
                Expect.stringContains
                    (shipItem.Decision.Reason.ToLowerInvariant())
                    "gate"
                    "reason names the 'gate' boundary"
            ...
            test "same-rollup blocker + warning at gate/light (SC-002 second half)" {
                let route =
                    mkRoute [ gate "build:ship" BlockOnShip; gate "build:rel" BlockOnRelease ] []
                ...
                Expect.equal
                    (d.Blockers |> List.map (fun i -> i.Id))
                    [ GateItem(GateId "build:ship") ]
                    "block-on-ship is the blocker"

                Expect.equal
                    (d.Warnings |> List.map (fun i -> i.Id))
                    [ GateItem(GateId "build:rel") ]
                    "block-on-release is the warning"
            }
                ...
                for mode in allModes do
                    for profile in allProfiles do
                        ...
                        for item in d.Blockers @ d.Warnings @ d.Passing do
                            ...
                            Expect.equal
                                item.Decision.BaseSeverity
                                expectedBase
                                (sprintf "base carry for %A at %A/%A" item.Id mode profile)
            }
        ]
