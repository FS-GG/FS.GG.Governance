    ...
    testList
        ...
        [
            testPropertyWithConfig
                fscheckConfig
                "evaluate returns a report and never throws over the full cross-product (SC-004, L-T1)"
            ...
            testPropertyWithConfig
                fscheckConfig
                "evaluateGate returns a verdict and never throws over the full cross-product (SC-004, L-T1)"
            ...
            test "the named edge cases all yield ordinary reports (no throw)" {
                ...
                Expect.equal
                    ...
                    "no candidates ⇒ empty report"
                // One candidate, matching store ⇒ one Reusable entry.
                Expect.equal
                    (CacheEligibility.entries (CacheEligibility.evaluate [ c ] store) |> List.length)
                    1
                ...
                let dup =
                    [
                        ...
                        candidate
                            (gid "build" "tests")
                            { baseInputs with
                                RuleHash = FS.GG.Governance.FreshnessKey.Model.RuleHash "r9"
                            }
                    ]

                Expect.equal
                    (CacheEligibility.entries (CacheEligibility.evaluate dup EvidenceReuse.empty)
                     |> List.length)
                    2
                    "duplicate gate ids ⇒ two entries"
            }
        ]
