...
let private entryKey (e: CacheEligibilityEntry) =
    (gateIdValue e.Gate, sprintf "%A" e.Verdict)
    ...
    testList
        ...
        [
            testPropertyWithConfig
                fscheckConfig
                "exactly one attributed verdict per candidate, none dropped/merged/duplicated (US3 #1, SC-006, L-E1)"
            <| fun (cs: CandidateGate list) (s: ReuseStore) ->
                ...
                let expected =
                    ...
                    |> List.map (fun c ->
                        {
                            Gate = c.Gate
                            Verdict = CacheEligibility.evaluateGate c s
                        })
            ...
            test "worked example: z:a, a:b, a:a ⇒ ordered a:a, a:b, z:a (ordinal) (US3 #2, SC-006, L-E2)" {
                let cs =
                    [
                        ...
                        candidate (gid "a" "b") baseInputs
                        candidate (gid "a" "a") baseInputs
                    ]

                let order =
                    CacheEligibility.entries (CacheEligibility.evaluate cs EvidenceReuse.empty)
                    |> List.map (fun e -> gateIdValue e.Gate)
            ...
            testPropertyWithConfig
                fscheckConfig
                "entries are in non-decreasing GateId ordinal order (US3 #2, SC-006, L-E2)"
            <| fun (cs: CandidateGate list) (s: ReuseStore) ->
            ...
            testPropertyWithConfig
                fscheckConfig
                "order-independence: any permutation yields a byte-identical report (US3, SC-006, L-E3)"
            <| fun (cs: CandidateGate list) (s: ReuseStore) ->
            ...
            test "explicit permutations of the worked example are byte-identical (L-E3)" {
                let mk order =
                    CacheEligibility.evaluate (order |> List.map (fun g -> candidate g baseInputs)) EvidenceReuse.empty
            ...
            test "duplicate GateId, DIFFERENT inputs ⇒ TWO entries under that gate (US3, Edge, L-E4)" {
                ...
                Expect.all
                    entries
                    (fun e -> e.Gate = gid "build" "tests")
                    "both entries are attributed to the duplicate gate"
            }
            ...
            test "empty candidate list ⇒ empty report, total not an error (US3, Edge, L-E5)" {
                Expect.equal
                    (CacheEligibility.evaluate [] (storeOf [ baseInputs, refA ]))
                    (CacheEligibilityReport [])
                    "empty input ⇒ empty report"

                Expect.equal
                    (CacheEligibility.entries (CacheEligibility.evaluate [] EvidenceReuse.empty))
                    []
                    "entries of the empty report is []"
            }
        ]
