...
let tests =
    testList
        ...
        [
            test
                "exact match ⇒ Reusable carrying the recorded evidence reference, not a bare flag (US2 #1, SC-002, L-G5)" {
            ...
            test "multiple matching entries ⇒ Reusable carrying F030's most-recent-wins reference (US2 #2, L-G5)" {
                ...
                Expect.equal
                    (CacheEligibility.evaluateGate c store)
                    expected
                    "the verdict carries exactly F030's most-recent-wins reference"

                Expect.equal
                    (CacheEligibility.evaluateGate c store)
                    (Reusable refB)
                    "most-recent recorded reference wins"
            }

            testPropertyWithConfig
                fscheckConfig
                "isReusable ⟺ F030 Reuse, and the carried reference is exactly F030's (US2, FR-004, L-G1)"
