...
let private singleCategoryVariants: (InputCategory * (FreshnessInputs -> FreshnessInputs)) list =
    [
        ...
        (fun i ->
            { i with
                Command = Some(CommandId "msbuild")
            })
        ...
        [
            ...
            test "same-gate single-field change ⇒ MustRecompute (InputsChanged [thatCategory]) (US1 #2, SC-003, L-G4)" {
                ...
                for (category, vary) in singleCategoryVariants do
                    ...
                    | other ->
                        failtestf
                            "category %A: expected MustRecompute (InputsChanged [%A]), got %A"
                            category
                            category
                            other
            }
            ...
            test
                "several same-gate changes ⇒ InputsChanged names them ALL in F029's fixed diff order, never truncated (US1 #2, L-G4)" {
                ...
                | MustRecompute(InputsChanged cats) ->
                    Expect.equal
                        cats
                        [ RuleHashCat; HeadRevisionCat ]
                        "both categories named, fixed order, never truncated to the first"
                | other ->
                    failtestf "expected MustRecompute (InputsChanged [RuleHashCat; HeadRevisionCat]), got %A" other
            }

            testPropertyWithConfig
                fscheckConfig
                "the named cause is EXACTLY F030's diff for any same-gate change (US1 #2, FR-004, L-G4)"
            <| fun (s: ReuseStore) ->
            ...
            testPropertyWithConfig
                fscheckConfig
                "no candidate is Reusable unless F030 deems a defensible match (US1 #3, SC-001, L-G2)"
            ...
            testPropertyWithConfig
                fscheckConfig
                "recompute-by-default: a Recompute relabels to MustRecompute carrying the same cause (US1, FR-004, L-G2)"
            <| fun (c: CandidateGate) (s: ReuseStore) ->
