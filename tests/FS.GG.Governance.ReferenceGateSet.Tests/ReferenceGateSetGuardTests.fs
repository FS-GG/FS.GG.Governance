        ...
        [
            ...
            test "G2 Routes registry has exactly the 9 reference gates including both floors" {
                ...
                Expect.equal
                    ids
                    expectedGateIds
                    "registry must hold exactly the 9 reference gates — 3 command-bound, the 2 gameplay floors, and the 4 composed F# constitution gates (guards 'rots to empty')"
            }
            ...
            test "G3 Routes every command-bound gate prerequisite is declared; gameplay is command-free" {
                ...
                Expect.equal
                    ...
                    3
                    "the 3 command-bound gates (build/test/evidence) each carry one command prerequisite"

                for c in referenced do
                    Expect.isTrue
                        (declaredCommands.Contains c)
                        (sprintf "command '%s' must be declared in tooling.yml (no dangling ref)" c)
                // The gameplay floor is command-free: it declares no prerequisite at all.
                let gameplay =
                    registry.Gates |> List.filter (fun gate -> gate.Domain = DomainId "gameplay")

                Expect.all
                    gameplay
                    (fun gate -> List.isEmpty gate.Prerequisites)
                    "both gameplay floors are command-free"
            }
            ...
            test "G4 Routes build test evidence gameplay each selected, no orphans" {
                ...
                let selectedIds =
                    selectedGates f |> List.map (fun g -> gateIdValue g.Id) |> Set.ofList

                Expect.equal
                    selectedIds
                    expectedGateIds
                    "each of build/test/evidence/gameplay is selected by its candidate path (0 orphan checks/unreachable domains)"
                ...
                Expect.equal
                    referenced
                    declaredCommands
                    "every declared tooling command is referenced by a gate (0 orphan/dead commands)"
            }
            ...
            test "G7 Profile under Strict at Verify at least one Blocking" {
                ...
                Expect.isNonEmpty
                    blocking
                    "under Strict @ Verify >=1 selected gate must derive Blocking on the same change"
            }
            ...
            test "evidence:evidence is a declared Profile advisory-everywhere gate" {
                ...
                | Some g ->
                    Expect.equal
                        g.Prerequisites
                        [ RequiresCommand(CommandId "build-evidence") ]
                        "evidence gate is bound to the declared build-evidence command"

                    Expect.equal g.Maturity Warn "evidence carries `warn` maturity (advisory everywhere)"

                    Expect.equal
                        (decideUnder Enforcement.Light g)
                        Enforcement.Advisory
                        "evidence stays Advisory under Light @ Verify"

                    Expect.equal
                        (decideUnder Enforcement.Strict g)
                        Enforcement.Advisory
                        "evidence stays Advisory under Strict @ Verify (never blocks on first touch)"
            }
        ]
