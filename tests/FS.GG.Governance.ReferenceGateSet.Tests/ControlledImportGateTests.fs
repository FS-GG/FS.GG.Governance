...
let private tempRoot () =
    let root =
        Path.Combine(Path.GetTempPath(), "fsgg-controlled-import-" + Guid.NewGuid().ToString("N"))
        ...
        [
            test "directory tree verifies before a descendant receives the exemption" {
                withTemp (fun root ->
                    ...
                    Expect.stringContains
                        output
                        "GOV-IMPORT-VERIFIED\tdirectory\tvendor/content"
                        "the tree is verified"

                    Expect.stringContains
                        output
                        "GOV-IMPORT-EXEMPT\tvendor/content/nested/beta.txt"
                        "only then is its descendant exempt")
            }
            ...
            test "mutating one descendant fails the named digest rule and grants no exemption" {
                withTemp (fun root ->
                    ...
                    Expect.stringContains
                        error
                        "GOV-IMPORT-DIGEST\tvendor/content"
                        "the named digest rule identifies the imported tree"

                    Expect.isFalse
                        (output.Contains "GOV-IMPORT-EXEMPT")
                        "a failed tree never grants a descendant exemption")
            }
            ...
            test "regular file is a distinct typed import kind" {
                withTemp (fun root ->
                    ...
                    Expect.stringContains
                        output
                        "GOV-IMPORT-VERIFIED\tfile\tvendor/NOTICE"
                        "file kind verifies by raw-file SHA-256"
            ...
            test "missing import and absent byte-identity attribute both fail closed" {
                withTemp (fun root ->
                    ...
                    Expect.stringContains
                        error
                        "GOV-IMPORT-ATTRIBUTES\t.gitattributes"
                        "checkout byte policy is mandatory"
            ...
            test "symlink inside a controlled tree is rejected before digest comparison" {
                if OperatingSystem.IsWindows() then
                    Tests.skiptest
                        "Windows symlink creation requires host privileges; the Linux CI leg exercises this contract"
                else
                    withTemp (fun root ->
                        let tree = Path.Combine(root, "vendor", "content")
                        Directory.CreateDirectory tree |> ignore
                        ...
                        Expect.stringContains
                            error
                            "GOV-IMPORT-SYMLINK\tvendor/content/escape.txt"
                            "the offending link is named")
            }
            ...
            test "symlink in the destination ancestry cannot escape the repository" {
                if OperatingSystem.IsWindows() then
                    ...
                    withTemp (fun root ->
                        let outside =
                            Path.Combine(Path.GetTempPath(), "fsgg-controlled-outside-" + Guid.NewGuid().ToString("N"))
                        ...
                        try
                            ...
                            Expect.stringContains
                                error
                                "GOV-IMPORT-SYMLINK\tvendor/escaped"
                                "the escaping ancestor is named"
                        finally
                            try
                ...
                if OperatingSystem.IsWindows() then
                    ...
                    withTemp (fun root ->
                        ...
                        try
                            ...
                            Expect.stringContains
                                error
                                "GOV-IMPORT-READ\tvendor/content"
                                "the controlled tree is named"
                        finally
