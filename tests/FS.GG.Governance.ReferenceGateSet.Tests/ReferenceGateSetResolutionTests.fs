...
let private restoredConsumer =
    lazy
        (let root =
            Path.Combine(Path.GetTempPath(), "fsgg-consumer-" + Guid.NewGuid().ToString("N"))
...
let private generatedProductScaffold () : string * string =
    let target =
        Path.Combine(Path.GetTempPath(), "fsgg-scaffold-" + Guid.NewGuid().ToString("N"))
...
let private resolveIntoFreshConsumerWith (extraArgs: string list) : string =
    ...
    let target =
        Path.Combine(Path.GetTempPath(), "fsgg-resolve-" + Guid.NewGuid().ToString("N"))
    ...
    if code <> 0 then
        failtestf "the documented resolution verb failed (exit %d)\nSTDOUT:\n%s\nSTDERR:\n%s" code out err
...
let private reResolve (consumerDir: string) (extraArgs: string list) =
    ...
    let code, out, err =
        runDotnet
            consumerDir
            packagesDir
            ([ "msbuild"; "Consumer.csproj"; "-t:FsggResolveReferenceGateSet" ] @ extraArgs)
    ...
    <| testList
        ...
        [
            ...
            test "R2 the resolution verb materializes the package payload byte-identically" {
                ...
                Expect.equal
                    resolved
                    (List.sort shippedFiles)
                    "the resolved .fsgg/ must hold exactly the package's published set"
            ...
            test "R4 deleting the floor from the resolved copy does not escape it" {
                ...
                Expect.notEqual
                    bound
                    original
                    "the escape test must actually bind a templateProfile, or it proves nothing"
                ...
                let localGameplay =
                    ...
                    |> List.filter (fun c -> c.Domain = DomainId "gameplay")
                ...
                let inheritedIds = inherited |> List.map (fun g -> gateIdValue g.Id) |> Set.ofList
            ...
            test "R7 a generated product's scaffolded .fsgg merges the composed profile without losing SDD's files" {
                ...
                let target =
                    Path.Combine(Path.GetTempPath(), "fsgg-generated-" + Guid.NewGuid().ToString("N"))
                ...
                let facts =
                    ...
                    | Invalid diags ->
                        failtestf
                            "a scaffolded generated product with the profile resolved into it must load Valid: %A"
                            diags
                ...
                for pack in FS.GG.Governance.SurfaceChecks.Profile.packs do
                    ...
                    Expect.isTrue
                        ...
                        (sprintf
                            "the generated product must carry the composed gate '%s' after resolving; got %A"
                            id
                            gateIds)
