...
let private schemaVersionOf (fileName: string) : int =
    ...
    if not m.Success then
        failtestf "no schemaVersion in %s" fileName
    ...
    <| testList
        ...
        [
            ...
            test "T005 Package carries exactly the six .fsgg files byte-identical to source" {
                ...
                let content =
                    entryNames archive
                    |> List.filter (fun n -> n.StartsWith contentPrefix)
                    |> List.sort
            ...
            test "AC2 Package carries the buildTransitive resolution target" {
                ...
                Expect.contains
                    names
                    buildTransitiveEntry
                    "the resolution target must ship at buildTransitive/<PackageId>.targets"

                let targets =
                    System.Text.Encoding.UTF8.GetString(readEntryBytes archive buildTransitiveEntry)
            ...
            test "T005 Package declares no dependency group" {
                use archive = ZipFile.OpenRead producedNupkg.Value
                let nuspecName = entryNames archive |> List.find (fun n -> n.EndsWith ".nuspec")
            ...
            test "T008 Pack aborts and writes no nupkg when a G1-G7 invariant is broken" {
                ...
                let outDir =
                    Path.Combine(Path.GetTempPath(), "fsgg-pack-broken-" + Guid.NewGuid().ToString("N"))
                ...
                try
                    ...
                    let broken =
                        (File.ReadAllText policy).Replace("defaultProfile: light", "defaultProfile: strict")
                    ...
                    Expect.notEqual
                        code
                        0
                        (sprintf "pack must fail when G1-G7 are red\nSTDOUT:\n%s\nSTDERR:\n%s" out err)
            ...
            test "T011 print-version emits the pinned plain SemVer" {
                ...
                Expect.equal
                    (out.Trim())
                    expectedVersion
                    "the version is the pinned plain SemVer (ADR-0055), not a schema-derived tuple"
            }
            ...
            test "T011 a schemaVersion bump moves the manifest, NOT the version" {
                ...
                try
                    ...
                    let bumped =
                        (File.ReadAllText policy).Replace("schemaVersion: 1", "schemaVersion: 2")
                    ...
                    Expect.equal
                        (vout.Trim())
                        expectedVersion
                        "a schemaVersion bump must NOT change the pinned version (ADR-0055)"
            ...
            test "ADR-0055 Package carries schema-manifest.json alongside .fsgg, not inside it" {
                ...
                Expect.contains
                    names
                    manifestEntry
                    "the manifest must be packed at contentFiles/any/any/schema-manifest.json"

                Expect.isFalse
                    (names
                     |> List.exists (fun n -> n.StartsWith contentPrefix && n.EndsWith "schema-manifest.json"))
            ...
            test "ADR-0055 schema-manifest.json records the on-disk schema generations" {
                ...
                let manifest =
                    System.Text.Encoding.UTF8.GetString(readEntryBytes archive manifestEntry)
            ...
            test "#148 the gate targets the configuration the caller asked for, not a hard-coded Debug" {
                let code, out, err =
                    runPack [ "--print-gate-command"; "--configuration"; "Release" ]
            ...
            test "#148 a --configuration with no value fails loudly instead of defaulting" {
                ...
                Expect.notEqual
                    code
                    0
                    (sprintf "a valueless --configuration must not silently default\nSTDOUT:\n%s" out)
            }
            ...
            test "#148 an unknown configuration fails loudly instead of gating an unintended tree" {
                ...
                Expect.notEqual
                    code
                    0
                    (sprintf "a typo'd configuration must be refused, not forwarded to MSBuild\nSTDOUT:\n%s" out)
            }
        ]
