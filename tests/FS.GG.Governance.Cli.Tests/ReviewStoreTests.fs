...
let private tempStore () =
    let dir =
        Path.Combine(Path.GetTempPath(), "fsgg-reviewstore-tests", Path.GetRandomFileName())
    ...
    testList
        ...
        [
            ...
            test "distinct keys that sanitize identically do NOT collide (#55 M-CLI-2)" {
                ...
                let r1: RecordedReview =
                    {
                        Rule = RuleId "rule.one"
                        Key = k1
                        Verdict = Fail "blocked-by-k1"
                    }

                let r2: RecordedReview =
                    {
                        Rule = RuleId "rule.one"
                        Key = k2
                        Verdict = Pass
