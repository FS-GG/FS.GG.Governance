    ...
    testList
        ...
        [
            test "dispatcher parses the watch/tui commands and the --plain flag (US3/US4 vocabulary)" {
                Expect.equal
                    (Cli.parse [ "watch" ] |> Result.map (fun r -> r.Command))
                    (Ok WatchCommand)
                    "watch ⇒ WatchCommand"
            ...
            test
                "JSON always wins over --plain at the dispatcher: Json format selected, render goes to JSON, never rich (SC-004)" {
            ...
            test
                "tui host parity: the ReportView the dispatcher composes IS the view Tui.init navigates, over the SAME report object (SC-006)" {
                withTempRepo (fun dir ->
                    ...
                    | Some view ->
                        ...
                        Expect.equal
                            model.View
                            view
                            "Tui.init navigates the SAME ReportView the dispatcher composed (report-object parity)"
                    | None -> failtest "the dispatcher composed no report view over the real temp repo")
            }

            test
                "tui host read-only: navigation never mutates the composed view, and the compose path writes no contract artifact (FR-009, SC-006)" {
                withTempRepo (fun dir ->
                    let req =
                        match Cli.parse [ "tui"; "--root"; dir ] with
                        | Ok r -> r
                    ...
                    Expect.equal
                        model3.View
                        view
                        "navigation changes only Path/Expanded — the ReportView is never mutated"

                    // composeRouteView is read-only: no contract artifact appears under the tree.
                    Expect.isFalse
                        (File.Exists(Path.Combine(dir, "readiness", "route.json")))
                        "tui compose wrote no route.json"

                    Expect.isFalse
                        (File.Exists(Path.Combine(dir, ".fsgg", "gates.json")))
                        "tui compose wrote no gates.json")
            }

            test
                "watch host wiring: the dispatcher's read-only re-render composes an ANSI-free plain projection and writes no contract artifact (FR-009)" {
                withTempRepo (fun dir ->
                    let req =
                    ...
                    | Some view ->
                        ...
                        Expect.isFalse
                            (plain.Contains esc)
                            "the composed plain projection is ANSI-free (the Plain mode the dispatcher selects off-TTY)"
                    | None -> failtest "no view composed"

                    Expect.isFalse
                        (File.Exists(Path.Combine(dir, "readiness", "route.json")))
                        "watch compose writes no contract artifact")
            }

            test
                "headless (CLI-1): the dispatcher's `tuiKeyReader` swallows an unreadable console (redirected stdin) and quits, never crashing" {
                ...
                if Console.IsInputRedirected then
                    ...
                    skiptest
                        "console is interactive here; the headless guard path is not exercised (ReadKey would block)"
            }

            test
                "headless (H3 / #47): the dispatcher's `runWatch` over a nonexistent root fails to arm the watcher and exits input-unavailable (66), never crashing" {
                ...
                let missing =
                    Path.Combine(Path.GetTempPath(), "fsgg-cli-watch-missing-" + Guid.NewGuid().ToString("N"))

                match Cli.parse [ "watch"; "--root"; missing ] with
                | Ok request ->
                    Expect.equal
                        (Program.runWatch request)
                        66
                        "a nonexistent watch root ⇒ InputUnavailable exit (66), not a FileSystemWatcher crash"
                | Error e -> failtestf "parse failed: %A" e
            }
        ]
