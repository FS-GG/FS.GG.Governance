...
let private routeExit (mode: string) (handoffs: Reader.HandoffRead list) : ExitDecision =
    ...
    let model, _ =
        Cli.update (SnapshotLoaded(Ok { snapshot with Handoffs = handoffs })) model
    ...
    testList
        ...
        [
            test "failing handoff blocks at --mode gate (GovernedBlocking, exit 2)" {
                Expect.equal
                    (routeExit "gate" [ failingHandoff ])
                    GovernedBlocking
                    "failing handoff must block the gate"
            }

            test "passing handoff passes at --mode gate (Success, exit 0)" {
                Expect.equal
                    (routeExit "gate" [ passingHandoff ])
                    Success
                    "an all-real handoff is advisory, not blocking"
            }

            test "failing handoff does NOT block in light mode (inner ⇒ Success)" {
                Expect.equal
                    (routeExit "inner" [ failingHandoff ])
                    Success
                    "light/non-strict mode never blocks on the handoff"
            }

            test "failing handoff does NOT block in light mode (sandbox ⇒ Success)" {
                Expect.equal
                    (routeExit "sandbox" [ failingHandoff ])
                    Success
                    "sandbox is light; the failing handoff is advisory"
            }
            ...
            test "a malformed handoff document blocks at gate (bad doc ⇒ blocking integrity gate)" {
                Expect.equal
                    (routeExit "gate" [ handoffRead "{ not json" ])
                    GovernedBlocking
                    "a bad document is a blocking integrity gate, never silently ignored"
            }
        ]
        ...
        [
            ...
            test "snapshot and output failures become distinct exit decisions" {
                ...
                Expect.isTrue
                    (effects
                     |> List.exists (function
                         | WriteOutput(_, result) -> result.Exit = InputUnavailable "missing root"
                         | _ -> false))
                    "input unavailable"
                ...
                Expect.isTrue
                    (model.Result
                     |> Option.exists (fun result -> result.Exit = ToolError "disk full"))
                    "tool error"
            }
        ]
