    ...
    testList
        ...
        [
            testPropertyWithConfig
                fscheckConfig
                "byte-for-byte determinism: serialise store = serialise store"
                (fun (store: ReuseStore) -> EvidenceReuseStore.serialise store = EvidenceReuseStore.serialise store)
            ...
            test "stable top-level key order: schemaVersion then recorded" {
                let text =
                    EvidenceReuseStore.serialise (storeOf [ inputs "fmt", syntheticRef "fmt" ])
            ...
            test "stable per-entry field order with both optionals present (D7)" {
                ...
                let text =
                    EvidenceReuseStore.serialise (storeOf [ inputs "full", syntheticRef "full" ])
            ...
            test "entries emitted in the store's verbatim newest-first order, coveredArtifacts verbatim (D5/D7)" {
                ...
                let checks =
                    [
                        for e in (doc.RootElement.GetProperty "recorded").EnumerateArray() ->
                ...
                let arts =
                    [
                        for a in
                            ((doc.RootElement.GetProperty "recorded").EnumerateArray() |> Seq.head)
                                .GetProperty("coveredArtifacts")
                                .EnumerateArray() -> a.GetString()
                    ]
