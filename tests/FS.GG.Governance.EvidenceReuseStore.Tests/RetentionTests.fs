    ...
    testList
        ...
        [
                ...
                let store =
                    storeOf
                        [
                            ...
                            inputs "d", syntheticRef "d"
                        ] // newest-first: d, c, b, a

                let bounded = EvidenceReuseStore.retain 2 store

                Expect.equal
                    (EvidenceReuse.entries bounded)
                    (EvidenceReuse.entries store |> List.truncate 2)
                    "first 2 of newest-first, in order"
            ...
            testPropertyWithConfig
                fscheckConfig
                "retain n is the prefix of length (min n len), in order"
            ...
            testPropertyWithConfig
                fscheckConfig
                "idempotent at/under bound: length ≤ n ⇒ retain n store = store"
                (fun (store: ReuseStore) ->
                    ...
                    EvidenceReuseStore.retain len store = store
                    && EvidenceReuseStore.retain (len + 5) store = store)

            testPropertyWithConfig
                fscheckConfig
                "removal-only: every survivor is byte-for-byte an input entry (a prefix subset)"
                (fun (store: ReuseStore) (n: int) ->
                    let all = EvidenceReuse.entries store
                    ...
                    survivors = (all |> List.truncate (max 0 n))
                    && survivors |> List.forall (fun e -> List.contains e all))
            ...
            test "defaultRetentionBound is a positive constant usable as the bound; pins current value 256 (D8)" {
                ...
                let bounded =
                    EvidenceReuseStore.retain EvidenceReuseStore.defaultRetentionBound store

                Expect.isLessThanOrEqual
                    (EvidenceReuse.entries bounded |> List.length)
