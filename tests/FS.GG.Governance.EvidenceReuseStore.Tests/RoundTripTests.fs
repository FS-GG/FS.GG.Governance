    ...
    testList
        ...
        [
            testPropertyWithConfig
                fscheckConfig
                "lossless round-trip: readBack (serialise store) = Some store"
            ...
            test "an ABSENT file loads as the empty store (Ok None)" {
                let missing =
                    Path.Combine(Path.GetTempPath(), "fsgg-no-such-store-" + string (System.Guid.NewGuid()) + ".json")
                ...
                let store =
                    ReuseStore
                        [
                            {
                                Inputs = i
                                Evidence = EvidenceRef "ev://a\"b\\c\td"
                            }
                        ]

                Expect.equal
                    (readBack (EvidenceReuseStore.serialise store))
                    (Some store)
                    "escaped strings round-trip verbatim"
            }
            ...
            test "sensed-empty covered set (CoveredArtifacts = []) round-trips as an empty list" {
                ...
                let store =
                    ReuseStore
                        [
                            {
                                Inputs = i
                                Evidence = syntheticRef "empty-cov"
                            }
                        ]
                ...
                match loaded with
                | Some(ReuseStore [ e ]) ->
                    Expect.equal e.Inputs.CoveredArtifacts [] "covered artifacts are the empty list"
            ...
            test "multi-element coveredArtifacts preserved in stored list order, verbatim (D5)" {
                let i =
                    { inputs "ord" with
                        CoveredArtifacts = [ ArtifactHash "z"; ArtifactHash "a"; ArtifactHash "a"; ArtifactHash "m" ]
                    }
