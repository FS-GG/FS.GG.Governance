...
let private key i =
    FreshnessKey.value (FreshnessKey.compute i)
...
let private placements: (string * FreshnessInputs * FreshnessInputs) list =
    ...
    [
        ...
        { neutral with
            RuleHash = RuleHash mark
        },
        { neutral with
            GeneratorVersion = GeneratorVersion mark
        }
        "base vs head", { neutral with Base = Revision mark }, { neutral with Head = Revision mark }
        "check vs domain", { neutral with Check = CheckId mark }, { neutral with Domain = DomainId mark }
        "ruleHash vs coveredArtifacts",
        { neutral with
            RuleHash = RuleHash mark
        },
        { neutral with
            CoveredArtifacts = [ ArtifactHash mark ]
        }
    ]
         ...
         @ [
             test "a delimiter-laden value cannot masquerade as a neighbouring field (FR-006)" {
                 ...
                 let x =
                     { neutral with
                         RuleHash = RuleHash "a"
                         GeneratorVersion = GeneratorVersion "b"
                     }
