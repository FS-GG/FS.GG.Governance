...
let private freshnessKeyAsm =
    SurfaceDrift.assemblyNamed "FS.GG.Governance.FreshnessKey"
    ...
    testList
        ...
        [
            ...
            test "the public surface is exactly the two modules (Model + FreshnessKey), nothing else" {
                ...
                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.FreshnessKey.ModelModule"))
                    "Model module is public"

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.EndsWith "FS.GG.Governance.FreshnessKey.FreshnessKeyModule"))
