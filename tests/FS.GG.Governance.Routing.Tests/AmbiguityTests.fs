    ...
    testList
        ...
        [
            test "co-specific competitors → AmbiguousRoute + deterministic LexicographicTiebreak (FR-006, SC-004)" {
                ...
                let report =
                    Routing.route
                        (facts "." [ "src/*/Eval.fs", "a"; "src/Kernel/*.fs", "b" ])
            ...
            test "same glob string, different domains → ConflictingGlobBinding (excluded, FR-009)" {
                ...
                Expect.equal
                    (resultFor report "src/x.fs")
                    (Some UnmatchedInRoot)
                    "conflicting glob excluded from matching"
            }
            ...
            test "reserved-char glob → UnsupportedGlobSyntax (excluded, not silent never-match, FR-010)" {
                ...
                Expect.equal
                    (resultFor report "src/c.fs")
                    (Some(Routed(dom "core", gp "src/**", OnlyMatch)))
                    "valid glob still routes"
            }
            ...
            test "in-root unmatched carries no severity (US3 AS3, FR-016)" {
                let report =
                    Routing.route (facts "src" [ "src/Kernel/**", "kernel" ]) [ gp "src/README.md" ]

                Expect.equal
                    (resultFor report "src/README.md")
                    (Some UnmatchedInRoot)
                    "in-root miss is plain UnmatchedInRoot"
