...
let ids report =
    report.Findings |> List.map _.Id |> Set.ofList
...
let tests =
    testList
        "F# simplicity architecture sensor"
        [
            ...
            test "System Type reflection call without namespace open Synthetic is detected" {
                ...
                let reflection =
                    ...
                    |> fun report ->
                        report.Findings
                        |> List.filter (fun finding -> finding.Id = ReflectionOrMetaprogramming)

                Expect.isNonEmpty reflection "actual reflection call is detected"

                Expect.isTrue
                    (reflection
                     |> List.exists (fun finding -> finding.Symbol = "System.Type.GetMethods"))
                    "finding names the invoked reflection API"
            }
            ...
            test "Measured array loop Synthetic current justification suppresses exact construct" {
                ...
                let imperative =
                    initial.Findings |> List.filter (fun f -> f.Id = ImperativeInPureDomain)

                Expect.isNonEmpty imperative "planted loop/mutation found"

                Expect.isTrue
                    (imperative |> List.exists (fun f -> f.Symbol.EndsWith(":FOR")))
                    "compiler-tokenized loop found"
                    ...
                    |> List.map (fun f ->
                        {
                            ...
                            SimplerAlternative = "Array.sum"
                            Reason = Measured
                            Evidence = "benchmark: 18% less allocation"
                        })
            ...
            test "Material edit and head change Synthetic stale justifications" {
                ...
                let first =
                    request source
                    |> analyzeNow
                    |> fun r -> r.Findings |> List.find (fun f -> f.Id = SharedMutableState)
                ...
                let current =
                    {
                        ...
                        Head = "abc123"
                        SourceDigest = sourceDigest source
                        SimplerAlternative = "thread state"
                        Reason = Interoperability
                        Evidence = "framework callback"
                    }

                let changed =
                    { request (source + "let x = 1\n") with
                        Justifications = [ current ]
                    }
                    |> analyzeNow

                Expect.equal
                    (changed.Findings |> List.find (fun f -> f.Id = SharedMutableState)).Justification
                    StaleSource
                    "source binding stales"
                ...
                Expect.equal
                    (moved.Findings |> List.find (fun f -> f.Id = SharedMutableState)).Justification
                    StaleHead
                    "head binding stales"
            }
                    ...
                    { request source with
                        ...
                        ApprovedPrimitives =
                            [
                                {
                                    Capability = "dispatch"
                                    ApprovedSymbols = [ "FS.GG.Dispatch" ]
                                    CandidateSymbols = [ "Domain.Dispatcher" ]
                                }
                            ]
                    }
            ...
            test "Legitimate adapter class Synthetic passes with bound justification" {
                ...
                let justifications =
                    first.Findings
                    |> List.distinctBy (fun f -> f.Path, f.Symbol)
                    |> List.map (fun f ->
                        {
                            Path = f.Path
                            Symbol = f.Symbol
                            Head = "abc123"
                            SourceDigest = sourceDigest source
                            SimplerAlternative = "module function"
                            Reason = Interoperability
                            Evidence = "host requires construction"
                        })
            ...
            test "Declared project reference Synthetic reaches real compiler rules and removal fails closed" {
                ...
                Expect.isTrue
                    ((ids withReference).Contains InheritanceHierarchy)
                    "declared sibling assembly enables real inheritance analysis"

                Expect.isFalse
                    ((ids withReference).Contains CompilerAnalysisFailed)
                    "declared sibling assembly does not short-circuit"

                let withoutReference = request source |> analyzeNow

                Expect.equal
                    withoutReference.Findings.Head.Id
                    CompilerAnalysisFailed
                    "removing the declared reference remains fail-closed"
            }

            test "Generated document Synthetic is explicitly excluded" {
                let req =
                    { request "module Domain\nlet mutable registry = 0" with
                        Documents =
                            [
                                {
                                    Path = "src/G.fs"
                                    Source = "module Domain\nlet mutable registry = 0"
                                    IsGenerated = true
                                    References = []
                                }
                            ]
