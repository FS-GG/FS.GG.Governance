...
let rec repoRoot (dir: DirectoryInfo) =
    if File.Exists(Path.Combine(dir.FullName, "FS.GG.Governance.sln")) then
        dir.FullName
    else
...
let surfaceTest =
    test "CodeChecks public surface equals committed baseline" {
        let path =
            Path.Combine(
                repoRoot (DirectoryInfo Environment.CurrentDirectory),
                "surface",
                "FS.GG.Governance.CodeChecks.surface.txt"
            )
...
let dependencyTest =
    test "CodeChecks references only FCS and framework assemblies" {
        let forbidden =
...
[<Tests>]
let tests = testList "SurfaceDrift" [ surfaceTest; dependencyTest ]
