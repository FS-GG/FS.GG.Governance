    ...
    testList
        ...
        [
            ...
            test "perturbing any one reproducible input changes both canonicalId and referenceOf" {
                ...
                let variants =
                    [
                        ...
                        "argument value",
                        mk
                            (Build.command (arguments = [ Argument "alpha"; Argument "GAMMA" ]))
                            baseOut
                            baseErr
                            (ExitCode 0)
                            100L
                        "argument order",
                        mk
                            (Build.command (arguments = [ Argument "beta"; Argument "alpha" ]))
                            baseOut
                            baseErr
                            (ExitCode 0)
                            100L
                        "working directory",
                        mk (Build.command (workingDirectory = "/elsewhere")) baseOut baseErr (ExitCode 0) 100L
                        "env Added",
                        mk
                            (Build.command (
                                environment =
                        ...
                        "captured output",
                        mk
                            (Build.command (capturedOutput = CapturedAt(CapturedOutputPath "x")))
                            baseOut
                            baseErr
                            (ExitCode 0)
                            100L
                ...
                for label, variant in variants do
                    ...
                    Expect.notEqual
                        ...
                        baseRef
                        (sprintf "%s changes referenceOf" label)
            }
            ...
            test "two outcomes differing only in SensedDuration share canonicalId and referenceOf" {
                ...
                Expect.equal
                    (CommandRecord.canonicalId baseRec)
                    (CommandRecord.canonicalId slower)
                    "duration-invariant id"

                Expect.equal
                    (EvidenceCapture.referenceOf baseRec)
                    (EvidenceCapture.referenceOf slower)
                    "duration-invariant ref"
            }
            ...
            <| fun (cmd: GateCommand) (out: byte[]) (err: byte[]) (d1: int64) (d2: int64) ->
                let a =
                    Interpreter.senseExecution (fakePort out err (ExitCode 0) (SensedDuration d1)) cmd

                let b =
                    Interpreter.senseExecution (fakePort out err (ExitCode 0) (SensedDuration d2)) cmd
