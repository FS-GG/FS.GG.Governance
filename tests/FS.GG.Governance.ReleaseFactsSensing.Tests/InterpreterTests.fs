        ...
        [
            ...
            test "deleted source + corrupted-unparseable source ⇒ both Unrecoverable, all seven still returned" {
                withTempDir (fun dir ->
                    ...
                    Expect.equal
                        sensed.Facts.States.[Provenance]
                        Unrecoverable
                        "absent source ⇒ Unrecoverable, not Met"

                    Expect.equal
                        sensed.Facts.States.[TemplatePins]
                        Unrecoverable
                        "unparseable source ⇒ Unrecoverable, not Met"
            ...
            test "a port read that throws ⇒ gather reifies Error ⇒ that family Unrecoverable (never a crash)" {
                let throwingPort =
                    { metPort with
                        ReadProvenance = fun () -> failwith "boom: simulated read failure"
                    }
                ...
                Expect.isTrue
                    (match bundle.Provenance with
                     | Error _ -> true
                     | Ok _ -> false)
                    "thrown read reified as Error"
            }
        ]
