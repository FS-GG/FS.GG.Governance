    ...
    testList
        ...
        [
            test "releaseFamilies is exactly the seven kinds in declaration order, all distinct" {
                ...
                Expect.equal (Sensing.releaseFamilies |> List.distinct |> List.length) 7 "all seven families distinct"
            }
            ...
            test "all-satisfying evidence ⇒ the six repo-sensed Met, ApiCompatibility Unrecoverable (host-overlaid)" {
                ...
                Expect.equal
                    states.[ApiCompatibility]
                    Unrecoverable
                    "ApiCompatibility host-overlaid ⇒ Unrecoverable here"
