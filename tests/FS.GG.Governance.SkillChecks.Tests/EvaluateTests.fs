...
let private req =
    requestFor "skill-foo" ".claude/skills/foo/SKILL.md" (Some "skill-evidence")
    ...
    testList
        ...
        [
            ...
            test "no mirror declared ⇒ not an error" {
                let f =
                    facts [ { Claimed = "x"; Outcome = PathHolds } ] TaskListConsistent NoMirrorDeclared
                ...
                let f =
                    facts
                        [
                            {
                                Claimed = "missing.sh"
                                Outcome = PathUnresolved "missing.sh"
                            }
                        ]
                        TaskListConsistent
                        NoMirrorDeclared
                ...
                let f =
                    facts
                        [
                            {
                                Claimed = "../escape"
                                Outcome = PathEscapesBounds "../escape"
                            }
                        ]
                        TaskListConsistent
                        NoMirrorDeclared
            ...
            test "drifted mirror ⇒ Blocking skill.mirror" {
                let f =
                    facts [] TaskListConsistent (MirrorDrifted("mirror/SKILL.md", "content differs"))
