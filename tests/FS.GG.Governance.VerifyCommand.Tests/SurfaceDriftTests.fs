...
let private verifyCommand =
    SurfaceDrift.assemblyNamed "FS.GG.Governance.VerifyCommand"
    ...
    testList
        ...
        [
            ...
            test "the public API surface is exactly Loop + Interpreter + the three 076 fold seams (plus the Exe entry)" {
                ...
                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.Contains "FS.GG.Governance.VerifyCommand.LoopModule"))
                    "Loop module is public"

                Expect.isTrue
                    (typeNames
                     |> Array.exists (fun n -> n.Contains "FS.GG.Governance.VerifyCommand.InterpreterModule"))
                ...
                Expect.isEmpty
                    unexpected
                    (sprintf
                        "only Loop/Interpreter/Program + the three 076 fold seams are public; found extra: %A"
                        unexpected)
            }
            ...
            test "VerifyCommand references VerifyJson (not AuditJson) and no kernel/host/cli" {
                ...
                Expect.isEmpty
                    forbidden
                    (sprintf
                        "verify must not reference AuditJson/RouteJson/GatesJson/kernel/host/cli/adapters/Spectre; found: %A"
                        forbidden)
            }
        ]
