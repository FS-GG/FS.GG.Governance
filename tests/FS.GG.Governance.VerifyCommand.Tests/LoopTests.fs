        ...
        [
            test "init: DefaultRange emits SenseScope; ExplicitPaths senses the release preview before the catalog" {
                ...
                Expect.isTrue
                    (hasEffect
                        (function
                        | Loop.SenseScope _ -> true
                        | _ -> false)
                        effDefault)
                    "DefaultRange senses scope"
                ...
                Expect.equal
                    ...
                    [
                        Loop.LoadHandoffs "."
                        Loop.SenseViewCurrency "."
                        Loop.SenseProvenance
                        Loop.SenseReleasePreview "."
                    ]
            ...
            test "Loaded(Valid) runs the F015→F019 selection at RunMode.Verify and senses freshness + store" {
                ...
                Expect.isFalse
                    (hasEffect
                        (function
                        | Loop.ExecuteGates _ -> true
                        | _ -> false)
                        eff)
                    "no execute before sensing"
                ...
                Expect.equal
                    m2.Decision
                    (Some(decisionOf validCatalog candidates Verify Standard))
                    "decision rolled at Verify"
            }
            ...
            test
                "empty selection short-circuits to a passing 'nothing to verify' verdict with no freshness/execute work" {
                ...
                Expect.isTrue
                    (hasEffect
                        ...
                        | _ -> false)
                        eff2)
                    "writes verify.json once surfaces land"
                ...
                Expect.stringContains
                    (Loop.render m1b Loop.Text)
                    "verdict: PASS"
                    "empty selection ⇒ passing verdict text"
