...
let private runWith profile sensor exec =
    ...
    let ports =
        fakePortsExec validCatalog gitSrcChange sensor absentStoreReader exec cap
...
let tests =
    testList
        ...
        [
            test
                "an empty store ⇒ every selected check is recompute (no prior evidence) in the cache-eligibility section" {
