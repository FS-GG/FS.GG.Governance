...
let private runProfile (g) (profile: Profile) =
    ...
    let req =
        { requestForProfile Loop.DefaultRange Loop.Json profile with
            Repo = "."
        }

    let model =
        Interpreter.run (fakePortsExec validCatalog g fakeSensor absentStoreReader fakeExecPortPass cap) req

    model, cap

let private outcomeFor (model: Loop.Model) (gid: string) : GateOutcome option =
    model.Outcomes
    |> List.tryPick (fun (g, o) -> if gateIdValue g = gid then Some o else None)
    ...
    testList
        ...
        [
            ...
            test "verify --profile Light defers the over-budget medium gate; the cheap gate still runs" {
                ...
                | Some doc ->
                    ...
                    Expect.stringContains
                        doc
                        "overBudget"
                        "the deferred gate is recorded overBudget (named, never silent)"
            ...
            test "verify --profile Light defers the High-cost workflow audit gate" {
                ...
                match writtenCostBudget cap with
                | Some doc ->
                    Expect.stringContains doc "workflow:audit" "the deferred audit gate is named in the sidecar"
            ...
            test "kindOf is total over the selected gates and runIdentity ignores sensed duration (FR-004)" {
                ...
                for gate in gates do
                    ...
                    Expect.isTrue
                        ...
                        "kindOf yields a closed-taxonomy kind"
                ...
                let baseRecord =
                    FS.GG.Governance.GateExecution.Interpreter.senseExecution (fakeExecPortExiting 0) cmd

                let run1 =
                    {
                        Kind = Loop.kindOf gate
                        Record = baseRecord
                    }
                ...
                let run2 =
                    {
                        ...
                        Record =
                            { baseRecord with
                                Duration = SensedDuration 999L
                            }
                    }

                Expect.equal
                    (FS.GG.Governance.CommandKind.Audit.runIdentity run1)
                    (FS.GG.Governance.CommandKind.Audit.runIdentity run2)
                    "duration never participates in the run identity"
            }
            ...
            test "verify.json is byte-identical with wiring active; the two sidecars are written beside it" {
                ...
                | Some(_, doc) ->
                    Expect.equal
                        doc
                        (verifyExpected validCatalog candidates Strict snap)
                        "verify.json bytes unchanged (sidecars never fold in)"
            ...
            test "nothing-to-verify still writes well-formed empty sidecars" {
                ...
                Interpreter.run
                    (fakePortsExec emptyCatalog gitEmpty fakeSensor absentStoreReader fakeExecPortPass cap)
                    req
                |> ignore
                ...
                | Some doc ->
                    Expect.stringContains doc "fsgg.provenance/v1" "empty provenance sidecar still carries its schema"
            ...
            test "cost/cache findings enforce as Advisory and never enter verify.json (FR-008)" {
                ...
                let findings =
                    FS.GG.Governance.CostBudget.Findings.cacheFindings report (fun _ ->
                        FS.GG.Governance.CostBudget.Findings.Real)
                ...
                for f in findings do
                    ...
                    Expect.equal
                        ...
                        Advisory
                        "a cost/cache finding is advisory — never a blocker"

                // The findings live in cost-budget.json, not verify.json.
                match writtenVerify cap, writtenCostBudget cap with
                | Some(_, vdoc), Some _ ->
                    Expect.isFalse (vdoc.Contains "syntheticTaint") "findings never leak into verify.json"
            ...
            test "an unreadable store still writes well-formed sidecars with no fabricated reuse" {
                ...
                let model =
                    Interpreter.run
                        (fakePortsExec validCatalog gitSrcChange fakeSensor malformedStoreReader fakeExecPortPass cap)
                        req
