...
let private runWith profile exec =
    ...
    let ports =
        fakePortsExec validCatalog gitSrcChange fakeSensor absentStoreReader exec cap
...
let tests =
    testList
        ...
        [
            test
                "clean: Standard profile + passing checks ⇒ passing verdict, exit 0, written bytes match the projection" {
                ...
                | Some(path, content) ->
                    ...
                    Expect.equal
                        content
                        (verifyExpectedWith fakeExecPortPass validCatalog srcCandidates Standard None)
                        "bytes equal the genuine projection"
            ...
            test "nothing-to-verify: an empty change ⇒ 'nothing to verify', exit 0" {
                ...
                let ports =
                    fakePortsExec validCatalog gitEmpty fakeSensor absentStoreReader fakeExecPortPass cap
            ...
            test
                "uncertain: a blocking check reporting an uncertain (exit 125) result is never coerced to pass ⇒ Blocked, exit 1" {
                ...
                match writtenVerify cap with
                | Some(_, content) ->
                    Expect.stringContains content "125" "the uncertain exit code is surfaced in verify.json"
                | None -> failtest "expected a verify.json write"
            }
            ...
            test "F070: stale view blocks verify under strict (C1) and warns (no-hide) under standard" {
                withTempRepo (fun dir ->
                    let sha (s: string) =
                        ...
                        h.ComputeHash(System.Text.Encoding.UTF8.GetBytes s)
                    ...
                    writeFile
                        ...
                        (sprintf
                            "{\"views\":{\"route-projection\":{\"sources\":[\"%s\"],\"generatorVersion\":\"g1\",\"output\":\"x\"}}}"
                            (sha "OLD\n"))

                    let runP (profile: string) =
                        let req =
                            match
                                Loop.parse [ "verify"; "--repo"; dir; "--since"; "HEAD~1"; "--profile"; profile ]
                            with
                            | Ok r -> r
                    ...
                    Expect.stringContains
                        ...
                        "block-on-pr blocks under verify+strict (C1)"
                    ...
                    Expect.stringContains
                        ...
                        "relaxed to a warning under verify+standard (FR-009)")
            }
        ]
