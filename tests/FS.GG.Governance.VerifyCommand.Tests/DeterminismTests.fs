...
let private runJson () =
    ...
    let req =
        { requestForProfile srcScope Loop.Json Strict with
            VerifyOut = "readiness/verify.json"
        }

    Interpreter.run (fakePortsExec validCatalog gitSrcChange fakeSensor absentStoreReader fakeExecPortFail cap) req
    |> ignore
