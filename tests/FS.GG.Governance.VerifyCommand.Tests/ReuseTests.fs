...
let private withTmp (body: string -> 'a) : 'a =
    let dir =
        Path.Combine(Path.GetTempPath(), "fsgg-verify-reuse-" + Guid.NewGuid().ToString("N"))
...
let private sensorBuildChanged: FreshnessSensing.FreshnessSensor =
    { fakeSensor with
        SenseCommandVersion =
            fun (CommandId c) ->
                if c = "dotnet-build" then
                    Some(CommandVersion "cmd-CHANGED")
                else
                    Some(CommandVersion "cmd-synthetic")
    }
        ...
        [
            test "run twice with no change ⇒ run 2 reuses every selected check (zero further executions), all fresh" {
                withTmp (fun dir ->
                    ...
                    Interpreter.run (portsWith fakeSensor (countingExecPort c1 0) (newCapture ())) req
                    |> ignore
                    ...
                    Interpreter.run (portsWith fakeSensor (countingExecPort c2 0) cap2) req
                    |> ignore
                    ...
                    Expect.isFalse
                        (text.Contains "no prior evidence")
                        "nothing recomputed for no-prior-evidence on run 2")
            }
            ...
            test "flipping one check's command version recomputes EXACTLY that check, reported stale with its category" {
                withTmp (fun dir ->
                    ...
                    // Populate the store.
                    Interpreter.run (portsWith fakeSensor (countingExecPort { Calls = 0 } 0) (newCapture ())) req
                    |> ignore
                    ...
                    Interpreter.run (portsWith sensorBuildChanged (countingExecPort c 0) cap) req
                    |> ignore
