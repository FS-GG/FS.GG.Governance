    ...
    testList
        ...
        [
            test "an invalid catalog ⇒ InputUnavailable (exit 3), no artifact, tagged diagnostic" {
                ...
                let model =
                    Interpreter.run (fakePorts invalidCatalog gitSrcChange cap) (requestFor srcScope Loop.Text)
                ...
                Expect.isTrue
                    (model.Diagnostics |> List.forall (fun d -> d.Category = Loop.InputUnavailable))
                    "tagged input-unavailable"
            }
            ...
            test "an absent catalog ⇒ InputUnavailable (exit 3)" {
                ...
                let model =
                    Interpreter.run (fakePorts Map.empty gitSrcChange cap) (requestFor srcScope Loop.Text)
            ...
            test "git sensing unavailable ⇒ InputUnavailable (exit 3)" {
                ...
                let model =
                    Interpreter.run (fakePorts validCatalog gitNotRepo cap) (requestFor Loop.DefaultRange Loop.Text)
