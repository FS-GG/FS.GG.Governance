    ...
    testList
        ...
        [ // ── US1 V4 (SC-004): a declared surface blocks verify --strict ──
            test "V4 — a declared verify-blocking path with no sensed change blocks verify --strict (SC-004)" {
                let m =
                    runWith (Loop.ExplicitPaths []) Strict [ handoffRead (declaringJson "\"src/Lib/Thing.fs\"") ]

                Expect.contains
                    (selectedIds m)
                    "package-api:build"
                    "the declared src path selects the verify-blocking package-api:build gate"

                Expect.equal
                    (Option.get m.Decision).Verdict
                    Fail
                    "the declared block-on-ship surface ⇒ Fail under Strict"
            ...
            test "V5a — no handoff ⇒ the verify doc is byte-identical to the pre-F082 projection (SC-002)" {
                ...
                let expected =
                    verifyExpected validCatalog candidates Standard (Some(snapshotOf gitSrcChange defaultOpts))

                Expect.equal
                    (Option.get m.VerifyDoc)
                    expected
                    "the absent-handoff verify doc matches the pre-seam pipeline (the identity merge)"
            }
