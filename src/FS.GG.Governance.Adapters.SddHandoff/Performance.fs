...
module Performance =
    ...
    let measurementKey (measurement: Fsgg.Schemas.PerformanceEvidenceMeasurement) =
        measurement.WorkloadId, measurement.P95Ms, measurement.P99Ms, measurement.MaxCatchUpFrames
    ...
    let evaluate (stale: bool) (evidence: Fsgg.Schemas.GovernanceHandoffPerformanceEvidence) : PerformanceEvaluation =
        let remediation =
        ...
        | Some intent when normalized intent.Disposition = "not-applicable" -> finish PerformanceNotApplicable [] []
        ...
        | None ->
            finish
                ...
                []
                [
                    "typed performance intent is required; self-attested performance evidence is insufficient"
                ]
        ...
        | Some intent ->
            ...
            let structuralFailures =
                [
                    ...
                    for workloadId, digest in validBindings do
                        ...
                        if not (Regex.IsMatch(digest, @"^sha256:[a-f0-9]{64}$", RegexOptions.CultureInvariant)) then
                    ...
                    for sample in samples do
                        ...
                        if
                            sample.WorkloadClass = "normal-play"
                            && not (Set.contains sample.WorkloadId declared)
                        then
                            $"{id} is an undeclared normal-play workload"

                        if Set.contains sample.WorkloadId declared then
                            if
                                Map.tryFind sample.WorkloadId expectedBindings
                        ...
                        if
                            List.isEmpty sample.PackageVersions
                            || sample.PackageVersions |> List.exists String.IsNullOrWhiteSpace
                        then
                            $"{id}.packageVersions must contain nonblank package identities"

                        if
                            List.isEmpty sample.Capabilities
                            || sample.Capabilities |> List.exists String.IsNullOrWhiteSpace
                        then
            ...
            let environmentFailures =
                [
                    if
                        ...
                        && samples
                           |> List.exists (fun sample -> sample.MeasurementMode <> "live-compositor")
                    then
            ...
            let producerFailures =
                ...
                [
                    if
                        produced |> List.map _.WorkloadId |> List.distinct |> List.length
                        <> produced.Length
                    then
                        "producer measurements contain duplicate workload ids"

                    if
                        not (List.isEmpty produced)
                        && List.map measurementKey produced <> List.map measurementKey measurements
                    then
                    ...
                    | Some false when
                        List.isEmpty structuralFailures
                        && List.isEmpty thresholdFailures
                        && List.isEmpty environmentFailures
                        ->
