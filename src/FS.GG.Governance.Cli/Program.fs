...
module Program =
    ...
    let markRequested (key: string) (budget: BudgetState) =
        { budget with
            Requested = addUnique key budget.Requested
        }

    let markHit (key: string) (budget: BudgetState) =
        { budget with
            CacheHits = addUnique key budget.CacheHits
        }

    let markMiss (key: string) (budget: BudgetState) =
        { budget with
            CacheMisses = addUnique key budget.CacheMisses
        }

    let markFresh (key: string) (budget: BudgetState) =
        { budget with
            FreshDispatches = addUnique key budget.FreshDispatches
        }

    let markPending (key: string) (budget: BudgetState) =
        { budget with
            Pending = addUnique key budget.Pending
        }

    let markExhausted (key: string) (budget: BudgetState) =
        { budget with
            BudgetExhausted = addUnique key budget.BudgetExhausted
        }
    ...
    let runHost (request: RunRequest) (snapshot: ProjectSnapshot) =
        ...
        let rec stepEffect (budget: BudgetState) (effect: FS.GG.Governance.Host.Effect) =
            ...
            | ReadArtifact artifact ->
                budget,
                [
                    FS.GG.Governance.Host.Msg.Sensed(artifact, ArtifactReading.readArtifact snapshot.Root artifact)
                ]
            | LoadReview key ->
            ...
            | RecordVerdict review ->
                budget,
                [
                    FS.GG.Governance.Host.Msg.Recorded(review.Key, ReviewStore.saveReview request review)
                ]
            | EmitOutput _ -> budget, []
    ...
    let writeOutput (request: RunRequest) (result: CommandResult) =
        try
            ...
            | Some path ->
                ...
                | Some dir when not (String.IsNullOrWhiteSpace dir) -> Directory.CreateDirectory(dir) |> ignore
                | _ -> ()
    ...
    let composeRouteView (root: string) (request: RunRequest) : ReportView.ReportView option =
        let model =
            FS.GG.Governance.RouteCommand.Interpreter.run (readOnlyRoutePorts root) (routeRequestFor root request)
    ...
    let drawTui (model: Tui.TuiModel) : unit =
        ...
        Console.Out.WriteLine(
            sprintf "report navigator — section %s  (up/down move, right/left expand/collapse, q quit)" path
        )
