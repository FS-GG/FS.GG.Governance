...
module CliRender =
    ...
    let evidenceStateText state =
        Json.ofEvidenceState state |> fun text -> text.Trim('"')
    ...
    let renderEvidenceNode (node: EvidenceNodeReport) =
        let declared =
            node.Declared |> Option.map evidenceStateText |> Option.defaultValue "none"

        let effective =
            node.Effective |> Option.map evidenceStateText |> Option.defaultValue "none"

        let freshness =
            node.Freshness |> Option.map freshnessText |> Option.defaultValue "unknown"
    ...
    let renderPayloadText (payload: CommandPayload) =
        ...
        | EvidencePayload report ->
            [
                ...
                yield!
                    report.Disclosures
                    |> List.map (fun d -> "- " + ruleIdText d.Rule + ": " + d.Justification)
            ]
            |> String.concat "\n"
        ...
        let requestLines =
            ...
            | Some request ->
                [
                    ...
                    "root: " + request.Root
                    "scope: "
                    + (if List.isEmpty request.Scope then
                           "."
                       else
                           String.concat "," request.Scope)
                ]
            | None -> [ "command: <usage>" ]
    ...
    let contractEntryJson (entry: ContractEntry) =
        ...
        "{"
        + "\"id\":"
        + quote (ruleIdText entry.Id)
        + ",\"severity\":"
        + quote severity
        + ",\"spec\":{\"document\":"
        + quote entry.Spec.Document
        + ",\"section\":"
        + quote entry.Spec.Section
        + "}"
        + ",\"statement\":"
    ...
    let handoffGateJson (gate: GatesModel.Gate) =
        "{"
        + "\"id\":"
        + quote (GatesModel.gateIdValue gate.Id)
        + ",\"maturity\":"
        + quote (maturityToken gate.Maturity)
        + ",\"blocking\":"
        + (if maturityBlocks gate.Maturity then "true" else "false")
        + ",\"description\":"
        + quote gate.Description
        + "}"
    ...
    let routeJson (route: Route) (handoffGates: GatesModel.Gate list) =
        ...
        + "\"kind\":\"route\""
        + ",\"stakes\":"
        + stakesJson route.Stakes
        + ",\"reason\":"
        + quote route.Reason
        + ",\"blocking\":["
        + (route.Blocking |> List.map contractEntryJson |> String.concat ",")
        + "]"
        + ",\"advisory\":["
        + (route.Advisory |> List.map contractEntryJson |> String.concat ",")
        + "]"
        + ",\"handoff\":["
        + (handoffGates |> List.map handoffGateJson |> String.concat ",")
        + "]"
        + "}"
    ...
    let evidenceNodeJson (node: EvidenceNodeReport) =
        ...
        + quote node.Id
        + ",\"declared\":"
        + optionStateJson node.Declared
        + ",\"effective\":"
        + optionStateJson node.Effective
        + ",\"freshness\":"
        + optionFreshnessJson node.Freshness
        + ",\"source\":"
        + quote node.Source
        + "}"
    ...
    let evidenceJson (report: ProjectEvidenceReport) =
        ...
        + (report.Disclosures
           ...
           |> List.map (fun d ->
               "{\"rule\":"
               + quote (ruleIdText d.Rule)
               + ",\"justification\":"
               + quote d.Justification
               + "}")
           |> String.concat ",")
        + "]"
        + "}"
    ...
    let requestJson (request: RunRequest) =
        "{"
        + "\"command\":"
        + quote (commandName request.Command)
        + ",\"mode\":"
        + quote (modeName request.Mode)
        + ",\"root\":"
        + quote request.Root
        + ",\"scope\":"
        + jsonArray request.Scope
        + ",\"format\":"
        + quote (formatName request.Format)
        + ",\"reviewBudget\":"
        + reviewBudgetJson request.ReviewBudget
        + ",\"judge\":{\"modelId\":"
        + quote request.Judge.ModelId
        + ",\"version\":"
        + quote request.Judge.Version
        + "}"
        + "}"
    ...
    let exitJson (decision: ExitDecision) =
        "{"
        + "\"category\":"
        + quote (exitCategory decision)
        + ",\"code\":"
        + string (Cli.exitCode decision)
        + "}"

    let budgetJson (budget: BudgetState) =
        "{"
        + "\"requested\":"
        + jsonArray budget.Requested
        + ",\"cacheHits\":"
        + jsonArray budget.CacheHits
        + ",\"cacheMisses\":"
        + jsonArray budget.CacheMisses
        + ",\"freshDispatches\":"
        + jsonArray budget.FreshDispatches
        + ",\"pending\":"
        + jsonArray budget.Pending
        + ",\"budgetExhausted\":"
        + jsonArray budget.BudgetExhausted
        + "}"

    let failuresJson (failures: Failure list) =
        failures |> List.map failureText |> jsonArray
    ...
    let renderJson (result: CommandResult) =
        ...
        + ",\"request\":"
        + (result.Request |> Option.map requestJson |> Option.defaultValue "null")
        + ",\"exit\":"
        + exitJson result.Exit
        + ",\"budget\":"
        + budgetJson result.Budget
        + ",\"failures\":"
        + failuresJson result.Failures
        + ",\"errors\":"
        + errorsJson result.Exit
        + ",\"payload\":"
        + (result.Payload |> Option.map payloadJson |> Option.defaultValue "null")
        + "}"
