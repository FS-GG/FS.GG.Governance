...
module DesignSystem =
    ...
    let identify (fact: DesignSystemFact) : FactId =
        ...
        | SurfaceObservation(probe, subject, _) -> FactId("observation:" + probe + ":" + Naming.artifactName subject)
            ...
            Eval =
                fun fs ->
                    ...
                    | Ok graph ->
                        ...
                        match effective |> Map.tryFindKey (fun _ s -> s = Synthetic || s = AutoSynthetic) with
