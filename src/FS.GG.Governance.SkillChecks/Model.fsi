...
module Model =
    ...
    /// `MirrorUnreadable` is a DECLARED mirror whose read failed — distinct from `NoMirrorDeclared`
    /// so a read error is never mislabelled as "no mirror declared" (#56/B12).
    type MirrorOutcome =
        | NoMirrorDeclared
        | MirrorInSync
