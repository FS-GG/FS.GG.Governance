...
module FSharpEffectBoundary =
    ...
    type Exemption =
        | NoExemption
        | ActiveExemption of owner: string * rationale: string * reviewBy: DateOnly
    ...
    let private names =
        ...
        | ClockOrRandomness -> "clock-randomness"
        | Network -> "network"
        | UiOrHost -> "ui-host"
        | Persistence -> "persistence"
        | MutableGlobalState -> "mutable-global-state"
    ...
    let private executableText (text: string) =
        ...
        let stringStart () =
            if startsWith "$@\"\"\"" || startsWith "@$\"\"\"" then
                Some(2, 3, false, true)
            elif startsWith "$\"\"\"" then
                Some(1, 3, false, true)
            elif startsWith "$@\"" || startsWith "@$\"" then
                Some(2, 1, true, true)
            elif startsWith "$\"" then
                Some(1, 1, false, true)
            elif startsWith "@\"" then
            ...
            while index < text.Length && not stopped do
                if stopAtInterpolationClose && text.[index] = '}' then
                    ...
                    while index < text.Length
                          && (depth > 0
                              || (index + 1 < text.Length && text.[index] = '(' && text.[index + 1] = '*')) do
    ...
    let private effectPatterns =
        ...
        [
            Filesystem,
            token
            + "(?<symbol>(?:System\\.IO\\.)?(?:File|Directory)\\.[A-Za-z_][A-Za-z0-9_']*)\\s*\\("
            Filesystem, token + "(?<symbol>(?:System\\.IO\\.)?FileStream)\\s*\\("
            Process, token + "(?<symbol>(?:System\\.Diagnostics\\.)?Process\\.Start)\\s*\\("
            Environment,
            token
            + "(?<symbol>(?:System\\.)?Environment\\.(?:GetEnvironmentVariable|SetEnvironmentVariable|Exit))\\s*\\("
            ...
            Persistence,
            token
            + "(?<symbol>[A-Za-z_][A-Za-z0-9_']*\\.(?:SaveChanges|SaveChangesAsync))\\s*\\("
            Persistence,
            token
            + "(?<symbol>(?:System\\.Text\\.Json\\.)?JsonSerializer\\.Serialize)\\s*\\("
    ...
    let private declarationPattern =
        "^//\\s*fsgg:effect-boundary\\s+(?<symbol>"
        + symbolPattern
        + ")(?<options>(?:\\s+[a-z][a-z-]*=(?:\"[^\"\\r\\n]+\"|[^\\s\"]+))*)\\s*$"

    let private optionPattern =
        "(?<key>[a-z][a-z-]*)=(?:\"(?<quoted>[^\"\\r\\n]+)\"|(?<plain>[^\\s\"]+))"

    let private bindingPattern =
        "^(?<indent>[ \\t]*)let\\s+(?:(?:inline|rec|private|internal|mutable)\\s+)*(?<symbol>"
        + symbolPattern
        + ")(?=\\s|\\(|=|:)"

    let private declarationBoundaryPattern =
        "^(?:let|and|type|module|namespace|exception|open)\\b|^\\[<"
    ...
    let private parseDeclaration source lineNumber (line: string) =
        ...
        if not declaration.Success then
            ...
            let pairs =
                ...
                |> Seq.map (fun option ->
                    let value =
                        if option.Groups.["quoted"].Success then
                            option.Groups.["quoted"].Value
                        else
                            option.Groups.["plain"].Value
    ...
    let private exemption source declaration =
        ...
        let present =
            values
            |> List.choose (fun (key, value) -> value |> Option.map (fun actual -> key, actual))

        match present with
        | [] -> Ok NoExemption
        | [ (_, owner); (_, rationale); (_, reviewText) ] ->
            match
                DateOnly.TryParseExact(reviewText, "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None)
            with
            | false, _ -> Error(sprintf "%s:%d has invalid exemption-review-by '%s'" source declaration.Line reviewText)
            | true, reviewBy when reviewBy <= DateOnly.FromDateTime(DateTime.UtcNow) ->
                Ok(InvalidExemption "review date expired")
            | true, reviewBy -> Ok(ActiveExemption(owner, rationale, reviewBy))
        | _ ->
            let missing =
                values
                |> List.choose (fun (key, value) -> if value.IsNone then Some key else None)

            Error(
                sprintf
                    "%s:%d has incomplete exemption options; missing %s"
                    source
                    declaration.Line
                ...
                Ok
                    {
                        ...
                        CallbackHiddenState =
                            visibleBody.Contains("Async.Start", StringComparison.Ordinal)
                            || visibleBody.Contains("ContinueWith", StringComparison.Ordinal)
                        ExceptionDrivenContinuation =
                            Regex.IsMatch(visibleBody, "\\btry\\b")
                            && Regex.IsMatch(visibleBody, "\\bwith\\b")
    ...
    let private senseSource project source (text: string) =
        ...
        let markerLines =
            ...
            |> Array.filter (fun (_, line) ->
                ...
                && (trimmed.Length = markerPrefix.Length
                    || Char.IsWhiteSpace trimmed.[markerPrefix.Length]))
            |> Array.toList
        ...
        let parseMarker (index, line) =
            ...
            | Ok declaration ->
                ...
                | None ->
                    Error(
                        sprintf
                            "%s:%d declaration '%s' has no following symbol"
                            source
                            declaration.Line
                            declaration.Symbol
                    )
                | Some candidate ->
                    match
                        ...
                        |> List.tryFind (fun binding ->
                            binding.StartLine = candidate && binding.Symbol = declaration.Symbol)
                    with
                    | None ->
                        Error(
                            sprintf
                                "%s:%d declaration '%s' does not bind the immediately following same-named let"
                                source
                                declaration.Line
                                declaration.Symbol
                        )
    ...
    let senseProject root project =
        try
            ...
            if not (File.Exists path) then
                Error(sprintf "F# project was not found: %s" project)
            else
                ...
                let sources =
                    ...
                    |> Seq.choose (fun node ->
                        node.Attribute(XName.Get "Include")
                        |> Option.ofObj
                        |> Option.map (fun a -> a.Value))
                    |> Seq.filter (fun source ->
                        source.EndsWith(".fs", StringComparison.OrdinalIgnoreCase)
                        && not (source.EndsWith(".fsi", StringComparison.OrdinalIgnoreCase)))
                    |> Seq.toList
                 ...
                 ||> List.fold (fun state source ->
                     ...
                     | Ok facts ->
                         ...
                         if not (File.Exists full) then
                             Error(sprintf "compiled F# source was not found: %s" source)
                         else
                             match senseSource project source (File.ReadAllText full) with
                             | Error reason -> Error reason
                             | Ok sensed -> Ok(facts @ sensed)))
        with ex ->
            Error(sprintf "unable to sense F# effect boundaries for '%s': %s" project ex.Message)
    ...
    let private emit request fact code detail input message =
        SC.mkFinding SC.DesignDomain packMaturity request fact.Source code detail Blocking input message

    let private excluded fact =
        fact.IsPureParserOrValidator
        || fact.IsThinOneShotAdapter
        ...
        | None, _ ->
            [
                emit
                    ...
                    fact.Symbol
                    false
                    "stateful workflow declares effects but no edge interpreter"
            ]
        | Some _, None ->
            [
                emit
                    ...
                    fact.Symbol
                    false
                    "edge interpreter must declare success/failure messages plus retry and idempotency semantics"
            ]
        | Some _, Some d ->
            [
                if d.SuccessMessage.IsNone || d.FailureMessage.IsNone then
                    yield
                        emit
                            request
                            fact
                            ...
                            fact.Symbol
                            false
                            "edge interpreter must return explicit success and failure messages into the transition"
                if d.RetryPolicy.IsNone then
                    yield
                        emit
                            request
                            fact
                            ...
                            fact.Symbol
                            false
                            "repeatable effect lacks visible retry policy"
                if d.Idempotency.IsNone then
                    yield
                        emit
                            request
                            fact
                            ...
                            fact.Symbol
                            false
                            "repeatable effect lacks visible idempotency semantics"
            ]
    ...
    let private findings request fact =
        ...
        | InvalidExemption reason ->
            [
                emit
                    ...
                    fact.Symbol
                    true
                    (sprintf "effect-boundary exemption is invalid: %s" reason)
            ]
        ...
        | _ ->
            let direct =
                ...
                |> List.map (fun call ->
                    emit
                        ...
                        (sprintf "%s %s@%d:%d" (names call.Category) call.Symbol call.Line call.Column)
                        false
                        (sprintf
                            "declared pure transition '%s' directly calls %s at %d:%d (%s); emit an effect for its edge interpreter"
                            fact.Symbol
                            call.Symbol
                            call.Line
                            call.Column
            ...
            let hidden =
                [
                    if fact.CallbackHiddenState then
                        yield
                            emit
                                request
                                fact
                                ...
                                fact.Symbol
                                false
                                "stateful workflow hides continuation state in a callback; model messages and requested effects explicitly"
                    if fact.ExceptionDrivenContinuation then
                        yield
                            emit
                                request
                                fact
                                ...
                                fact.Symbol
                                false
                                "stateful workflow uses exception-driven continuation; return explicit failure messages into the transition"
                ]

            let deliveryContract =
                if
                    not (List.isEmpty fact.DirectEffects)
                    || fact.EdgeInterpreter.IsSome
                    || fact.Delivery.IsSome
                then
                    delivery request fact
                else
                    []

            direct @ hidden @ deliveryContract

    let evaluate request boundaries =
        boundaries
        |> List.collect (findings request)
        |> List.sortBy (fun f -> f.Code, f.Location.File, f.Location.Detail)
