...
module CodeChecks =
    ...
    let safeSymbolName (symbol: FSharpSymbol) =
        try
            if String.IsNullOrWhiteSpace symbol.FullName then
                symbol.DisplayName
            else
                symbol.FullName
        with _ ->
    ...
    let isPurePath prefixes path =
        ...
        prefixes
        |> List.exists (fun prefix -> p.StartsWith(normalizePath prefix, StringComparison.Ordinal))
    ...
    let disposition request digest path symbol =
        ...
        | [] -> Missing
        | [ j ] when
            String.IsNullOrWhiteSpace j.SimplerAlternative
            || String.IsNullOrWhiteSpace j.Evidence
            ->
            Invalid
    ...
    let thresholdFinding request digest threshold measured id path symbol range subject =
        ...
        | Some limit when limit >= 0 && measured > limit ->
            candidate
                ...
                range
                (sprintf
                    "%s measures %d, above the configured review trigger %d; simplify it or bind measured justification to this symbol and source."
                    subject
                    measured
                    limit)
        | _ -> None
    ...
    let tokenRanges (path: string) (source: string) (tokenNames: Set<string>) =
        ...
        |> Array.mapi (fun index line ->
            ...
            let rec scan state acc =
                ...
                | Some token, next ->
                    let acc' =
                        if Set.contains token.TokenName tokenNames then
                            (index + 1, token.LeftColumn, token.RightColumn + 1, token.TokenName) :: acc
                        else
                            acc
    ...
    let hasReflectionOwner (name: string) =
        ...
        || (reflectionNamespaces
            |> List.exists (fun prefix -> name = prefix || name.StartsWith(prefix + ".", StringComparison.Ordinal)))
    ...
    let analyzeDocument request (doc: SourceDocument) =
        async {
            ...
            let! parsed, checkedAnswer =
                checker.ParseAndCheckFileInProject(path + ".fsx", 0, text, options)
                ...
                return
                    [
                        prohibited
                            ...
                            path
                            r
                            "F# compiler analysis aborted; the gate cannot report a clean result."
                    ],
                    optionMessages @ [ "compiler analysis aborted: " + path ]
            | FSharpCheckFileAnswer.Succeeded check when
                ...
                let first =
                    errors
                    |> List.tryHead
                    |> Option.defaultValue ("compiler analysis failed: " + path)
            ...
            | FSharpCheckFileAnswer.Succeeded check ->
                ...
                let addCandidate id symbol range message =
                    candidate request digest id path symbol range message
                ...
                let rec walkEntity (entity: FSharpEntity) =
                    ...
                    then
                        addCandidate
                            ...
                            symbol
                            er
                            "Public class shape requires evidence that a record, DU, or module cannot express the contract."
                    ...
                    if entity.IsAbstractClass then
                        addCandidate
                            ...
                            symbol
                            er
                            "Abstract class hierarchy requires a bounded interoperability or measured-complexity justification."

                    match entity.BaseType with
                    | Some baseType when
                        entity.IsClass
                        && baseType.HasTypeDefinition
                        && entityName baseType.TypeDefinition <> "obj"
                        && not (
                            (entityName baseType.TypeDefinition).StartsWith("System.Object", StringComparison.Ordinal)
                        )
                        ->
                        addCandidate
                            ...
                            symbol
                            er
                            "Inheritance hierarchy detected from compiler base-type facts; prefer a DU/module or justify the exception."
                    | _ -> ()

                    entity.MembersFunctionsAndValues
                    |> Seq.filter (fun m -> not m.IsCompilerGenerated && not m.IsImplicitConstructor)
                    |> Seq.iter (fun m ->
                        definitionSymbols.Add(safeSymbolName m, m.DeclarationLocation.StartLine, false))
                ...
                |> Array.choose (fun u ->
                    ...
                    | _ -> None)
                |> Array.distinctBy (fun (m, _) ->
                    safeSymbolName m, m.DeclarationLocation.StartLine, m.DeclarationLocation.StartColumn)
                |> Array.iter (fun (m, u) ->
                    ...
                    if m.IsModuleValueOrMember then
                        addCandidate
                            ...
                            symbol
                            (rangeOf u.Range)
                            "Module/global mutable state requires a simpler-local-state or measured/interoperability justification."
                    elif isPurePath request.PureDomainPrefixes path then
                        addCandidate
                            ImperativeInPureDomain
                            symbol
                            (rangeOf u.Range)
                            "Local mutation in a declared pure domain requires measured evidence or relocation to an imperative seam.")

                uses
                |> Array.filter (fun u -> not u.IsFromDefinition)
                |> Array.filter (fun u ->
                    openRanges
                    |> Array.exists (fun openRange -> rangeContains openRange u.Range)
                    |> not)
                ...
                |> Array.iter (fun u ->
                    ...
                    addCandidate
                        ...
                        symbol
                        (rangeOf u.Range)
                        "Reflection, quotations, or expression-tree use requires an interoperability or measured justification.")
                ...
                thresholdFinding
                    request
                    digest
                    request.Thresholds.ModuleLines
                    lineCount
                    ModuleSizeReview
                    path
                    path
                ...
                let orderedDefs =
                    definitionSymbols
                    |> Seq.distinct
                    |> Seq.sortBy (fun (_, line, isType) -> line, not isType)
                    |> Seq.toArray

                orderedDefs
                |> Array.iteri (fun i (symbol, startLine, isType) ->
                    let nextLine =
                        if i + 1 < orderedDefs.Length then
                            let _, line, _ = orderedDefs.[i + 1] in line
                        else
                            lineCount + 1
                    ...
                    let id, threshold, subject =
                        if isType then
                            TypeSizeReview, request.Thresholds.TypeLines, "Type region"
                        else
                            MemberSizeReview, request.Thresholds.MemberLines, "Member region"
                ...
                thresholdFinding
                    request
                    digest
                    request.Thresholds.DependencyFanOut
                    dependencies.Length
                ...
                let declared =
                    uses
                    |> Array.filter _.IsFromDefinition
                    |> Array.map (fun u -> safeSymbolName u.Symbol)
                    |> Set.ofArray
                    ...
                    |> List.iter (fun symbol ->
                        findings.Add(
                            prohibited
                                ...
                                fullRange
                                (sprintf
                                    "Capability '%s' already declares approved primitive(s) [%s]; replace duplicate '%s' or change the capability declaration."
                                    primitive.Capability
                                    (String.concat ", " primitive.ApprovedSymbols)
                                    symbol)
                        )))
